...
def get_roi_at_each_frame(bp_data, rois, check_inroi):
    ...
    def sort_roi_points(roi):
        return np.sort([roi.topleft[0], roi.bottomright[0]
    ...
    for idx, center in enumerate(centers):
        ...
        dist = np.hypot(np.subtract(cnt[:, 0], bp_data[:, 0]),
                        np.subtract(cnt[:, 1], bp_data[:, 1]))
        distances[:, idx] = dist
    ...
    if not check_inroi:
        ...
        for i, roi in enumerate(roi_at_each_frame):
            ...
            X, Y = sort_roi_points(
                rois[roi])  # get x,y coordinates of roi points
...
def get_timeinrois_stats(data,
    ...
    # Check arguments
    if data.shape[
            1] == 2:  # only X and Y tracking data passed, calculate speed
        speed = calc_distance_between_points_in_a_vector_2d(data)
        data = np.hstack((data, speed.reshape((len(speed), 1))))

    elif data.shape[1] != 3:
        raise ValueError(
            "Tracking data should be passed as either an Nx2 or Nx3 array. Tracking data shape was: {}. Maybe you forgot to transpose the data?"
            .format(data.shape))
    ...
    avg_time_in_roi = {
        ...
        for transits, time in zip(transitions_count.items(),
                                  data_time_inrois.values())
    }

    # avg time spend in each roi (seconds)
    if fps is not None:
        data_time_inrois_sec = {
            name: t / fps
            for name, t in data_time_inrois.items()
        }
        avg_time_in_roi_sec = {
            name: t / fps
            for name, t in avg_time_in_roi.items()
        }
    else:
