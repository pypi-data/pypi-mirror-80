...
def get_module_from_choice(connection, powerOnDevice, report_dict, return_val, is_qps=False):
    ...
    if str_pos != -1:
        ...
        try:
            ...
            comms.send_stop_test()
            logging.warning("Could not establish connection to root device. Test aborted")
    ...
    ret_dict = {"key_return": my_quarch_device.ConString, "value_return": return_val}
...
class QuarchComplianceSuite:
    ...
    def commandParser(self, data):
        ...
        try:
            ...
            logging.error("ERROR - Bad remote command format : " + str(traceback.print_exc()))
        except AttributeError as e:
            traceback.print_exc()
            logging.warning("Hit an attribute error, restarting : " + str(traceback.print_exc()))
            self.attempt_restart()
    ...
    def parse_request(self, xml_tree):
        ...
        if xml_tree.find('function'):
            ...
            if sub_elem.find('function_call').text == "init_test":
                ...
                # from quarchpy.disk_test.test.power_margining_test import PowerMarginingTest
                # self.test = PowerMarginingTest()

                self.test.check_prerequisites()
                if self.test.test_errors:
                    missing_imports = "\r\n".join(self.test.test_errors)
    ...
    def setup_mdns_server(self):
        ...
        if zeroConfAvail:
            try:
                ...
                mdns_info = ServiceInfo("_http._tcp.local.",
                                        "quarchCS._http._tcp.local.",
                                        self.tcp_port,
...
def main(argstring):
    ...
    if platform == "linux" or platform == "linux2":
        try:
            ...
            print("Quarch Compliance Suite requires linux package 'lsscsi', please install and restart this program")
            return
