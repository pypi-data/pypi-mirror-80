...
class IdGenerator:
    ...
    def up_tier(self, description=None, singular=False, ):
...
class BaseTest(ABC):
    ...
    def check_prerequisites(self):
        ...
        if os.name is not 'nt':
            if not is_tool("lsscsi"):
                self.test_errors.append("Lsscsi not found on server machine")
            if not is_tool("lspci"):
                self.test_errors.append("Lspci not found on server machine")
    ...
    def test_point(self, unique_id=None, function=None, function_args=None, function_description=None, debug=None,
                   test_description=None, has_return=False, stop_timeout=False):
        ...
        if self.execution_mode is "seek":
        ...
        if test_description:
            self.comms.sendMsgToGUI(self.comms.create_request_log(time.time(), "testDescription",
                                                                  test_description, os.path.basename(__file__)
    ...
    def check_point(self, unique_id, description, function, function_args, has_return=False):
        ...
        self.comms.sendMsgToGUI(self.comms.create_request_log(time.time(), "testResult", "description : " + description,
                                                              os.path.basename(
                                                                  __file__) + " - " + sys._getframe().f_code.co_name,
                                                              {"testResult": str(return_obj.return_item)},
                                                              uId=unique_id))
    ...
    def _execute_test_function(self, function_description, function, function_args, has_return, unique_id,
        ...
        if function_description:
            self.comms.sendMsgToGUI(self.comms.create_request_log(time.time(), "testDescription",
                                                                  "function : {}, description : {}".format(
                                                                      function.__name__, function_description),
        ...
        if has_return:
            ...
            if return_desc is not None:
                self.comms.sendMsgToGUI(self.comms.create_request_log(time.time(), "testResult",
    ...
    def request_qps(self):
        ...
        if not isQpsRunning(host=dtsGlobals.GUI_TCP_IP, timeout=5):
            ...
            print("starting qps")
            self.comms.sendMsgToGUI(self.comms.create_request_function("start_qps", path))
    ...
    def select_drive(self, drive_type="all"):
        ...
        self.test_point(self.test_id.gen_next_id(), function_description="Finding available drives",
                        function=self.my_host_info.get_drives, has_return=True, function_args={'drive_type': "all"})
    ...
    def select_quarch_module(self, use_qps=False, ip_lookup=None, filter_module=None):
        ...
        module_list = self.test_point(self.test_id.gen_next_id(), has_return=True,
                                      function_description="User selection – Quarch breaker module to use",
        ...
        if "rescan" in str(user_choice):
            ...
            self.select_quarch_module(use_qps=use_qps, ip_lookup=ip_lookup, filter_module=filter_module)
