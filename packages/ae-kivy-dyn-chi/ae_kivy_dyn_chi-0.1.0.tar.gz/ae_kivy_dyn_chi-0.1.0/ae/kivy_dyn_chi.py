...
:class:`DynamicChildrenBehavior` for to add children
widgets dynamically and data-driven to your kivy
container widgets (like DropDowns, Popups or any
layout widgets).
...
class DynamicChildrenBehavior:
    ...
    At least one of the classes that is mixing in this class has to inherit from Widget (or EventDispatcher)
    to get the :attr:`~DynamicChildrenBehavior.child_data_maps` attribute correctly initialized and firing on change.
    """
    ...
    * `attributes`: dict of attributes where the key is specifying the attribute name/path and the
    ...
    def __init__(self, **kwargs):
        ...
        self.fbind('on_child_data_maps', self.refresh_child_data_widgets)
