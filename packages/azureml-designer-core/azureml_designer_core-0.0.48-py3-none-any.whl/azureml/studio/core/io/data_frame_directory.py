...
class DataFrameDirectory(AnyDirectory):
    ...
    def create(cls,
        ...
        if isinstance(data, pd.DataFrame) and isinstance(schema, dict):
            data, schema = cls.remove_arrow_bug_columns_from_data_frame_and_schema(df=data, schema=schema)
    ...
    @classmethod
    def remove_arrow_bug_columns_from_data_frame_and_schema(cls, df: pd.DataFrame, schema: dict):
        invalid_columns = [index for index in _get_arrow_index_names(df) if index in df.columns]
        df_schema = DataFrameSchema.from_dict(schema)
        for name in invalid_columns:
            col_index = df_schema._get_column_index_by_key(name)
            df_schema._remove_from_label_score_feature(col_index=col_index)
            df_schema.column_attributes.remove(col_index)

        df = _remove_arrow_bug_columns(df)
        return df, df_schema.to_dict()
