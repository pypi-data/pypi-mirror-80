...
print(crayons.cyan(f'Discord server: https://discord.gg/fnpy - For support, questions, etc.'))
...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, **kwargs) -> None:
        self.status = '🔥 Made. By. Pirxcy Dont wear it out 🔥'
        ...
        self.default_emote = "EID_Floss"

        self.sanic_app = sanic_app
        self.server = server
        self.welcome_message = "Made. By. Pirxcy Dont wear it out"
        self.whisper_message = "Made. By. Pirxcy Dont wear it out"
    ...
    async def update_settings(self) -> None:
        while True:
            async with self.session.request(
                method="GET",
                url="https://ghostbin.co/paste/dru86/raw"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://ghostbin.co/paste/suzqc/raw"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://ghostbin.co/paste/fddqy/raw"
            ) as r:
                data = await r.json()
    ...
    async def pbjoin(self, ctx: fortnitepy.ext.commands.Context, *, epic_username: Optional[str] = None) -> None:
        if epic_username is None:
