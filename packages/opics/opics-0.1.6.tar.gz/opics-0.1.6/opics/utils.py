    ...
    def readfile(self):
        ...
        for each_line in temp_lines:
            ...
            if each_line.startswith("*"):
                ...
                each_line = "".join([''.join(filter(None,each_section.split(' '))) if ('"' in each_section) else each_section for each_section in re.split(r"""("[^"]*"|'[^']*')""", each_line)])
                ...
                if(len(temp_data)>1):#if line is not an empty one
                    ...
                    if(temp_data[0] == ".subckt"):
                        ...
                        for i in range(1, len(temp_data)):
                            ...
                            if("N$" in temp_data[i] and "N$None".lower()!= temp_data[i].lower()):
        ...
        #return all data
        return {"circuitNets": circuitNets, "circuitConns": circuitConns, "compLibs": componentLibs, "compModels": circuitModels, "compLabels": circuitLabels, "compAttrs":componentAttrs, "networkID": circuitID, "inp_net": inp_net, "out_net": out_net, "sim_params": freq_data, "OID": orthogonal_ID}