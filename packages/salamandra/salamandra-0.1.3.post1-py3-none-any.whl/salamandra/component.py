...
class Component(Object):
    ...
    def get_pinbusses(self):
        return self.__pinbusses.values()  # my_change
    ...
    def add_pin(self, pin):
        ...
        if pin_name in self.pin_names():
            # raise Exception('pin with same name already exists')
            raise Exception('pin "'+pin_name+'" already exists in component')  # my_change
    ...
    def add_pinbus(self, pinbus):
        ...
        if pinbus_name in self.pinbus_names():  # my_change
            raise Exception('pinbus "'+pinbus_name+'" already exists in component')

        # temporarily disable add_pin_adds_net feature so that pinbus is connected to netbus (later, and not to nets that are not part of bus)
    ...
    def get_netbus(self, netbus_name):  # my_change
    ...
    def add_netbus(self, netbus):
        ...
        if netbus_name in self.netbus_names():  # my_change
    ...
    def connect(self, net_str, pin_str):
        ...
        #
        #     if (inst, inst_pin) not in self.__virtual_pins:
        #         virtual_pin = Pin(inst + '.' + inst_pin)
        #         self.__virtual_pins[(inst, inst_pin)] = virtual_pin
        #         self.__virtual_pins[virtual_pin] = pin
        #
        #         pin = virtual_pin
        #     else:
        #         raise Exception('[' + inst + '.' + inst_pin + '] already connected')
        #
        # else:  # too deep
        #     # raise Exception('too deep [' + '.'.join(pin) + ']')
        #     raise Exception(pin_str + ' is too deep')  # my_change

        str2pin_dict = self.__str2pin(pin_str, is_bus=False)

        # Errors
        if not str2pin_dict['is_pin_exist']:
            if str2pin_dict['is_pin_of_subcomponent']:
                raise Exception('pin [' + str2pin_dict['subcomponent_pin_str'] + '] not found '
                                'in sub-component [' + str2pin_dict['subcomponent_str'] + ']')
            else:
                raise Exception('cannot find pin [' + pin_str + '] in component')
        if str2pin_dict['is_virtual_exist']:
    ...
    def disconnect(self, pin_str):
        ...
        #         pin = virtual_pin
        #     else:
        #         raise Exception('[' + inst + '.' + inst_pin + '] not found as connected')
        #
        # else:  # too deep
        #     # raise Exception('too deep [' + '.'.join(pin) + ']')
        ...
        if not str2pin_dict['is_pin_exist']:
            raise Exception('pin [' + str2pin_dict['subcomponent_pin_str'] + '] not found '
                            'in sub-component [' + str2pin_dict['subcomponent_str'] + ']')
        if not str2pin_dict['is_virtual_exist']:
            raise Exception('[' + str2pin_dict['subcomponent_str'] + '.' +
    ...
    def disconnect_bus(self, pinbus_str):
        ...
        # elif len(lpin)==2: #pin of sub-component
        #     inst, inst_pin = lpin # lpin[0]=instance name, pin[1]=pin name
        #     try:
        #         sub = self.__subcomponents[inst]
        #     except:  # todo: add type
        #         raise Exception('sub-component [' + inst + '] not found')
        #
        #     try:
        #         pinbus = self.get_pinbus(inst_pin) #next(p.bus() for p in sub.get_pins() if p.is_part_of_bus() and p.bus().get_name()==inst_pin)
        #     except:  # todo: add type
        #         raise Exception('cannot find pinbus [' + pinbus_str + '] in component')
        ...
        if not str2pin_dict['is_pin_of_subcomponent']:
            ...
            if not str2pin_dict['is_pinbus_exist']:
                raise Exception('pinbus [' + str2pin_dict['subcomponent_pin_str'] + '] not found '
                                'in sub-component [' + str2pin_dict['subcomponent_str'] + ']')
    ...
    def connect_bus(self, netbus_str, pinbus_str):
        ...
        # lpin = pinbus_str.split('.')
        # if len(lpin)==1:  # pin of component
        #     pinbus = self.get_pinbus(lpin[0]) #next(p.bus() for p in self.get_pins() if p.is_part_of_bus() and p.bus().get_name()==lpin[0])
        #     netbus = self.get_netbus(lpin[0]) #next(n.bus() for n in self.get_nets() if n.is_part_of_bus() and n.bus().get_name()==lpin[0])
        # elif len(lpin)==2: #pin of sub-component
        #     inst, inst_pin = lpin # lpin[0]=instance name, pin[1]=pin name
        #     try:
        #         sub = self.__subcomponents[inst]
        ...
        #
        #     try:
        #         pinbus = sub.get_pinbus(inst_pin)#next(p.bus() for p in sub.get_pins() if p.is_part_of_bus() and p.bus().get_name()==inst_pin)
        #     except:  # todo: add type
        #         raise Exception('cannot find pinbus [' + pinbus_str + '] in component')
        #
        # # my_change
        # else:  # too deep
        #     raise Exception(pinbus_str + ' is too deep')

        str2pin_dict = self.__str2pin(pinbus_str, is_bus=True)

        # Errors
        if not str2pin_dict['is_pin_of_subcomponent']:
            if not str2pin_dict['is_netbus_exist']:
    ...
    def __str2pin(self, pin_str, is_bus):
        ...
        if len(lpin) == 1:  # pin of component
            ...
            str2pin_dict['is_virtual_exist'] = False

            if not is_bus:  # connect & disconnect method
                try:
                    str2pin_dict['pin'] = self.get_pin(pin_str)
                    str2pin_dict['is_pin_exist'] = True
                except:  # todo: add type
                    str2pin_dict['is_pin_exist'] = False

            else:  # connect_bus & disconnect_bus method
                try:
                    str2pin_dict['pinbus'] = self.get_pinbus(pin_str)
                    str2pin_dict['is_pinbus_exist'] = True
                except:
                    str2pin_dict['is_pinbus_exist'] = False
                try:
                    str2pin_dict['netbus'] = self.get_netbus(pin_str)
                    str2pin_dict['is_netbus_exist'] = True
                except:
                    str2pin_dict['is_netbus_exist'] = False
            ...
            if not is_bus:  # connect & disconnect method
                ...
                if str2pin_dict['is_pin_exist']:
                    if (inst, inst_pin) not in self.__virtual_pins:  # for connect method
                        ...
                        str2pin_dict['is_virtual_exist'] = True
                        virtual_pin = self.__virtual_pins[(inst, inst_pin)]
                        del self.__virtual_pins[(inst, inst_pin)]
                        del self.__virtual_pins[virtual_pin]
                        str2pin_dict['pin'] = virtual_pin
...
5 - endpoints -dupes
    ...
    def all_fan_in(self, net_str, output_type=0):
        fan_in = []
        drive_pins = 0
                ...
                if pin.verilog_type() == "output":
                    if sub.__is_sequential:
                        ...
                        for inp in [p for p in sub.get_pins() if p.verilog_type() == "input"]:
                            fan_in.extend(x+'>'+pin_str for x in self.all_fan_in(inst_str+'.'+sub.__pin_connectivity[inp].get_object_name()))
            if drive_pins > 1:
    ...
    def all_fan_out(self, net_str, output_type=0):
        fan_out = []
        for pin_str in self.all_connected(net_str):
            lpin = pin_str.split('.')
            if len(lpin)==1:  # pin of component
                ...
                if pin.verilog_type() == "input":
                    if sub.__is_sequential:
                        ...
                        for inp in [p for p in sub.get_pins() if p.verilog_type() == "output"]:
                            fan_out.extend(pin_str+'>'+x for x in self.all_fan_out(inst_str+'.'+sub.__pin_connectivity[inp].get_object_name()))
        if output_type==0:
            return fan_out
        elif output_type==1:
            temp = {}
