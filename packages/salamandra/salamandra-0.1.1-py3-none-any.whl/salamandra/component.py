...
class Component(Object):
    ...
    def add_pinbus(self, pinbus):
        ...
        self.__pinbusses[pinbus.get_name()] = pinbus

        if self.add_pin_adds_net:
            n = pinbus.get_name()
            self.add_netbus(Bus(Net, n, pinbus.width()))
    ...
    def add_net(self, net):
        ...
        if net.get_object_name() in self.net_names():
            raise Exception('net "'+net.get_object_name()+'" already exists in component')

        net.associate(self)
        self.__nets[net.get_object_name()] = net #noytzach - performance
    ...
    def add_netbus(self, netbus):
        ...
        self.__netbusses[netbus.get_name()] = netbus
    ...
    def disconnect_bus(self, pinbus_str):
        ...
        if len(lpin)==1:  # pin of component
            ...
            try:
                pinbus = self.__pinbusses[inst_pin] #next(p.bus() for p in sub.get_pins() if p.is_part_of_bus() and p.bus().get_name()==inst_pin)
            except:  # todo: add type
    ...
    def connect_bus(self, netbus_str, pinbus_str):
        ...
        try:
            netbus = self.__netbusses[netbus_str] #next(n.bus() for n in self.get_nets() if n.is_part_of_bus() and n.bus().get_name()==netbus_str)
        except:  # todo: add type
            raise Exception('cannot find netbus [' + netbus_str + '] in component')

        lpin = pinbus_str.split('.')
        if len(lpin)==1:  # pin of component
            pinbus = self.__pinbusses[lpin[0]]#next(p.bus() for p in self.get_pins() if p.is_part_of_bus() and p.bus().get_name()==lpin[0])
            netbus = self.__netbusses[lpin[0]]#next(n.bus() for n in self.get_nets() if n.is_part_of_bus() and n.bus().get_name()==lpin[0])
            ...
            if netbus.width() == pinbus.width():
                for n, p in zip(netbus.all_bits(), pinbus.all_bits()):
                    self.connect(n.get_object_name(), inst + '.' + p.get_object_name())
            else:
                raise Exception('bus widths does not match')
    ...
    def uniq(self, count = None, numbering = None):
        ...
        for inst in sorted(self.get_subcomponents(), key=lambda t: my_fun(*re.match(r'([^\d]+)(\d+)([^\d]+)(\d+)',t+'0_0').groups())):
            dev = self.__subcomponents[inst]
    ...
    def write_netlist(self):
        ...
        #subcircuits
        for sub in self.get_subcomponents_recursive():
	...
	#main
        ...
        for inst in sorted(self.get_subcomponents(), key=lambda t: my_fun(*re.match(r'([a-zA-Z_]+)(\d+)',t).groups())):
            dev = self.__subcomponents[inst]
            l = inst + ' ('
            ports = []
