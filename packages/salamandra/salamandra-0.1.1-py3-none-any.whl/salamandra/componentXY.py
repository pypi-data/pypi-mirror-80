...
class ComponentXY(Component):
    def __init__(self, name):
        super(ComponentXY, self).__init__(name)
    ...
    def add_component(self, component, instance_name, xcoord=0.0, ycoord=0.0, xmin=None, ymin=None, xmax=None, ymax=None, fixed_OR_placed = 'fixed', group_type = None):
        self.add_subcomponent(component, instance_name, xcoord, ycoord, xmin, ymin, xmax, ymax, fixed_OR_placed, group_type)
		

    def add_subcomponent(self, component, instance_name, xcoord=0.0, ycoord=0.0, xmin=None, ymin=None, xmax=None, ymax=None, fixed_OR_placed = 'fixed', group_type = None):
    ...
    def set_position(self, instance_name, xcoord=0.0, ycoord=0.0, xmin=None, ymin=None, xmax=None, ymax=None, fixed_OR_placed = 'fixed', group_type = None):
    ...
    def set_component_dimensions(self, width, hight):
        self.__width = width
        self.__hight = hight
    ...
    def get_positions_recursive(self, path="", xmin=0.0, ymin=0.0, xmax=0.0, ymax=0.0, fixed_OR_placed='fixed', group_type=None):
        ...
        if len(comp_dict) == 0: #dictionary is empty --> leaf cell
            ...
            for inst, comp in comp_dict.items():
                ...
                if isinstance(comp, ComponentXY):
                    inst_pos_list = comp.get_positions_recursive(path+inst, xmin + inst_position[0], ymin + inst_position[1], xmax + inst_position[2], ymax + inst_position[3], inst_position[4], inst_position[5]) 
                    l += inst_pos_list
                else: 
                    raise Exception(str(inst)+' is not ComponentXY')
            return l
    ...
    def write_addStripe_tcl_commands(self, welltap_width = 0.4, min_stripe_spacing = 0.1, min_stripe_width = 0.1):
        ...
        print('\n==========+=============')
        dictt =self.get_subcomponents_dict()
        for k in sorted(dictt):
            #if dictt[k].get_object_name() == 'welltap_stripe':
            if re.match('welltap_stripe', dictt[k].get_object_name()):

                x_pos = round(self.get_positions_dict()[k][0],2)
                stripe_area = str(x_pos) + ' 0.0 '+ str(round(x_pos + 0.4,2)) +' '+ str(round(dictt[k].get_component_dimensions()[1],2))
