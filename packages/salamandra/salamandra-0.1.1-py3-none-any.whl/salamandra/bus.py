...
class Bus(Associable):
    def __init__(self, bit_type, name, width, associate_with = None):
    ...
    def verilog_declare(self, p):
        if p is self.get_bit(self.lsb()):
            return self.verilog_type() + ' [' + str(self.msb()) + ':' + str(self.lsb()) + '] ' + self.__name + ';'
        else:
            return None
    ...
    def all_bits(self):
        return (self.get_bit(i) for i in sorted(self.indices(), reverse=True))
