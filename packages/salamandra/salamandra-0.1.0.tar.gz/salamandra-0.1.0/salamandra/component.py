...
class Component(Object):
    ...
    def disconnect_bus(self, pinbus_str):
        ...
        if len(lpin)==1:  # pin of component
            ...
            try:
                pinbus = next(p.bus() for p in sub.get_pins() if p.is_part_of_bus() and p.bus().get_name()==inst_pin)
            except:  # todo: add type
    ...
    def connect_bus(self, netbus_str, pinbus_str):
        ...
        try:
            netbus = next(n.bus() for n in self.get_nets() if n.is_part_of_bus() and n.bus().get_name()==netbus_str)
        except:  # todo: add type
            raise Exception('cannot find netbus [' + netbus_str + '] in component')

        lpin = pinbus_str.split('.')
        if len(lpin)==1:  # pin of component
            pinbus = next(p.bus() for p in self.get_pins() if p.is_part_of_bus() and p.bus().get_name()==lpin[0])
            netbus = next(n.bus() for n in self.get_nets() if n.is_part_of_bus() and n.bus().get_name()==lpin[0])
