...
class FileHandler:
    ...
    def __init__(self, req_url):
        ...
        self.async_opt = ThreadPoolExecutor()
    ...
    def create_file(self, caller_code, remote_path, x_type, file=None, filename=None,
        ...
        :return:
        """
        channel = self._channel_create()
        stub = FileWorkerStub(channel)
        result = create_file(caller_code, remote_path, x_type, self.req_url, file=file, filename=filename,
                file_path=file_path, mount_path=mount_path, replace=replace,
                recovery_path=recovery_path, timeout=timeout, logger=logger, stub=stub)
        self.async_opt.submit(self._optmize, channel)
        return result
    ...
    def describe_file(self, caller_code, remote_full_path, local_full_path=None,
        ...
        channel = self._channel_create()
        stub = FileWorkerStub(channel)
        result = describe_file(caller_code, remote_full_path, self.req_url, local_full_path=local_full_path,
                  recovery_full_path=recovery_full_path, return_file=return_file,
                             timeout=timeout, logger=logger, stub=stub)
        self.async_opt.submit(self._optmize, channel)
        return result
    ...
    def modify_file(self, caller_code, remote_full_path, modified_file_name, recovery_full_path=None, replace=True, timeout=3, logger=logging):
        ...
        channel = self._channel_create()
        stub = FileWorkerStub(channel)
        result = modify_file(caller_code, remote_full_path, self.req_url, modified_file_name,
    ...
    def copy_file(self, caller_code, original_full_path, new_full_path, timeout=3, logger=logging):
        ...
        channel = self._channel_create()
        stub = FileWorkerStub(channel)
        result = copy_file(caller_code, original_full_path, self.req_url, new_full_path,
                         timeout=timeout, logger=logger, stub=stub)
        self.async_opt.submit(self._optmize, channel)
        return result
    ...
    def move_file(self, caller_code, original_full_path, new_full_path, timeout=3, logger=logging):
        ...
        channel = self._channel_create()
        stub = FileWorkerStub(channel)
        result = move_file(caller_code, original_full_path, self.req_url, new_full_path, timeout=timeout,
                         logger=logger, stub=stub)
        self.async_opt.submit(self._optmize, channel)
        return result

    def _channel_create(self):
        return grpc.insecure_channel(self.req_url, options=[
