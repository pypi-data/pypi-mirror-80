...
def get_ax_along(self, *args, unit="SI", is_norm=False, axis_data=[]):
    ...
    *args: list of strings
    ...
    if "axial" in self.components.keys():
        return_dict = self.components["axial"].get_along(args, unit=unit, is_norm=is_norm, axis_data=axis_data)
        return_dict[self.symbol + "_z"] = return_dict.pop(self.components["axial"].symbol)
        
    elif "z" in self.components.keys():
        return_dict = self.components["z"].get_along(args, unit=unit, is_norm=is_norm, axis_data=axis_data)
