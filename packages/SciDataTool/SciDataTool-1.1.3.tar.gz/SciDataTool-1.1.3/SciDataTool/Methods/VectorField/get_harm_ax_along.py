...
def get_harm_ax_along(self, N_harm, *args, unit="SI", is_norm=False, axis_data=[]):
    ...
    *args: list of strings
    ...
    if "axial" in self.components.keys():
        return_dict = self.components["axial"].get_harmonics(N_harm, args, unit=unit, is_norm=is_norm, axis_data=axis_data)
        return_dict[self.symbol + "_z"] = return_dict.pop(self.components["axial"].symbol)
        
    elif "z" in self.components.keys():
        return_dict = self.components["z"].get_harmonics(N_harm, args, unit=unit, is_norm=is_norm, axis_data=axis_data)
