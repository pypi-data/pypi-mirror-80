    ...
    *args: list of strings
    ...
    if "x" in self.components.keys() and "y" in self.components.keys():
        ...
        if "axial" in self.components.keys():
            resultz = self.components["axial"].get_along(args, unit=unit, is_norm=is_norm, axis_data=axis_data)
            field_z = resultz[self.components["axial"].symbol]
        elif "z" in self.components.keys():
            resultz = self.components["z"].get_along(args, unit=unit, is_norm=is_norm, axis_data=axis_data)
        ...
        resultr = self.components["radial"].get_along(args, unit=unit, is_norm=is_norm, axis_data=axis_data)
        field_r = resultr[self.components["radial"].symbol]
        shape = field_r.shape
        if "tangential" in self.components.keys():
            resultphi = self.components["tangential"].get_along(args, unit=unit, is_norm=is_norm, axis_data=axis_data)
