...
class ContractFunctions:
    ...
    """
        def get_{languageSpecificName}_event(
            ...
            tx_receipt = self._tron_trx.getTransactionReceipt(tx_hash)
            return self._tron_trx.contract(address=to_checksum_address(self.contract_address), abi={{contractName}}.abi()).events.{{name}}().processReceipt(tx_receipt)
    ...
    def print_functions(self):
        ...
        for func in self._functions:
            ...
            if func['type'] == 'constructor':
                self._p('constructor', func)

    def _p(self, typecontrol, func):
        method = "{} {}({}) -> {}"
        print(method.format(typecontrol, func['name'], func['input']), func['output'])
