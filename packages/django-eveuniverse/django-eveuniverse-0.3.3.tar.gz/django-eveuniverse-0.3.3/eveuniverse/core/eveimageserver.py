...
def _eve_entity_image_url(
    ...
    # input validations
    categories = {
        _ESI_CATEGORY_ALLIANCE: {"endpoint": "alliances", "variants": ["logo"]},
        _ESI_CATEGORY_CORPORATION: {"endpoint": "corporations", "variants": ["logo"]},
        _ESI_CATEGORY_CHARACTER: {"endpoint": "characters", "variants": ["portrait"]},
    ...
    # compose result URL
    result = "{}/{}/{}/{}?size={}".format(
        _EVE_IMAGE_SERVER_URL, endpoint, entity_id, variant, size
    )
    if tenant:
...
def corporation_logo_url(corporation_id: int, size: int = _DEFAULT_IMAGE_SIZE) -> str:
    ...
    return _eve_entity_image_url(_ESI_CATEGORY_CORPORATION, corporation_id, size)
...
def faction_logo_url(faction_id: int, size: int = _DEFAULT_IMAGE_SIZE) -> str:
    """image URL for the given alliance ID"""
    return _eve_entity_image_url(_ESI_CATEGORY_FACTION, faction_id, size)
