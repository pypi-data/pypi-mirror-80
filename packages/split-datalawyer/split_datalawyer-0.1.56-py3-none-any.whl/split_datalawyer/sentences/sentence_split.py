...
class SentenceSplit():
    ...
    def __init__(self, stop_patterns_path=__DEFAULT_STOP_PHRASES__, debug_log=False, modules=[]):
    ...
    def _get_contunuation_terms(self):
        continuation_terms_path = os.path.join(self._get_local_directory(), "continuation_terms.csv")
    ...
    def _concat_broken_phrases(self, df):
        continuation_terms = [",", "no", "na", "nos", "nas", "de", "do", "da", "dos", "das", "sua", "pela", "pelo",
                              "pelas", "pelos", "para", ]
        ...
        for i in range(len(df['final_text'])):
            ...
            current_text_end_with_abbreviation = current_text.split()[-1].lower() in self._list_abbreviations
                ...
                for j in range(i + 1, len(df['final_text'])):
                    ...
                    if j - previous_phrase_index >= 0:
                        ...
                        if previous_text is not "":
                            previous_end = previous_text.split()[-1].lower()

                            previous_ends_with_continuation = previous_end in continuation_terms
                            previous_text_end_with_abbreviation = previous_end in self._list_abbreviations
                    ...
                    if (next_text.split()[0][0].islower() or
                        ...
                        next_text_end_not_with_abbreviation = not str(
                            next_text.split()[-1]).lower() in self._list_abbreviations
    ...
    def get_sentences_with_index(self, text, remove_duplicated=True, concat_phrases=True, concat_inverted_phrases=True,
        ...
        if remove_stop_phrases:
            df = self._remove_stop_phrases(df)
