...
class DiMotif12Classificator(MotifClassificator):
    ...
    def __call__(self, motif):
        a, b1, b2 = motif
        cl = [""] * 3
        cl[0] = "I" * (self.G.has_edge(b1, b2) + self.G.has_edge(b2, b1) + 1)
        if self.G.has_edge(b1, b2) ^ self.G.has_edge(b2, b1):
            # asymmetrical
            # suppose x -> y
            x = b1 if self.G.has_edge(b1, b2) else b2
            y = b2 if self.G.has_edge(b1, b2) else b1
            int_code = sma.binaryArrayHornersMethod([self.G.has_edge(a, x),
                                                     self.G.has_edge(x, a),
                                                     self.G.has_edge(a, y),
                                                     self.G.has_edge(y, a)])
            if int_code == 0:
                ...
                cl[1:] = "C", str(int_code - 4)
            elif int_code == 8:
                cl[1:] = "B", "5"
            elif int_code <= 11:
                cl[1:] = "C", str(int_code - 5)
            elif int_code == 12:
                cl[1:] = "B", "6"
            else:
                cl[1:] = "C", str(int_code - 6)
        else:
            # symmetrical
            if ((self.G.has_edge(a, b1) or self.G.has_edge(b1, a)) and
                (self.G.has_edge(a, b2) or self.G.has_edge(b2, a))):
                cl[1] = "C"
                if (self.G.has_edge(a, b1) and self.G.has_edge(a, b2) and
                    self.G.has_edge(b1, a) and self.G.has_edge(b2, a)):
                    cl[2] = "6"
                elif ((self.G.has_edge(a, b1) and self.G.has_edge(b1, a) and self.G.has_edge(a, b2)) or
                      (self.G.has_edge(a, b2) and self.G.has_edge(b2, a) and self.G.has_edge(a, b1))):
                    cl[2] = "5"
                elif ((self.G.has_edge(a, b1) and self.G.has_edge(b1, a) and self.G.has_edge(b2, a)) or
                      (self.G.has_edge(a, b2) and self.G.has_edge(b2, a) and self.G.has_edge(b1, a))):
                    cl[2] = "4"
                elif self.G.has_edge(a, b1) and self.G.has_edge(a, b2):
                    cl[2] = "3"
                elif self.G.has_edge(b1, a) and self.G.has_edge(b2, a):
                    cl[2] = "1"
                else:
                    cl[2] = "2"
            elif (self.G.has_edge(a, b1) or self.G.has_edge(b1, a) or
                  self.G.has_edge(a, b2) or self.G.has_edge(b2, a)):
                cl[1] = "B"
                if ((self.G.has_edge(a, b1) and self.G.has_edge(b1, a)) or
                    (self.G.has_edge(a, b2) and self.G.has_edge(b2, a))):
                    cl[2] = "3"
                elif self.G.has_edge(a, b1) or self.G.has_edge(a, b2):
                    cl[2] = "2"
                else:
                    cl[2] = "1"
            else:
