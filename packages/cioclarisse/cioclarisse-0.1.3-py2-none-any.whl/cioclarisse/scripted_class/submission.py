...
import ix
from cioclarisse.scripted_class import missing_files_ui
...
class Submission(object):
    ...
    def _get_render_package_path(self):
        ...
        return Path("{}.cio.project".format(path))
    ...
    def common_context(self):
        ...
        def comps(image):
            return str(image).replace("project://", "").split("/")[:-1]
        ...
        levels = zip(*[comps(image) for image in images])
        common = [x[0] for x in takewhile(_all_the_same, levels)]
        if not len(common):
            return
        ctx = "project://"+"/".join(common)
        return ix.get_item(ctx)
    ...
    def write_render_package(self):
        ...
        print "COMMON CONTEXT IS", context
        package_file = self.render_package_path.posix_path()
        print "PACKAGE_FILE IS", package_file
        ...
        print "App should be re-enabled now", context
        self._after_write_package()

        if not success:
            msg = "Failed to export render package {}".format(package_file)
            raise ConductorError(msg)
    ...
    #                 msg=  "{} - while resolving reference {}.filename = {}".format( str(ex), str(context), filename )
    #                 raise ConductorError(msg)
    ...
    def _copy_system_dependencies_to_temp(self):
        ...
        ../clarisse_config.py
        """
        print  "_copy_system_dependencies_to_temp()"
        ...
        print  "DONE _copy_system_dependencies_to_temp()"
