...
class Client:
    ...
    def get_configurable_options(self, category: str) -> List[ConfigItem]:
        """
        Get all config options configurable through expert settings

        """
        req_ = dict(category=category)
    ...
    def list_pretransformers(self) -> List[PreTransformerWrapper]:
        """
    ...
    def list_datas(self) -> List[DataWrapper]:
        """
    ...
    def get_orig_shapley_job(self, key: str) -> OriginalShapleyJob:
        """

        """
        req_ = dict(key=key)
        res_ = self._request('get_orig_shapley_job', req_)
        return OriginalShapleyJob.load(res_)
    ...
    def get_dt_surrogate_job(self, key: str) -> DecisionTreeSurrogateJob:
        """

        """
        req_ = dict(key=key)
        res_ = self._request('get_dt_surrogate_job', req_)
        return DecisionTreeSurrogateJob.load(res_)
    ...
    def get_experiment_tuning_suggestion(self, dataset_key: str, target_col: str, is_classification: bool, is_time_series: bool, config_overrides: str, cols_to_drop: List[str], is_image: bool) -> ModelParameters:
        """

        """
        validation.validate_toml(config_overrides, 'config_overrides')
        req_ = dict(dataset_key=dataset_key, target_col=target_col, is_classification=is_classification, is_time_series=is_time_series, config_overrides=config_overrides, cols_to_drop=cols_to_drop, is_image=is_image)
        res_ = self._request('get_experiment_tuning_suggestion', req_)
        return ModelParameters.load(res_)

    def get_experiment_preview(self, dataset_key: str, validset_key: str, classification: bool, dropped_cols: List[str], target_col: str, is_time_series: bool, time_col: str, enable_gpus: bool, accuracy: int, time: int, interpretability: int, fold_column: str, config_overrides: str, reproducible: bool, resumed_experiment_id: str) -> str:
        """

        """
        validation.validate_toml(config_overrides, 'config_overrides')
        req_ = dict(dataset_key=dataset_key, validset_key=validset_key, classification=classification, dropped_cols=dropped_cols, target_col=target_col, is_time_series=is_time_series, time_col=time_col, enable_gpus=enable_gpus, accuracy=accuracy, time=time, interpretability=interpretability, fold_column=fold_column, config_overrides=config_overrides, reproducible=reproducible, resumed_experiment_id=resumed_experiment_id)
    ...
    def get_mli_subtask_status(self, mli_key: str, subtask_type: str) -> MliSubtaskStatus:
        """

        """
        req_ = dict(mli_key=mli_key, subtask_type=subtask_type)
        res_ = self._request('get_mli_subtask_status', req_)
        return MliSubtaskStatus.load(res_)

    def get_mli_subtask_statuses(self, mli_key: str, subtask_types: List[str]) -> List[MliSubtaskStatus]:
        """

        """
        req_ = dict(mli_key=mli_key, subtask_types=subtask_types)
        res_ = self._request('get_mli_subtask_statuses', req_)
        return [MliSubtaskStatus.load(b_) for b_ in res_]
