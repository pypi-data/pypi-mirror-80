...
class Experiments:
    ...
    def __init__(self, client: "_core.Client") -> None:
        ...
        self._default_experiment_settings = {
            setting.name.strip(): _utils.sanitize_server_setting(setting.val)
    ...
    def _parse_api_settings(self, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for setting, value in settings.items():
            if setting == "task":
                ...
                config_overrides += f"\n{_utils.api_setting_to_toml(setting, value)}"
        custom_settings["config_overrides"] = config_overrides
    ...
    def _parse_server_settings(self, server_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        settings.update(
            _utils.toml_to_api_settings(
                toml=server_settings["config_overrides"],
    ...
    def download(
        ...
        if len(self._jobs) == 1:
            return self._client._download(
                server_path=self._jobs[0]._info.entity.predictions_csv_path,
                dst_dir=dst_dir,
                dst_file=dst_file,
                overwrite=overwrite,
            )
        ...
        with tempfile.TemporaryDirectory() as pred_dir:
            tmp_paths = [
                self._client._download(
                    server_path=job._info.entity.predictions_csv_path,
                    dst_dir=pred_dir,
                    overwrite=True,
                    verbose=False,
                )
