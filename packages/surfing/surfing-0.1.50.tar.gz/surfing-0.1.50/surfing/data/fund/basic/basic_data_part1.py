...
from ....constant import SectorType, IndexPriceSource, INDEX_PRICE_EXTRA_TRADE_DAYS, FUND_NAV_EXTRA_TRADE_DAYS
...
class BasicDataPart1:
    ...
    def fund_info_from_raw(self, fund_id_list: Set[str], benchmark_profile: pd.Series):
        ...
        def _invest_type_class_to_wind_class_2(invest_type_class: str) -> str:
            '''Choice的基金二级类型转换为wind的'''

            if invest_type_class == '混合债券型基金（一级）':
                return '混合债券型一级基金'
            elif invest_type_class == '长期纯债型基金':
                return '中长期纯债型基金'
            elif invest_type_class == '中短期纯债型基金':
                return '短期纯债型基金'
            elif invest_type_class == '混合债券型基金（二级）':
                return '混合债券型二级基金'
            elif invest_type_class == '被动指数型基金':
                return '被动指数型债券基金'
            elif invest_type_class in ('QDII基金', '封闭式基金', '保本型基金'):
                # error case
                return
            else:
                return invest_type_class
        ...
        df['wind_class_2'] = df.invest_type_class_2.transform(_invest_type_class_to_wind_class_2)
    ...
    def fund_status_latest(self, end_date: str):
        try:
            ...
            # 将场内变为场外的
            df['CODES'] = df.CODES.transform(lambda x: x.split('.')[0] + '.OF' if '.OF' not in x else x)
            df = df.pivot(index='DATES', columns='CODES', values=['PURCHSTATUS', 'REDEMSTATUS', 'LPLIMIT', 'OTCLPLIMITJG', 'OTCLPLIMITGR', 'TRADESTATUS']).fillna(method='ffill')
    ...
    def fund_size_hold_rate_history(self):
        ...
        fund_holding_rate = raw.get_em_fund_holding_rate(start_date=start_date, end_date=end_date)
        ...
        fund_scale = fund_scale.pivot(index='DATES', columns='CODES', values='FUNDSCALE').fillna(method='ffill').stack()
        fund_scale = pd.DataFrame(fund_scale, columns=['FUNDSCALE']).reset_index()
        fund_scale['fund_id'] = fund_scale.apply(
            lambda x: self._data_helper._get_fund_id_from_order_book_id(x['CODES'].split('.')[0], x['DATES']), axis=1)
        fund_scale = fund_scale.drop(['CODES'], axis=1).rename(columns={'DATES': 'datetime', 'FUNDSCALE': 'size'})
        fund_scale = fund_scale.dropna().drop_duplicates(subset=['size', 'fund_id'], keep='first')

        df = fund_scale.set_index(['fund_id', 'datetime']).join(fund_holding_rate.set_index(['fund_id', 'datetime']), how='outer').reset_index()
    ...
    def fund_hold_asset(self, start_date: str, end_date: str):
        ...
        em_hold_asset = em_hold_asset.dropna(subset=['CODES']).drop(columns='_update_time')
        name_dict = {
            'CODES': 'fund_id',
            'DATES': 'datetime',
    ...
    def fund_hold_industry(self, end_date: str):
        ...
        em_hold_industry['CODES'] = em_hold_industry.apply(lambda x:
                            self._data_helper._get_fund_id_from_order_book_id(x.CODES.split('.')[0], x.DATES), axis=1)
        em_hold_industry = em_hold_industry.dropna(subset=['CODES']).drop(columns='_update_time')
        name_dict = {
            'CODES': 'fund_id',
            'DATES': 'datetime',
        }
