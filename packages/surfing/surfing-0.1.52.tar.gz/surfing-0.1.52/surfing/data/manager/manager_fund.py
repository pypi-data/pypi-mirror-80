...
from .fund_info_filter import filter_fund_info, fund_info_update, active_fund_info
...
class FundDataManager(DataManager):
    ...
    def __init__(self, start_time=None, end_time=None, fund_score_param: FundScoreParam = None, score_manager: FundScoreManager = None, s3_retriever_activation: bool = False):
    ...
    def _init_from_s3_with_score_customized(self, score_pre_calc=True, use_weekly_monthly_indicators=False):
    ...
    def try_to_init_from_s3(self, default_value_flag: List[bool], score_pre_calc, use_weekly_monthly_indicators) -> bool:
    ...
    def init(self, index_list=None, score_pre_calc=True, print_time=False, use_weekly_monthly_indicators=False):
        if self._s3_retriever.is_activated:
            ...
            if self.try_to_init_from_s3([self.start_date == std_dm.start_date,
                                        ...
                                        ], score_pre_calc, use_weekly_monthly_indicators):
                # 从s3成功读取数据，直接返回
                return
        ...
        index_list = [_.replace(INDEX_REPLACE[0],INDEX_REPLACE[1]) for _ in index_list]
        ...
        def _fund_manager_score_filter(query):
            def _fund_manager_score_filter_etl(x):
                return x[x.fund_type.isin(['stock']) & (x.datetime >= self.start_date) & (x.datetime <= self.end_date)].reset_index(drop=True)
        ...
        with BasicDatabaseConnector().managed_session() as quant_session:
            ...
            in_market_fund_list = self.dts.fund_status_latest[self.dts.fund_status_latest['purch_status'] == '场内交易'].fund_id.tolist()
            ...
            self.dts.index_price = index_price_raw.pivot_table(index='datetime', columns='index_id', values='close').reindex(trim_trading_days).fillna(method='ffill').rename(columns = {INDEX_REPLACE[1]:INDEX_REPLACE[0]})
        ...
        with DerivedDatabaseConnector().managed_session() as derived_session:
            ...
            self.dts.fund_manager_score = self.dts.fund_manager_score.pivot_table(index='datetime', columns='manager_id').reindex(trim_trading_days).stack().reset_index()
            ...
            self.dts.fund_manager_rank['fund_list'] = [ json.loads(i) for i in self.dts.fund_manager_rank.fund_list]
            self.dts.fund_manager_rank = self.dts.fund_manager_rank.set_index(['datetime','mng_id'])
            ...
            _df = _df.explode('fund_list').dropna()
            _df = _df.reset_index().drop('mng_id',axis=1).rename(columns={'fund_list':'fund_id'})
            ...
            #self.dts.fund_indicator['active'] = self.dts.fund_indicator['active'].fillna(0)
    ...
    def find_same_size_type_funds(self, desc_name):
        ...
        fund_info['trim_name'] = fund_info.desc_name.map(lambda x : re.subn(r'[ABCDEFR]{1,2}(\(人民币\)|\(美元现汇\)|\(美元现钞\)|1|2|3)?$', '', x))
