...
class FundAlphaProcessor:
    ...
    def init(self, start_date: str, end_date: str):
        # self.log = []
        # 向前取12年
        start_date: str = (pd.to_datetime(start_date, infer_datetime_format=True) - datetime.timedelta(days=self._NATURAL_DAYS_PER_YEAR*12+1)).strftime('%Y%m%d')
    ...
    def calculate(self):
        fund_list = self._fund_nav.columns.intersection(self._benchmark_ret.columns).to_list()
        p = Pool()
