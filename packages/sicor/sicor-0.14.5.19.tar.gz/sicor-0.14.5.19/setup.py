...
# define packages needed for the multispectral tables download (except for pyprind, all packages are installed
# automatically using anaconda, but need to be listed here for environments based on miniconda)
setup_requirements = requirements + ["setuptools-git"]
...
setup(
    ...
    package_data={"sicor": ["AC/data/*", "tables/*", "options/*"]},
...
if os.path.exists(path_LUT_default):
    print("EnMAP AC LUT is already available. Skipping download.")
    pass
else:
    print("Downloading EnMAP AC LUT from Git repository.")
...
from sicor.options import get_options, python_to_json
sicor_table_path = os.path.join(path_sicorlib, "tables")
for sensor in ["s2", "l8"]:
    get_tables(sicor_table_path=sicor_table_path, sensor=sensor)
    # update multispectral user options
    ...
    for scat_type, op in opts["RTFO"].items():
        table_fn = os.path.basename(op["atm_tables_fn"])
        op["atm_tables_fn"] = os.path.join(sicor_table_path, table_fn)

    clou_mask_fn = os.path.basename(opts['cld_mask']["persistence_file"])
    nov_det_fn = os.path.basename(opts['cld_mask']["novelty_detector"])
    opts['cld_mask']["persistence_file"] = os.path.join(sicor_table_path, clou_mask_fn)
    opts['cld_mask']["novelty_detector"] = os.path.join(sicor_table_path, nov_det_fn)
...
if not util.find_spec('pygrib'):
    warnings.warn('You need to install pygrib manually (use pip or conda install pygrib) if you are using Linux'
