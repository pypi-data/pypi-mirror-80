...
class Pymata4(threading.Thread):
    ...
    def __init__(self, com_port=None, baud_rate=115200,
        ...
        self.report_dispatch.update({PrivateConstants.DFROBOT_MESSAGE: [self._dfrobot_message, 7]})
    ...
    def set_pin_analog_input(self, pin_number, callback=None):
        ...
        command = [PrivateConstants.SET_PIN_MODE, pin_number, PrivateConstants.ANALOG]
    ...
    def neopixel_config(self,pin_number, n):
        data = [PrivateConstants.SUB_MESSAGE_NEOPIXEL, 0x00, pin_number, n>>7, n&0x7F]
        self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, data)
    ...
    def neopixel_set_range_color(self, pin_number, start, end, color):
        data = [PrivateConstants.SUB_MESSAGE_NEOPIXEL, 0x01, pin_number, start>>7, start&0x7F, end>>7, end&0x7F, (color>>21)&0x7F, (color>>14)&0x7F, (color>>7)&0x7F, color&0x7F]
        self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, data)

    def neopixel_set_rainbow(self, pin_number, start, end, hsv_start, hsv_end):
        data = [PrivateConstants.SUB_MESSAGE_NEOPIXEL,0x03, pin_number, start>>7, start&0x7F, end>>7, end&0x7F, (hsv_start>>21)&0x7F, (hsv_start>>14)&0x7F, (hsv_start>>7)&0x7F, hsv_start&0x7F, (hsv_end>>21)&0x7F, (hsv_end>>14)&0x7F, (hsv_end>>7)&0x7F, hsv_end&0x7F]
        self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, data)
    ...
    def show_bar_graph(self, pin_number, start, end, numerator, denominator):
        data = [PrivateConstants.SUB_MESSAGE_NEOPIXEL, 0x06, pin_number, start>>7, start&0x7F, end>>7, end&0x7F, numerator, denominator]
        self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, data)
    ...
    def ir_send(self, pin_number, value):
      data = [PrivateConstants.SUB_MESSAGE_IR, 0x00, pin_number, (value<<28)&0x7F, (value<<21)&0x7F, (value<<14)&0x7F, (value<<7)&0x7F, value&0x7F]
      self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, data)
    ...
    def _dfrobot_message(self, data):
        ...
        if data[0] == PrivateConstants.SUB_MESSAGE_I2CSCAN:
            ...
            value = data[2] + (data[3]<<7) + (data[4]<<14) + (data[5]<<21) + (data[6]<<28)
            pin_number = data[1]
            #method = None
