...
class DeviceTab(QtWidgets.QWidget, Ui_DeviceTab):
    ...
    def extra_ui_setup(self):
        ...
        sp = self.bootloaderIndicator.sizePolicy()
        sp.setRetainSizeWhenHidden(True)
        self.bootloaderIndicator.setSizePolicy(sp)
    ...
    def setup_channel(self, stream_id, stream, channel_id, channel,
        ...
        index = len(self.channel_names)  # the index into many of the arrays
    ...
    def handle_alerts(self):
        ...
        for key in sorted(self.alerts.keys()):
            ...
            for i, (t, v) in enumerate(zip(alert_triggered, alert_value)):
                if t:
                    subchannel_name = self.subchannel_names[key[0]][key[1]]
                    uf = self.unit_formatters[key[0]]
    ...
    def create_edit_alert_action(self, channel_id, channel_index,
        ...
        def update_alert():
            ...
            # reset alert state
            try:
                channel_fields = self.subchannel_fields[channel_index]
                mean, std = channel_fields[subchannel_index]
                mean.set_alert(False)
                std.set_alert(False)
            except IndexError:
