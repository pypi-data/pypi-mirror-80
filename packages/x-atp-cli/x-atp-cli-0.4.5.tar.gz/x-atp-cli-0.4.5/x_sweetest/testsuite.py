...
from x_sweetest.globals import g, timestamp
...
class TestSuite:
    ...
    def setup(self, testcase, case):
        ...
        # 如果前置条件为setup的用例执行失败，则尝试一次先执行BASE的用例，再执行setup用例
        if setup_flag == 'N':
    ...
    def run_testcase(self, testcase):
        ...
        # 前置添加为BASE,则标准输出说明，并继续执行用例
        if testcase['condition'].lower() == 'base':
            logger.info('*** BASE testcase ↓ ***')
        # 前置条件为setup，则先跳出不执行这条用例，
        ...
        if testcase['condition'].lower() not in ('base', 'setup'):
            ...
            # 如果前置条件为 skip，则此用例不执行前置条件
                ...
                # 执行前置条件为setup的用例，并返回执行结果
                result = self.setup(testcase, case)
                # if result == 'N':
                # 前置条件setup执行失败，则直接跳出不执行当前用例
                if not result:
                    # 统计结束时间
                    testcase['end_timestamp'] = timestamp()
                    return
        ...
        try:
            ...
            if testcase['result'] == 'success':
                ...
                # 用例执行失败时，判断是BASE用例，则输出提示信息
                if testcase['condition'].lower() == 'base':
                    logger.warn('Run the testcase: %s|%s Failure, BASE is not success. Break the AutoTest' % (
                        testcase['id'], testcase['title']))
                    # BASE用例执行失败，则将阻断标志置为True，后续用例均不会执行
                    self.blcoked_flag = True
                    return
                # ---- 以下这段代码源码中有开启，但是实际没必要，因为setup的用例在self.setup函数中执行，并返回结果，如果失败就不会执行到这个步骤 --- #
                # if testcase['condition'].lower() == 'setup':
