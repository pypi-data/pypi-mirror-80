...
class CloudvectorDAST(object):
    ...
    def _prepare_spec_for_test_generation(self, input_spec, specific_apis=None):
        ...
        for api, info in parsed_new_spec["paths"].items():
            ...
            if api not in params_info_from_spec:
                params_info_from_spec[api] = {}
            for method, minfo in info.items():
                if method not in params_info_from_spec[api]:
                    params_info_from_spec[api][method]={"required":[], "optional":[]}
                x = {}
                ...
                for _ in minfo.get("parameters", []):
                    if _.get("required", True):
                        if type(_.get("required")) is bool:
                            params_info_from_spec[api][method]["required"].append(_.get("name"))
                        else:
                            params_info_from_spec[api][method]["required"]+=_.get("required",[])
                    else:
                        params_info_from_spec[api][method]["optional"].append(_.get("name"))
                    if _.get("in") == "header":
                        x["header"][_.get("name")] = ""
                    elif _.get("in") == "query":
                        if "?" not in x["url"]:
                ...
                for each in v:
                    if type(each) is dict:
                        for a, b in keypaths(each):
                            list(update_nested_dict(a[-1], "$" + a[-1] + "$", each))
                    else:
                        list(update_nested_dict(k[-1], "$" + k[-1] + "$", self.params_details_from_spec))
            elif type(v) is dict:
                list(update_nested_dict(a[-1], "$" + a[-1] + "$", v))
    ...
    def generate_fuzz_report(output_report):
        ...
        def get_metadata_for_fuzzinput(in_file):
            for type, subtype in attack_info.items():
                for name, info in subtype.items():
                    #print(name, info.get("file_set"), in_file, "\n\n\n")
        ...
        for test in report["report"]["tests"]:
            ...
            try:
                ...
                response_code = 0
            try:
                fuzz_sub_type = metadata.split("::")[3].replace("\\","/").split("/")[-1]
            except:
                fuzz_sub_type = "na"
            try:
                fuzz_type = metadata.split("::")[3].replace(fuzz_sub_type,"").replace("\\","/").replace("attack","").replace("//","/").split("/")[1]
            except:
                fuzz_type = "na"
            try:
                test_response = metadata.split("::")[4]
            except:
                test_response = None
            try:
                curl_command_to_try = stdout.split("CURL command to retry:")[-1].split("---------------------------------")[0].replace("\n","")
            except:
                curl_command_to_try = ""
            ...
            fuzz_metadata = get_metadata_for_fuzzinput(fuzz_sub_type)
            ...
            if str(test["outcome"]).lower() == "failed":
                ...
                if fuzz_type not in fuzz_types.get(fuzz_severity,{}):
                    if str(fuzz_type):
                        fuzz_types[fuzz_severity][fuzz_type] = {
                            ...
                            "ease_of_explanation": fuzz_ease_of_exploitation,
                            "executions": [{
                                                "api": api_name,
        ...
        for severity, types in fuzz_types.items():
            for k, v in types.items():
                vulns_stats[severity] = len(v.get("executions",[]))
        ...
        with open(output_report, "wb+") as file_:
            file_.write(template.render(api_data=api_data, fuzz_data=fuzz_data_processed, total_api_calls=total_api_calls,
                                        ...
                                        test_time=datetime.datetime.fromtimestamp(time.time()).isoformat(), total_apis_scanned=total_apis,
...
def main():
    ...
    for _ in os.listdir("tests"):
        if ".py" in _:
            ...
            printProgressBar(i + 1, len(os.listdir("tests")), prefix='Progress:', suffix='Complete', length=50)
            i+=1
    print("\n\n\t\t..........done")
