...
class DerkEnv(gym.Env):
  ...
  * ``mode="server"``: You control the home team, and other environments can connect to you through http.
  * ``mode="connected"``: Connects to an environment running in "server" mode.
  ...
  Args:
    ...
    connected_host: Host to connect to in "connected" mode. Defaults to "ws://localhost:8789". (Environment variable: DERK_CONNECTED_HOST)
  ...
  Attributes:
    ...
    n_sides: Number of controllable sides, i.e. are you controlling home and away or just home (1 or 2)
    n_agents_per_arena: Number of controllable agents per arena (3 or 6)
    n_actions: Number of actions per creature (5)

  """
  def __init__(self,
      mode: str=False, server_port: int=None, connected_host: str=None,
      app_host: str=None,
    ...
    self.server_port = server_port if server_port is not None else os.environ.get('DERK_SERVER_PORT', 8789)
    self.connected_host = connected_host if connected_host is not None else os.environ.get('DERK_CONNECTED_HOST', 'ws://localhost:8789')
    ...
    if self.mode == 'server':
      ...
      start_server = websockets.serve(self._handle_websocket, "localhost", self.server_port)
      asyncio.get_event_loop().run_until_complete(start_server)
      self.remote_messages = asyncio.Queue()
      print('Serving on ws://localhost:' + str(self.server_port))

    self.n_agents_per_team = 3
    self.n_sides = (2 if self.mode == 'normal' else 1)
    self.n_agents_per_arena = self.n_agents_per_team * self.n_sides
  ...
  @property
  def n_agents_per_side(self):
    """Total number of agents per side

    I.e. ``env.n_arenas * env.n_agents_per_team``
    """
    return self.n_arenas * self.n_agents_per_team
  ...
  def step(self, action_n: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray, List[bool], List[Dict]]:
    ...
    * Numpy array of shape (:attr:`n_sides`, :attr:`n_arenas`, :attr:`n_agents_per_team`, :attr:`n_actions`)
    * Numpy array of shape (:attr:`n_sides`, :attr:`n_agents_per_side`, :attr:`n_actions`)
    * Numpy array of shape (:attr:`n_arenas`, :attr:`n_agents_per_arena`, :attr:`n_actions`)
    ...
    * List of actions (i.e. ``[[1, 0, 0, 2, 0], [0, 1, 0, 0, 3], ...]``), one inner array per agent. This is just cast to a numpy array of shape (:attr:`n_agents`, :attr:`n_actions`).
    ...
    be reshape like the above. For instance: ``observations.reshape((env.n_arenas, env.n_agents_per_arena, -1))``

    The away team will be omitted if you are running against another environment (``mode="server"`` or ``mode="connected"``).
  ...
  async def _handle_websocket(self, websocket, path):
    logger.info('[_handle_websocket] Got websocket connection')
    self.remote_connection.set_result(websocket)
    try:
      async for message in websocket:
        await self.remote_messages.put(message)
    except websockets.exceptions.ConnectionClosedError:
      logger.info('[_handle_websocket] Connection lost')
    await self.remote_messages.put('connection_ended')
    self.remote_connection = asyncio.Future()
    self.remote_messages = asyncio.Queue()
    logger.info('[_handle_websocket] Websocket disconnected')
  ...
  async def async_reset(self):
    ...
    config = {
      ...
      'connectedHost': self.connected_host if self.mode == 'connected' else None
    }
  ...
  async def async_step(self, action_n: np.ndarray = None):
    """Async version of :meth:`step`"""
    if action_n is not None:
      actions_arr = np.asarray(action_n, dtype='float32')
      base64_actions = base64.b64encode(actions_arr.reshape((-1))).decode('utf-8')
    else:
      base64_actions = None
    if self.dummy_mode:
      return (
    ...
    if self.mode == 'server':
      try:
        ...
        raise ConnectionLostError()
      await websocket.send(base64_actions)
      msg = await self.remote_messages.get()
      if msg == "connection_ended":
        raise ConnectionLostError()
      res = json.loads(msg)
