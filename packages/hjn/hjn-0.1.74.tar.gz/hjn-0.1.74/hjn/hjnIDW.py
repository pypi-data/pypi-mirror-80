...
import numpy as np
import pandas as pd
from hjn.mkNCHJN import envelope, dataClass, mkNCCommonUni
import os
...
class IDW():
    def __init__(self, ltc, y, x, z, ele, resolution=0.01, candidate=15):
        self.evn = ltc
    ...
    def mkNC(self, outpathNC):
        dataClassList = [dataClass(self.resultMat, self.element, "f4", ["time", "lat", "lon"], "", missing_value=0)]
        os.makedirs(os.path.dirname(outpathNC), exist_ok=True)
        mkNCCommonUni(outpathNC, datetime.datetime.now(), [0], None, self.latArr, self.lonArr,
                      dataClass2D=dataClassList)

if __name__ == '__main__':
    JJJevn = envelope(44, 35, 112, 123)
    b = IDW(JJJevn, df["Lat"], df["Lon"], df[ele], ele)
    b.initDict()
    mat = b.getMat()
