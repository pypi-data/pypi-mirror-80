...
DEFAULT_LABELS_BY_NAME = {v[1]: np.array(k) for k, v in DEFAULT_COLOR_MAPPING.items()}
DEFAULT_LABEL_NAMES_BY_LABEL = {v: k for k, v in DEFAULT_LABELS_BY_NAME.items()}
...
class ColorMap:
    ...
    def __init__(self, mapping: Dict[Tuple[int, int, int], Union[int, Tuple[int, str]]]):
        ...
        for k, v in mapping.items():
            ...
            elif DEFAULT_LABEL_NAMES_BY_LABEL.get(v) is not None:
    ...
    def to_rgb_array(self, labels: np.ndarray) -> np.ndarray:
        return np.asarray(self.to_image(labels))
