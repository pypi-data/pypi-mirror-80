...
#define STDP_CONNECTION_COSEXP_H
...
public:
  ...
  STDPCosExpConnection();

  STDPCosExpConnection( const STDPCosExpConnection& );
...
//
// Implementation of class STDPCosExpConnection.
//
...
template < typename targetidentifierT > STDPCosExpConnection< targetidentifierT >::STDPCosExpConnection( const STDPCosExpConnection& rhs )
  : ConnectionBase( rhs )
  , vt_ ( rhs.vt_ )
  , weight_( rhs.weight_ )
...
template < typename targetidentifierT > void STDPCosExpConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const{
