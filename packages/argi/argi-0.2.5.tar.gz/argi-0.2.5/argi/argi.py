...
class argi:
  ...
  def count_anomalies(self,start, stop, freq):
    ...
    for node1, node2, data in self.G.edges(data=True):
      ...
      if len(timeseries) > 1:
        ...
        s = validate_series(timeseries[self.columnas['value']])
        ts_clustering.append(timeseries[self.columnas['value']].array)
  ...
  def create_clustering(self,start,stop, freq, num_cluster):
    ts_clustering = []
    ...
    # kShape clustering
    ks = KShape(n_clusters=num_cluster, verbose=True, random_state=seed)
