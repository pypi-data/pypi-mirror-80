...
import sys
from ReadData import readData,keepValidPositions
from PyQt5.QtGui import QStandardItemModel, QStandardItem, QPixmap, QPalette, QBrush, QImage, QIcon
from pcaClass import FigurePCA
from dialogPositions import valuesPositions
import pandas as pd
from dialogInterpolation import dInterpolation
from threadsOpenClass import LoadFile, LoadFolder
from threadsPlotClass import plotSurface,plotPositionEntropy,plotGumbel, codonDecay
from tsneClass import FigureTSNE
from tsneThreadProcess import tsneProcess,tsneProcessUpdate
from lvrvClass import FigureLVRV
from lvrvThread import lvrv_poly,lvrv_average
from codonDecayClass import FigureCD
from threadPCA import pcaProcess
...
path_pkl = 'data_experiments/' #también a cambiar
path_grafics='plot_images'  #esto es algo a cambiar en todas las funciones que plotean
loaded_dfs = []
...
def logger(new,experiment,log):
    ...
    if new:
        logFile = open(path_pkl+experiment,'w')
    else:
        logFile = open(path_pkl+experiment,'a')

    logAux = str(date_time) + ' ' + log
    logFile.write(logAux + '\n')
    logFile.close()
    logging.info(log)
...
def initLoggin():
    ...
    logging.basicConfig(filename='logs/vev-'+date_time+'.log',format='%(asctime)s %(message)s', level=logging.INFO)
...
#Load FILES AND FOLDERS
    ...
    def loadExperimentPK(self):
        ...
        options |= QFileDialog.DontUseNativeDialog
        options |= QFileDialog.DontUseCustomDirectoryIcons
        (fileName, aux) = QFileDialog.getOpenFileName(self, caption='Select a .parquet File', filter='*.parquet',options=options)
        if fileName != "":
...
# Complete or Errors with open
    ...
    def addUser(self):
        if not self.db.users.find({'_id': {"$in": [self.namePC]}}).count() > 0:
