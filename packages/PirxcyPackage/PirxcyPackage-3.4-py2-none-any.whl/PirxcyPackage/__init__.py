...
async def emotes(ctx: fortnitepy.ext.commands.Context) -> None:
    ...
    for new_skin in [new_eid for new_eid in response if new_eid.split('/')[-1].lower().startswith('eid_')]:
        await client.party.me.set_emotet(
...
commands.dm_only()
@client.command()
async def pbv2(ctx: fortnitepy.ext.commands.Context) -> None:
    await client.party.me.join()
