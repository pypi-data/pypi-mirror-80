...
print(crayons.cyan(f'[PartyBot] [{time()}] Made By Pirxcy Dont Fucking Steal My Shit.'
                   'Ty for 300 subs.'))
print(crayons.cyan(f'[PartyBot] [{time()}] Enjoy UwU'))
...
async def event_party_member_join(member: fortnitepy.PartyMember) -> None:
    delay.sleep(2)
    await client.party.send(f'Hey! {member.display_name} This  is  a bot made by Pirxcy You Can Support Him  By joining his discord https://discord.gg/wxEBMTZ ')
...
async def blockx(ctx: fortnitepy.ext.commands.Context, *, epic_username: str) -> None:
    ...
    if member is None:
        ...
        try:
            ...
            print(f"[PartyBot] [{time()}] Blocked and Kicked: {member.display_name}")
        except fortnitepy.errors.Forbidden:
...
async def starwars(ctx: fortnitepy.ext.commands.Context) -> None:
    ...
    print(f'[PartyBot] [{time()}] Skin set to Star Wars')
...
async def leaks(ctx: fortnitepy.ext.commands.Context) -> None:
    ...
    print(f'[PartyBot] [{time()}] New Leaked skins')
