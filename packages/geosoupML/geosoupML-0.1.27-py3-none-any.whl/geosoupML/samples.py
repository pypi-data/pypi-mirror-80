...
class Samples:
    ...
    def __init__(self,
        ...
        # label name or csv file are provided
            ...
            if not any(label_colname in s for s in header):
            ...
            self.x = np.array(list(list(samp_dict[feat_name] for feat_name in feat_names)
                                   for samp_dict in temp))
            self.y = np.array(list(samp_dict[label_colname] for samp_dict in temp))
    ...
    def subsample(self,
        ...
        :param item: list, tuple, numpy array or integer of index locations
        ...
        if isinstance(item, list) or \
            ...
            outsamp.x = self.x[np.array(item), :]
            outsamp.y = self.y[np.array(item)]
