...
from sktmls.data_status import StatusClient
...
class MLModelClient(MLSClient):
    ...
    def is_batch_prediction_ready(self, manual_model: ManualModel) -> bool:
        ...
        running_jobs = [j["jobName"] for j in status_cli.get_batch_status("running").jobs]

        if f"{table}-loading" in running_jobs or f"{table}-deletion" in running_jobs:
            return False
