...
class HealpixMap(HealpixBase):
    ...
    * A structured array or ``recarray``. This will initialize a MOC map. The 
    ...
    Args:
        data (array): ``HealpixMap``, for an emty map, or value for an 
            initialize map. A MOC map is recarray whose first field is called 
            'UNIQ'. The second field, if it exists, will initialize the contents,
            otherwise the map will be zero-initialized.
    ...
    def __init__(self,
        ...
        if data is not None:
            ...
            if (data.dtype.names is not None and
    ...
    @classmethod
    def adaptive_moc_map(cls, max_nside, pix_fun, combine_fun, density = False):
        """
        Discretize a function into an adaptive multi-order map.

        This method evaluates ``pix_fun`` for all pixels in a map of nside
        ``max_nside`` and then, guided by ``combine_fun``, recursively groups 
        together sets of 4 pixels into a single parent pixel of a lower order.

        Args:
            max_nside (int): The function will be initially sampled at this nside
        ...
        for pix in range(0, hp.nside2npix(max_nside), 4):
    ...
    def _operation(self, map1, operation):
        ...
        if np.isscalar(map1):
            ...
            if (map0.is_moc or map1.is_moc) and not map0.conformable(map1):
                ...
                # Create new map
                new_data = np.rec.fromarrays([new_uniq[:pix_new], new_data[:pix_new]],
                                              names = ["UNIQ", "CONTENTS"])

                return HealpixMap(new_data, density = new_density)
