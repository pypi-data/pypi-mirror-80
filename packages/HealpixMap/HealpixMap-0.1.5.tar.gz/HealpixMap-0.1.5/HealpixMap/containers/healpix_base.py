...
class HealpixBase:
    ...
    def plot_grid(self,
        ...
        for pix in range(self.npix):
            ...
            if self.is_moc:
                pix_nside = hmap.uniq2nside(self.pix2uniq(pix))
                pix_step *= int(self.nside / pix_nside)
            
            # Get boundaries
            vec = self.boundaries(pix, step = pix_step)
        ...
        if proj == 'moll':
            ...
            # 2:1 ellipse
            theta = np.linspace(0, 2*np.pi, step * 8 * self.nside)
            x = 2 * np.cos(theta)
            y = np.sin(theta)
            ...
            # 2:1 rectangle
            x = np.linspace(-180, 180, step * 4 * self.nside)
            y = np.ones(len(x))
            ax.plot(x, -90*y, color = color, **kwargs)
            ax.plot(x,  90*y, color = color, **kwargs)

            y = np.linspace(-90, 90, step * 3 * self.nside)
            x = np.ones(len(y))
            ...
            theta = np.linspace(0, 2*np.pi, step * 6 * self.nside)
            x = np.cos(theta)
            y = np.sin(theta)
