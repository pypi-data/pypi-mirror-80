...
from jiangsu_jqyeviuijksd.get_proxy import add_to_white_list, get_proxy
...
@main.command()
@click.option("-c", help="配置文件位置, 默认为 ./proxy.json", default='./proxy.json')
def add_ip_to_white_list(c: str):
    ret = add_to_white_list(Path(c))
    if ret == {}:
...
@main.command()
@click.option("-c", help="配置文件位置, 默认为 ./proxy.json", default='./proxy.json')
def get_a_proxy(c: str):
    ret = get_proxy(Path(c))
    if ret == {}:
        logger.error('未找到配置文件')
    else:
        logger.info(ret)
