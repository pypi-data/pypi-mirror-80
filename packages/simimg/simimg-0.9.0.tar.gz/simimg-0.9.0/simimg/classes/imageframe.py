...
class ImageFrame(ttk.Frame):
    ...
    def createThumbContent(self):
        ...
        if self.checksum:
            ...
            if self._Ctrl.Cfg.get('filenameonthumbnail'):
                annotation_text += self._Ctrl.FODict[self.checksum][0].fileName
            if self._Ctrl.Cfg.get('imagesizeonthumbnail'):
                print('herb')
                size = self._Ctrl.FODict[self.checksum][0].size()
                annotation_text += f'[{size[0]}x{size[1]}]'
    ...
    def showOptionalElements(self):
        if (
                self._Ctrl.Cfg.get('filenameonthumbnail') or
                self._Ctrl.Cfg.get('imagesizeonthumbnail')
        ):
