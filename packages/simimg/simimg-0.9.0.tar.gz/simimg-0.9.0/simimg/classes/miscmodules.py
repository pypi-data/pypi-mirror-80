...
class ThumbOptions(ttk.Frame):
    ...
    def __init__(self, parent, Controller=None):
        ...
        self.showimagesize = tk.BooleanVar()
        self.showimagesize.set(self._Ctrl.Cfg.get('imagesizeonthumbnail'))
        sis = ttk.Checkbutton(
            self,
            text='Image size',
            variable=self.showimagesize,
            command=self._showOptionChanged
        )
        sis.pack(fill='x')
        msg = '''Show the image size on each thumbnail.'''
        TT.Tooltip(sis, text=msg)
    ...
    def _showOptionChanged(self, *args):
        ...
        self._Ctrl.Cfg.set('imagesizeonthumbnail', self.showimagesize.get())
