...
class Transform:
    ...
    def transform_model(self,model='single'):
...
class MydataSet():
    def __init__(self,images_path,transform,encode_list,train_class_len,classification_scheme):
...
class Train_Captcha:
    def __init__(self,
        ...
        self.transform = Transform(image_size=image_size).transform_model(model=classification_scheme)
        if not test_model:
            self.encode_list = dict(zip(train_class, range(0, len(train_class))))
            self.images_path = images_train_path
            train_data = MydataSet(images_train_path, self.transform,self.encode_list,self.train_class_len,classification_scheme)
            ...
            if show_train_images_demo:
                ...
                exit()
            if images_test_path:
                test_data = MydataSet(images_test_path, self.transform,self.encode_list,self.train_class_len,classification_scheme)
