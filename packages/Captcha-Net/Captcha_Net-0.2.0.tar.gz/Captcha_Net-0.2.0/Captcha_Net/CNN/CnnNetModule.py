...
class Train_Captcha:
    def __init__(self,
        ...
        '''
        # train_class -> number
        label_len = [len(i.replace('.jpg', '').replace('.png', '').split('_')[0]) for i in
                     os.listdir(images_train_path)]
        ...
        self.transform = Transform(image_size=image_size).transform_model(model=classification_scheme, mean=mean,
                                                                          std=std)
        if not test_model:
