...
class VectorsInternalSplitedDataReader:
    def read(self, directory_path):
        ...
        for file in os.listdir(directory_path):
            if re.match(r'^.*_block_[0-9]+.*', file):
                print("file :", file)
...
class LargeFileWriter:
    def _create_block_file(self, data,prefix,index, directory_path, ext):
        
        if "." in ext:
            ext = ext.replace(".", "")

        file = os.path.join(directory_path, "{0}_block_{1}.{2}".format(prefix, index, ext))
        joblib.dump(data, file)
        print(file, " finished!")
    
    def write(self, data_list,prefix,directory_path, ext):
        threadlist = []
        for index, data in enumerate(data_list):
            t = Thread(target=self._create_block_file, args=(data, prefix,index, directory_path, ext))
            threadlist.append(t)
    ...
    #def read(self, directory_path, prefix, ext):
    #    files = [f for f in os.listdir(directory_path) if re.match(r'^.*_block_[0-9]+.*', f)]
