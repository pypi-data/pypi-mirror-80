...
def get_sample_bridges():
    #first check if the file is beside script
    samples = os.path.dirname(os.path.abspath(__file__))+"/SampleBridges"
    if os.path.isfile(samples):
        return samples
    #then check for package installed file
    else:
        return [skel_path for skel_path in GLib.get_system_data_dirs() if os.path.exists(skel_path) and "tractor" in os.listdir(skel_path)][-1]+"/tractor/SampleBridges"
