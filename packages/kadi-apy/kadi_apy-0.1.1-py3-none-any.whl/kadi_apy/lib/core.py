...
class KadiAPI:
    ...
    def _make_request(self, endpoint, method=None, headers=None, **kwargs):
        if not endpoint.startswith(self.host):
            endpoint = self.host + endpoint

        return getattr(requests, method)(
            endpoint, headers={"Authorization": f"Bearer {self.token}"}, **kwargs
        )
...
class Resource(KadiAPI):
    ...
    def __init__(self, id=None, identifier=None, create=False, **kwargs):
        super().__init__()
        self.id = id
        self.created = False
        ...
        if self.id is not None:
            ...
            if response.status_code != 200:
                raise KadiAPYRequestError(
                    f"Could not access the {self.__class__.__name__} with ID {self.id}."
            ...
            if identifier is not None:
                ...
                # The identifier already exists but we cannot access it.
                    ...
                    raise KadiAPYRequestError(
                        f"The identifier '{identifier}' is already in use."
                    )
        ...
        # than we raise this exception.
        if self.id is None:
            raise KadiAPYInputError(
                f"{self.__class__.__name__} with identifier '{identifier}' is not "
    ...
    def invalidate_meta(self):
        """Excplicitely invalidate the cached metadata of the resource."""
        self._meta = None
