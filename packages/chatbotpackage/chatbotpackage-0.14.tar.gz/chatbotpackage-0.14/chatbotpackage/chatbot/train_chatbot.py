    ...
    try:
        ...
        if parser_setclient.has_option('chat_trainer', 'client'):
            if clientname.strip() == "":
                ...
                with open(os.path.dirname(__file__) + '/training.ini', 'w') as configfile:
...
def exectraining():
    try:
        ...
        global parser
        parser = SafeConfigParser()
        parser.read(os.path.dirname(__file__) + '/training.ini')
        ...
        # Reading the values for the client details to train
        intent_file_client = parser.get('chat_trainer', 'client')
        intent_file_name = os.path.dirname(__file__) + '/' + parser.get(intent_file_client, 'intents')
        # intents_file = open('intents.json').read()
        ...
        # Details of class & word files from the config file
        word_name = os.path.dirname(__file__) + '/' + parser.get(intent_file_client, 'words')
        class_name = os.path.dirname(__file__) + '/' + parser.get(intent_file_client, 'class')
        pickle.dump(words, open(word_name, 'wb'))
        pickle.dump(classes, open(class_name, 'wb'))
        ...
        # Details of model file from the config file
        model_name = os.path.dirname(__file__) + '/' + parser.get(intent_file_client, 'model')
        model.save(model_name, hist)
