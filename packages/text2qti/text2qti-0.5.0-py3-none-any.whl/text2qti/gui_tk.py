...
def main():
    ...
    latex_url_label = tk.Label(
        window,
        text='LaTeX rendering URL:\n(only needed for LaTeX math)',
        justify='right',
    )
    ...
    run_code_blocks_label = tk.Label(
        window,
        text='Allow executable code:\n(only use for trusted quizzes)',
        justify='right',
    )
    ...
    def run():
        ...
        try:
            ...
            error_message = f'Quiz creation failed. Technical details:\n\n{e}'
        finally:
            os.chdir(cwd)
        if error_message:
            run_message_text.delete(1.0, tk.END)
