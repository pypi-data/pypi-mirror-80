...
class creport():
    ...
    def crawl(self, url, due_date):
        ...
        while now_date >= due_date:
            ...
            if len(datelist) == 0 or page == 100:
                ...
                dates = re.findall("([0-9]{4}-[0-9]{2}-[0-9]{2})\xa0", str(datelist[0]))
                times.extend(dates)
                now_date = min(times)
    ...
    def save(self, data, stock, dir):
        data.to_csv(dir+'report/'+stock+'.csv',index=False, encoding='utf-8')

if __name__ == '__main__':
    us = UrlSummary()
    stock = '600004'
    url = us.industry_url(stock)
    due_date = '2020-01-01'
    c = creport()
