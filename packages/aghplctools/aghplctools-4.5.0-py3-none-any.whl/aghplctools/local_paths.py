    ...
    def construct_from_ini(cls, ini_path: Union[str, pathlib.Path] = None) -> List['ChemStationConfig']:
        ...
        while True:
            try:
                pcsnum = f'PCS,{inst}'
                out.append(cls(
                    core_path=configuration.get(pcsnum, '_EXEPATH$'),
                    method_path=configuration.get(pcsnum, '_METHPATH$'),
                    sequence_path=configuration.get(pcsnum, '_SEQPATH$'),
                    data_path=configuration.get(pcsnum, '_DATAPATH$'),
                    version=version,
                    number=inst,
                ))
                inst += 1
