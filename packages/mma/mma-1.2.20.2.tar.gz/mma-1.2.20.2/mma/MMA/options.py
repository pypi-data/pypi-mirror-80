...
def opts(l=None):
    ...
    for o, a in opts:
        if o == '-b':
            ...
            import mma.MMA.debug   # circular dep. problem
            mma.MMA.debug.cmdLineDebug(o[-1])
            ...
            if a == 'xl':
                ...
                # important! Needs a space before the trailing LF for mma.MMA.el
                print("Base track names: %s \n" % 
                      ' '.join([a for a in sorted(mma.MMA.alloc.trkClasses)]))
                print("Commands: %s BEGIN END DEFAULT\n" % 
                      ' '.join([a for a in sorted(mma.MMA.parse.simpleFuncs)]))
                print("TrackCommands: %s \n" %
                      ' '.join([a for a in sorted(mma.MMA.parse.trackFuncs)]))
            ...
            import mma.MMA.sync
            mma.MMA.sync.synchronize(['START'])

        elif o == '-1':
            import mma.MMA.sync
            mma.MMA.sync.synchronize(['END'])
            ...
            if a == 'I':
                mma.MMA.regplug.secOverRide = True
            ...
            mma.MMA.exits.files.append(tfile)

            args = [tfile]

        elif o=='-x':  # any one of some xtra, seldom used, options
            import mma.MMA.xtra
            mma.MMA.xtra.xoption(a, args)
