...
class _AStarMetrics():
    ...
    def __repr__(self):
        return f"{self.graph_node.pos} g: {self.g} h: {self.h} f: {self.f}"
...
class Graph(object):
    ...
    def astar(self, start: Node, end: Node, g_func: Callable[[Node, Node], float] = None, h_func: Callable[[Node, Node], float] = None, ignored_disablers:List[str]=None) -> AStarResults:
        ...
        start = _AStarMetrics(None, start)
        end = _AStarMetrics(None, end)
        ...
        while len(open_set) > 0:
            ...
            steps[cc] = (open_set, closed_set, current_item)
            ...
            if current_item == end:
                ...
                return AStarResults(path[::-1], steps)  # Return reversed path
            ...
            for new_node in self._graph_dict[current_item.graph_node]:  # Adjacent nodes
                ...
                calc_h = h_func(new_node,
                                end.graph_node) if h_func else new_node.pos.distance_from(
                    end.graph_node.pos)
