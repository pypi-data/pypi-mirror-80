...
from openapi_parser.model import Filter, HavingDescription, HavingPath
...
@dataclass_json(letter_case=LetterCase.CAMEL)
@dataclass
class EnumFilter(HavingDescription, HavingPath, FilterImpl[T], Generic[T]):
    possible_values: List[T] = field(metadata=config(field_name='enum'))
    description: Optional[str] = None
    title: Optional[str] = None
    example: Optional[str] = None
    
    name: str = field(init=False, default='enum')
    path: str = field(init=False)
    pretty_path: str = field(init=False)
...
class DateFilter(FilterImpl[str], Generic[D]):
    ...
    @property
    def decoder(self) -> Callable[[str], D]:
        return self.cls.fromisoformat
    @property
