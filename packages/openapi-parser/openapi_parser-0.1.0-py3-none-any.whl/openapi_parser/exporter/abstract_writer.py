...
from openapi_parser.model import Model
from openapi_parser.util.naming_conventions import field_name, const_name
...
class Writer(ABC):
    ...
    @classmethod
    def class_name_pretty(cls, ref: Union[Type, GenericProxy, Model]) -> str:
...
def writer(yielder_func: Callable[[Writer], Iterator[Tuple[int, str]]]) -> Callable[[Writer], Optional[Iterator[str]]]:
    @wraps(yielder_func)
    def wrapper(self: Writer, *args, stream: TextIO = None, **kwargs):
        if (stream is not None):
            for line in wrapper(self, *args, **kwargs, stream=None):
                stream.write(line)
