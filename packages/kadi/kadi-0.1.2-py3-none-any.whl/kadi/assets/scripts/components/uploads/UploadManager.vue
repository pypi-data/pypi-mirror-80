  ...
  <div>
    ...
    <div class="card bg-light py-2 px-4 mt-4 mb-3" v-if="uploads.length > 0">
      <div class="form-row align-items-center">
        <div class="col-xl-8">
          {{ `${completedUploadsCount}/${uploads.length} ${i18n.t('misc.uploadCompleted', {count: uploads.length})}` }}
    ...
    <div class="card"
      ...
      <div class="card-body py-2">
        <div class="form-row align-items-center" :class="{'mb-2': upload.state !== 'completed'}">
          ...
          <div class="col-xl-2 d-xl-flex justify-content-end">
            <span class="badge badge-primary">{{ upload.state | capitalize }}</span>
          </div>
        </div>
  ...
  methods: {
    ...
    inputChange(e) {
      ...
      if (file.size !== this.resumedUpload.size) {
        const message = 'The file you have selected has a different size than the previous upload.\n'
      ...
      if (this.uploadQueue.length > 0 && !this.uploadInProgress) {
        ...
        if (!upload.chunkCount) {
          const _errorInitiatingUpload = (error) => {
            ...
            if (error.request.status === 413) {
              ...
              kadi.alert('Error initiating upload.', {xhr: error.request});
            }
          ...
          try {
            ...
            if (error.request.status === 409) {
              const message = `A file with the name '${upload.name}' already exists in the current record.\n`
        ...
        while (true) {
          ...
          while (true) {
            ...
            try {
              ...
              timeout += 5000;
              kadi.alert(
                `Error uploading chunk. Retrying in ${timeout / 1000} seconds.`,
                {xhr: error.request, timeout},
              );
        ...
        try {
          ...
          if (error.request.status === 413) {
            ...
            kadi.alert('Error finishing upload.', {xhr: error.request});
          }
    ...
    finalizeUpload(upload) {
      ...
      const _updateStatus = () => {
        ...
        axios.get(upload.getStatusEndpoint, {params: {_internal: true}})
          ...
          .catch((error) => kadi.alert('Error updating upload status.', {xhr: error.request}));
      };
    ...
    cancelUploads(force, upload = null) {
      ...
      if (upload === null) {
        ...
        uploads.push(upload);
        message = `Are you sure you want to cancel '${upload.name}'?`;
      }
          ...
          axios.delete(_upload.deleteUploadEndpoint)
            ...
            .catch((error) => {
              if (error.request.status !== 404) {
                kadi.alert('Error removing upload.', {xhr: error.request});
              } else {
