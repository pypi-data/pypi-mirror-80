...
<template>
  <div class="card bg-light" ref="container">
...
import DynamicComponent from 'scripts/lib/workflows/components/DynamicComponent';
...
export default {
  ...
  methods: {
    ...
    loadWorkflow(endpoint) {
      ...
      axios.get(endpoint)
        .then((response) => {
          // Catches errors in both our custom and Rete's parsing.
          try {
  ...
  mounted() {
    ...
    const flagComponent = new DynamicComponent(flagSocket);
