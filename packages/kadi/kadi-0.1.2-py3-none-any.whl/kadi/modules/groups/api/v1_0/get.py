...
@qparam(
    ...
    "",
    parse=strip,
    description="A query to filter the records by their title and identifier.",
)
@qparam(
    "action",
    [],
...
def get_group_records(id, page, per_page, qparams):
    """Get the records the group specified by the given *id* is linked with."""
    group = Group.query.get_active_or_404(id)

    paginated_records = (
        get_linked_resources(Record, group.records, actions=qparams["action"])
        .filter(
            db.or_(
    ...
    data = {
        "items": RecordSchema(many=True, linked_group=group).dump(
...
@qparam(
    ...
    "",
    parse=strip,
    description="A query to filter the collections by their title and identifier.",
)
@qparam(
    "action",
    [],
...
def get_group_collections(id, page, per_page, qparams):
    """Get the collections the group specified by the given *id* is linked with."""
    group = Group.query.get_active_or_404(id)

    paginated_collections = (
        get_linked_resources(Collection, group.collections, actions=qparams["action"])
        .filter(
            db.or_(
    ...
    data = {
        "items": CollectionSchema(many=True, linked_group=group).dump(
