...
def manage_links(id):
    """Page to link a group to other resources."""
    group = Group.query.get_active_or_404(id)

    form = LinkResourcesForm()
    if form.validate_on_submit():
        add_links(Record, group.records, form.records.data)
        add_links(Collection, group.collections, form.collections.data)
        db.session.commit()
        flash("Changes saved successfully.", "success")

    return render_template(
        "groups/manage_links.html", title=_("Links"), form=form, group=group
    )
