...
class BaseGroupForm(KadiForm):
    ...
    visibility = SelectField(
        ...
        description=(
            _l(
                "Public visibility makes the content of this group visible to every"
                " logged in user."
            )
        ),
    )
...
class LinkResourcesForm(KadiForm):
    """A form for use in linking groups with records or collections."""
    ...
    def validate(self, extra_validators=None):
        ...
        if success and (self.records.data or self.collections.data):
            return True
