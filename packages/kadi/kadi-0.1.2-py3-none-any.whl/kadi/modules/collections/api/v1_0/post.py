...
@scopes_required("collection.link")
@reqschema(
    GroupSchema(only=["id"]), description="The group to link the collection with."
)
@status(201, "Collection successfully linked with group.")
@status(409, "The link already exists.")
def add_collection_group(id, schema):
    """Link the collection specified by the given *id* to a group."""
    collection = Collection.query.get_active_or_404(id)
    group = Group.query.get_active_or_404(schema.load_or_400()["id"])

    return add_link(collection.groups, group)
