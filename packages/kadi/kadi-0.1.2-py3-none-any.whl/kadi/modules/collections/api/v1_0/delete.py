...
@route("/collections/<int:collection_id>/groups/<int:group_id>")
@permission_required("link", "collection", "collection_id")
@scopes_required("collection.link")
@status(204, "Collection successfully removed from group.")
def remove_collection_group(collection_id, group_id):
    """Remove a collection link from a group.

    Will remove the collection specified by the given *record_id* from the linked group
    specified by the given *group_id*.
    """
    collection = Collection.query.get_active_or_404(collection_id)
    group = Group.query.get_active_or_404(group_id)

    return remove_link(collection.groups, group)
