...
@route("/collections/<int:id>/groups")
@permission_required("read", "collection", "id")
...
@status(200, "Return a paginated list of groups, sorted by title in ascending order.")
def get_collection_groups(id, page, per_page, qparams):
    """Get the groups the collection specified by the given *id* is linked with."""
    collection = Collection.query.get_active_or_404(id)

    paginated_groups = (
        get_linked_resources(Group, collection.groups, actions=qparams["action"])
        .filter(
            db.or_(
    ...
    data = {
        "items": GroupSchema(many=True, linked_collection=collection).dump(
            paginated_groups.items
        ),
        **create_pagination_data(
            paginated_groups.total,
            page,
            per_page,
            "api.get_collection_groups",
            id=collection.id,
            **qparams,
        ),
    }
...
def get_collection_user_roles(id, page, per_page, qparams):
    ...
    when = []
    for index, role in enumerate(get_object_roles("collection")):
        when.append((role["name"], index))

    paginated_user_roles = (
        get_user_roles("collection", object_id=id)
        .filter(User.id.notin_(qparams["exclude"]))
        .order_by(
            (User.id == collection.creator.id).desc(),
    ...
    data = {
        "items": UserRoleSchema(obj=collection).dump_from_iterable(
...
def get_collection_group_roles(id, page, per_page):
    ...
    paginated_group_roles = (
        get_group_roles("collection", object_id=id)
    ...
    data = {
        "items": GroupRoleSchema(obj=collection).dump_from_iterable(
        ...
        **create_pagination_data(
            ...
            page,
            per_page,
            "api.get_collection_group_roles",
            id=collection.id,
        ),
    }
