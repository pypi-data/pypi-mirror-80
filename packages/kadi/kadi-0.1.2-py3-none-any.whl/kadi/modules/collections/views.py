...
def new_collection(qparams):
    ...
    if request.method == "POST":
        ...
        flash("Error creating collection.", "danger")
...
def edit_collection(id):
    ...
    if request.method == "POST":
        ...
        flash("Error editing collection.", "danger")
...
def manage_links(id):
    """Page to link a collection to other resources."""
    collection = Collection.query.get_active_or_404(id)

    form = LinkResourcesForm()
    if form.validate_on_submit():
        add_links(Record, collection.records, form.records.data)
        add_links(Group, collection.groups, form.groups.data)
        db.session.commit()
        flash("Changes saved successfully.", "success")
        ...
        records = collection.records.filter(
            Record.id.in_(
                get_permitted_objects(
                    current_user, "permissions", "record"
