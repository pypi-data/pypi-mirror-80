...
from .forms import LinkResourcesForm
...
def edit_record(id):
    ...
    if request.method == "POST":
        ...
        flash("Error editing record.", "danger")
...
def manage_links(id, qparams):
    """Page to link a record to other resources."""
    record = Record.query.get_active_or_404(id)

    record_form = LinkRecordForm(_suffix="record")
    resources_form = LinkResourcesForm(_suffix="resources")
    ...
    if resources_form.validate_on_submit():
        add_links(Group, record.groups, resources_form.groups.data)
        add_links(Collection, record.collections, resources_form.collections.data)
        db.session.commit()
        flash("Changes saved successfully.", "success")
...
def edit_file(record_id, file_id):
    ...
    if form.validate_on_submit():
        ...
        flash("Error editing file.", "danger")
