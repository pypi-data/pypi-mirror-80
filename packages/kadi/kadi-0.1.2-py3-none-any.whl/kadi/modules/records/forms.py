...
class BaseRecordForm(KadiForm):
    ...
    visibility = SelectField(
        ...
        description=(
            _l(
                "Public visibility makes the content of this record visible to every"
                " logged in user."
            )
        ),
    )
...
class NewRecordForm(BaseRecordForm):
    ...
    copy_permission = DynamicSelectField(
        _l("Permissions"),
        coerce=int,
        description=_l("Copy the permissions of another record."),
    )
...
class LinkResourcesForm(KadiForm):
    """A form for use in linking records with collections or groups."""

    collections = DynamicMultiSelectField(_l("Collections"), coerce=int)

    groups = DynamicMultiSelectField(_l("Groups"), coerce=int)

    submit = SubmitField(_l("Link resources"))

    def validate(self, extra_validators=None):
        success = super().validate(extra_validators=extra_validators)

        if success and (self.collections.data or self.groups.data):
            return True
