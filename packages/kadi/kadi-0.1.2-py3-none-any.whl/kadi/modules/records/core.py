...
def _make_extra_key_query(extra_type, extra_key):
    return Q(
        "bool",
        should=[
            Q("match", **{f"extras_{extra_type}.key": extra_key}),
            Q("term", **{f"extras_{extra_type}.key.keyword": extra_key}),
        ],
    )
        ...
        if str_value:
            str_query.append(
                Q(
                    "bool",
                    should=[
                        Q("match", extras_str__value=str_value),
                        Q("term", extras_str__value__keyword=str_value),
                    ],
                )
            )
