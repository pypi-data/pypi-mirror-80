...
@permission_required("link", "record", "id")
@scopes_required("record.link")
@reqschema(GroupSchema(only=["id"]), description="The group to link the record with.")
@status(201, "Record successfully linked with group.")
@status(409, "The link already exists.")
def add_record_group(id, schema):
    """Link the record specified by the given *id* to a group."""
    record = Record.query.get_active_or_404(id)
    group = Group.query.get_active_or_404(schema.load_or_400()["id"])

    return add_link(record.groups, group)
...
def new_upload(id, schema):
    ...
    Once the new upload is initiated, the actual file chunks can be uploaded using the
    endpoint specified in the ``_actions.upload_chunk`` property of the upload.
    """
    record = Record.query.get_active_or_404(id)
    data = schema.load_or_400()
    ...
    response = check_upload_user_quota(current_user, data["size"])
    if response is not None:
        return response
...
def finish_upload(record_id, upload_id):
    ...
    response = check_upload_user_quota(current_user, additional_size)
    if response is not None:
        upload.state = "active"
        db.session.commit()
        return response

    task = start_merge_chunks_task(upload, current_user)
