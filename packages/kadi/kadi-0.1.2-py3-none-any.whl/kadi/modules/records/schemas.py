...
class RecordSchema(KadiSchema):
    ...
    :param linked_group: (optional) A group that is linked to each record that should be
        serialized. Will be used to build endpoints for corresponding actions.
    :param check_identifier: (optional) Flag indicating wether the identifier should be
    ...
    def _generate_links(self, obj):
        links = {
            ...
            "groups": url_for("api.get_record_groups", id=obj.id),
    ...
    def _generate_actions(self, obj):
        actions = {
            ...
            "link_group": url_for("api.add_record_group", id=obj.id),
        ...
        if self.linked_group:
            actions["remove_link"] = url_for(
                "api.remove_group_record",
                group_id=self.linked_group.id,
                record_id=obj.id,
            )
