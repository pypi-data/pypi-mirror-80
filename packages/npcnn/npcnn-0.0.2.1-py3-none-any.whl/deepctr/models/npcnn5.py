...
from ..layers.interaction import InnerProductLayer, NPCNNLayer
...
def NPCNN5(linear_feature_columns, dnn_feature_columns, conv_kernel_width=(7, 7, 7, 7), conv_filters=(14, 16, 18, 20),
          new_maps=(3, 3, 3, 3), recomb_apply=(1, 1, 1, 1)
    ...
    :param recomb_apply: recombination process application for new_maps 
    ...
    if len(conv_filters) > 0:
        new_features = NPCNNLayer(
            conv_filters, conv_kernel_width, new_maps, recomb_apply, pooling_width)(fg_input)
        combined_input = new_features
    else:
        combined_input = origin_input
    inner_product_raw = tf.keras.layers.Flatten()(InnerProductLayer()(
        tf.keras.layers.Lambda(unstack, mask=[None] * int(origin_input.shape[1]))(origin_input)))
    inner_product_new = tf.keras.layers.Flatten()(InnerProductLayer()(
        tf.keras.layers.Lambda(unstack, mask=[None] * int(combined_input.shape[1]))(combined_input)))
    dnn_input = tf.keras.layers.Concatenate()([inner_product_raw, inner_product_new])
    ...
    final_logit = tf.keras.layers.Dense(1, use_bias=False, kernel_initializer='he_normal')(final_logit)
