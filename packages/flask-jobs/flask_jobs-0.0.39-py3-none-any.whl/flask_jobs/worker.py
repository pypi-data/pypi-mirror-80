...
class Worker:
    ...
    def ServerIsRunning(self):
        ...
        with open(SERVER_STATUS_PATH, mode='rt') as file:
            ...
            self.print('self._serverStartTime=', self._serverStartTime)

            if timestamp and self._serverStartTime:
                if self._serverStartTime != timestamp:
                    self.print('the server has been rebooted, kill this worker')
                    return False

            elif timestamp is None:
                self.print('The server has stopped, kill this worker')
                return False
