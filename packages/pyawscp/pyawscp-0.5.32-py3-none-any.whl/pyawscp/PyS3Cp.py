...
class PyS3CP:
    ...
    def preSignURL(self,bucketName,objectName):
        s3api = self.botoSession().client('s3')
        # one week
        expiration = 604800
        # To Access the object
        url = s3api.generate_presigned_url('get_object',Params={'Bucket': bucketName,'Key': objectName},ExpiresIn=expiration)
    ...
    def listUploads(self):
        ...
        pyTransferS3 = PyTransferS3(True,self.config,bucket,self.config.awsProfile,self.config.awsRegion,prefix)
    ...
    def showTargetsS3(self):
        pyTransferS3 = PyTransferS3(True,self.config,"",self.config.awsProfile,self.config.awsRegion,None)
