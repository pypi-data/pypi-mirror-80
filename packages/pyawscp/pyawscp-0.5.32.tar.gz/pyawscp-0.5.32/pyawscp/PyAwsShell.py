...
class PyAwsShell(cmd.Cmd):
    ...
    intro =  "\n" + \
             ...
             Style.GREEN + "  Use  "  + Style.BLUE + " <TAB>" + Style.GREEN + "           to autocomplete or view commands/arguments " + Style.RESET + "\n" + \
             Style.GREEN + "  Enter " + Style.BLUE + "navigator" + Style.GREEN + "       to try one of the features... " + Style.CYAN + Emoticons.seeYa() + Style.RESET + "\n"
             
    prompt = '(\033[33maws\033[0m-\033[36mshell\033[0m-\033[34mcockpit\033[0m)\033[33m' + Emoticons.prompt() + '\033[0m:\033[33m~\033[0m '
    ruler      = '-'
    ...
    def do_environment(self, arg):    
        ...
        output +=Style.BLUE + "                    " + Style.BLUE + "Command to set:" + Style.GREEN+ " set region {PROFILE}" + Style.RESET + "\n"
        output += "\n"
    ...
    def complete_listVpc(self,text,line,begidx,endidx):
        return ([i for i in ('subnets','ispublic','showtags','desc','tags','sort3') if i.startswith(text)])        
