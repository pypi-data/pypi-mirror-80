...
class WikiExtractor(object):
    ...
    def get_nearby_images(self, lat, lon, radiusmetres=10000, nametomatch=False, matchfilter=False):
        ...
        nametomatch: if set to a string then results are given a match rating against the string. If False then no matching is done.
    ...
    def get_page_match(self, keyword, searchlat, searchlon, bestmatch, maxdistance=30, minnamematch=0):
        """
        ...
        Note: Works best for unique, proper names of geographically located places (e.g. landmarks, buildings, parks)
        ...
        results = [place for place in data if ((place['distance'] < maxdistance) and (place['name match'] > minnamematch))]
        if(results):
