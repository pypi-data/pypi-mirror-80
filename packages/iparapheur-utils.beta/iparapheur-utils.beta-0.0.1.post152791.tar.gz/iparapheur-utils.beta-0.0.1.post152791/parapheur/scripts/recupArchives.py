...
def get_reduced_folder_path(dossier_type, dossier_subtype, dossier_title, dossier_id):
    ...
    os.makedirs(reduced_path, 0o755)
...
def cleanup_special_chars(string):
    ...
    # Special chars
    cleaned = re.sub('[ÀÁÂÄ]', 'A', cleaned)
    cleaned = re.sub('[ÈÉÊË]', 'E', cleaned)
    cleaned = re.sub('[ÍÌÎÏ]', 'I', cleaned)
    cleaned = re.sub('[ÒÓÔÖ]', 'O', cleaned)
    cleaned = re.sub('[ÙÚÛÜ]', 'U', cleaned)
    cleaned = re.sub('[áàâä]', 'a', cleaned)
    cleaned = re.sub('[éèêë]', 'e', cleaned)
    cleaned = re.sub('[íìîï]', 'i', cleaned)
    cleaned = re.sub('[óòôö]', 'o', cleaned)
    cleaned = re.sub('[úùûü]', 'u', cleaned)
    cleaned = re.sub('Æ', 'AE', cleaned)
    cleaned = re.sub('æ', 'ae', cleaned)
    cleaned = re.sub('Œ', 'OE', cleaned)
    cleaned = re.sub('œ', 'oe', cleaned)
    cleaned = re.sub('Ç', 'C', cleaned)
    cleaned = re.sub('ç', 'c', cleaned)

    # Fix for Lille Metropole and Ville Lille
    # cleaned = re.sub(r'[^\w\d\.\-_\(\)]', '_', cleaned)

    cleaned = cleaned.replace('\xb0', ".")
    cleaned = cleaned.replace('\xa0', ".")
    cleaned = cleaned.replace('\xa1', ".")
    cleaned = cleaned.replace('\xab', ".")
    cleaned = cleaned.replace('\xa9', "c")
    cleaned = cleaned.replace('\xbb', ".")
    cleaned = cleaned.replace('\xb2', "2")
    cleaned = cleaned.replace('\xe7', "c")
    cleaned = cleaned.replace('\xe8', "e")
    cleaned = cleaned.replace('\xe9', "e")
    cleaned = cleaned.replace('\xea', "e")
