...
class UsersImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        ...
        # Sinon, on récupère l'id existant
            ...
            user_exists = next((x for x in self.users_exists if x['username'] == element['username']))
...
class GroupsImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        ...
        if not response:
            ...
            if "status" not in response:
                ...
                # group exist
                group_exists = next((x for x in self.groups_exists if x['shortName'] == element['shortName']))
    ...
    def __after_func__(self):
        ...
        for group in self.groups:
            old_b = next((b for b in self.old_groups if b['shortName'] == group['shortName']))
...
class BureauxImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        ...
        while True:
            if not any(x['name'] == element['name'] for x in self.bureaux_exists):
                ...
                bureau_exists = next((x for x in self.bureaux_exists if x['name'] == element['name']))
    ...
    def __after_func__(self):
        ...
        for bureau in self.bureaux:
            old_b = next((b for b in self.old_bureaux if b['name'] == bureau['name']))
