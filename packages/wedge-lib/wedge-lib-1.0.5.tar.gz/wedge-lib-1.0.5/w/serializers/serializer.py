...
class ManyToManyField(Field):
    def __init__(
        self, serializer: Serializer, attr=None, call=False, label=None, required=True
    ):
        super().__init__(attr, call, label, required)
        self.serializer: SerpySerializer = serializer

    def to_value(self, value):
        if value and value.exists():
            return self.serializer(
                self.serializer.get_optimized_queryset(value.all()), many=True
            ).data
        return None
