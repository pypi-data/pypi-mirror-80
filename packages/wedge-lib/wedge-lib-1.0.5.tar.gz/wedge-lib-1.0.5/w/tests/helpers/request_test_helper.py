...
@contextmanager
def request_failure(response, method="get"):
    """
    Request failure
    Args:
        response (dict): response params :
            {
                "status_code": <status code, default:400>,
                "content": <content to use for response :optional>,
                "reason": <error reason: default:Bad request>,
...
@contextmanager
def request_success(response, method="get"):
    """
    Request success
    Args:
        response (dict): response params :
...
def mock_request(responses, method="get"):
    ...
    """
    params = {
        "target": f"w.services.technical.request_service." f"RequestService.{method}",
...
# todofsc : revoir RequestResponse pour lui passer une fausse reponse request
        ...
        params (dict): response params :
            {
                ...
                "content": <optional:str: content to use for response>,
                "json_file": <optional:json file or list json files for response>,
                "file": <optional:file for response>,
