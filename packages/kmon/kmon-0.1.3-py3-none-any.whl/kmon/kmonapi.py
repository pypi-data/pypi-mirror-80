import requests
from kmon.kmoncode import KmonException
import kmon.codedef as codedef
import numpy as np
import json
...
class KmonAPI:
    ...
    def set_token(self,token):
        try:
            ...
            if res != None and res.status_code != 200: raise KmonException(codedef.INVALID_HTTP_REQUEST)
            # data = res.json()
    ...
    def job_init(self,type,params,space=''):
        try:
            if self.__is_valid_token == False: raise KmonException(codedef.NOT_SET_TOKEN)
            ...
            if res != None and res.status_code != 200: raise KmonException(codedef.INVALID_HTTP_REQUEST)
            data = res.json()
            if data['code'] != codedef.SUCCESS.code: raise KmonException(data['code'],data['message'],data['data'])
