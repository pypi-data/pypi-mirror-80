...
from pyknp_eventgraph.component import Base
...
class Event(Base):
    ...
    def adnominal_events(self) -> List['Event']:
        ...
        return list(map(lambda r: r.modifiers, self.adnominal_relations))

    @property
    def sentential_complement_events(self) -> List['Event']:
        """Sentential complement events that modifies this event."""
        return list(map(lambda r: r.modifiers, self.sentential_complement_relations))
    ...
    def finalize(self):
        ...
        self.surf = self.to_string(space=False)
        self.surf_with_mark = self.to_string(mark=True, space=False)
        self.mrphs = self.to_string()
        self.mrphs_with_mark = self.to_string(mark=True)
        self.normalized_mrphs = self.to_string(truncate=True)
        self.normalized_mrphs_with_mark = self.to_string(mark=True, truncate=True)
        self.normalized_mrphs_without_exophora = self.to_string(truncate=True, needs_exophora=False)
        self.normalized_mrphs_with_mark_without_exophora = self.to_string(mark=True, truncate=True,
                                                                          needs_exophora=False)
        self.reps = self.to_string('repname', mark=False, truncate=False)
        self.reps_with_mark = self.to_string('repname', mark=True, truncate=False)
        self.normalized_reps = self.to_string('repname', mark=False, truncate=True)
        self.normalized_reps_with_mark = self.to_string('repname', mark=True, truncate=True)
        self.content_rep_list = self.to_content_rep_list()
    ...
    def to_tags(self, include_modifiers: bool = False) -> List[Tag]:
        """Return constituent tags.

        Args:
            include_modifiers: Specify ``True`` to include modifiers' tags.

        Returns:
            A list of :class:`pyknp.knp.tag.Tag` objects.

        """
        return self.to_basic_phrase_list(include_modifiers).to_tags()

    def to_string(self, type_: str = 'midasi', mark: bool = False, space: bool = True, truncate: bool = False,
                  needs_exophora: bool = True, include_modifiers: bool = False) -> str:
        ...
        """
        return self.to_basic_phrase_list(include_modifiers).to_string(
            type_=type_,
            mark=mark,
            space=space,
            normalize='predicate',
            truncate=truncate,
            needs_exophora=needs_exophora,
            normalizes_child_bps=False
        )
    ...
    def to_content_rep_list(self) -> List[str]:
        ...
        """
        return self.to_basic_phrase_list().to_content_rep_list()
    ...
    def to_basic_phrase_list(self, include_modifiers: bool = False) -> BasicPhraseList:
        ...
        def get_modifier_basic_phrase_list(bp: BasicPhrase) -> BasicPhraseList:
            ...
            for argument in argument_list:
                ...
                for argument_bp in argument.bpl:
                    ...
                    if include_modifiers:
                        for modifier_bp in get_modifier_basic_phrase_list(argument_bp):
        ...
        for predicate_bp in self.pas.predicate.bpl:
            ...
            if include_modifiers:
                for modifier_bp in get_modifier_basic_phrase_list(predicate_bp):
                    modifier_bp = copy.deepcopy(modifier_bp)
