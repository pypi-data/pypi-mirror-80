...
try:
    (cvc_configure, cvc_backup, cvc_recover, cvc_restore_default_overwrite,
     cvc_https, cvc_restrict_alt_client) = process_env_variables()
...
def get_token_path(instanceName):
    ...
    If no local simpana installation exists, ask the user to provide path in the command and exit the program.
    '''
    tokenpath = None
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        ...
        if status1 is True:
            ...
            LOG.error(
                "No Local Simpana Installation. Its now mandatory to pass the token file path as commandline argument.")
            print(
                "Operation failed. No Local Simpana Installation. Its now mandatory to pass the token file path as commandline argument.")
            exit(2)
    if tokenpath:
        return tokenpath
    else:
...
class Noninteractive():
    ...
    def login_ni(
        ...
        # Creating Commcell and other dependent objects to perform the
        # operations based on passed parameters :
        cs = {
        ...
        try:
            self._commcell_obj = commcell.Commcell(**cs)

        except Exception as login_exception:
            LOG.error("Error Encountered in Login : " + str(login_exception))
            print(
                "Operation failed. Error while login: " +
                str(login_exception))
            exit(2)
        pickledData = None
        cipherData = None
...
class MyPrompt(Cmd, object):
    ...
    def do_login(self, args):
        ...
        p.add_argument(
            '-https',
            dest="force_https",
            help='Login is forced to use https protocol. %s by default.' %
            str(cvc_https).lower(),
            choices=[
                'true',
                'false'],
            type=str.lower,
            default=str(cvc_https).lower(),
            required=False)
        p.add_argument(
            '-cert',
        ...
        force_https = True if pargs.force_https == 'true' else False
        ...
        try:
            self._commcell_obj = commcell.Commcell(**cs1)
        except Exception as login_exception:
            self._LOG.error(
                "Error Encountered in Login : %s" %
                (login_exception))
            self._LOG.error("Login failed for user %s" % commcell_username)
            print("Error in login : %s" % login_exception)
            return
        self._LOG.info(self._commcell_obj)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if len(
                    sys.argv) == 2 and sys.argv[1] not in [
                    "login",
                    "logout",
                    "find",
                    "backup",
                    ...
                    "job_status",
                    "subclient",
                    "browse"] and sys.argv[1].startswith("-") is False:
            ...
            if args.action == 'login':
                force_https = True if args.force_https == 'true' else False
