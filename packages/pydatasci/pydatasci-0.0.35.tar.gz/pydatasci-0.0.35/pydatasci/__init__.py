...
app_dir_free = appdirs.user_data_dir("pydatasci")
	...
	if permissions:
		...
		try:
			if os.name == 'nt':
				...
				command = 'icacls "' + app_dir_free + '" /grant users:(F) /c'
				os.system(command)
			else:
				# posix
...
def get_config():
	...
	if pds_config_exists:
		...
		print("\n=> Welcome to PyDataSci.\nConfiguration has not been set. To get started, run `pds.create_folder()` followed by `pds.create_config() in Python shell.\n")
