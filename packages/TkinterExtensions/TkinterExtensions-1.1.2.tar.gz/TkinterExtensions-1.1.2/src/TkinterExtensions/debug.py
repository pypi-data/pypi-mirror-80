...
def _childData(obj):
    ...
    return obj
def DebugWidgetRecursively(w, *, root: tk.Tk or tk.Toplevel, Message: str):
    assert (isinstance(w, _BaseTkinterWidget_) and isinstance(w, tk.BaseWidget))
    from pprint import PrettyPrinter
    pp = PrettyPrinter(indent=4)
    print(f'---------------- {Message} < {w.__class__.__name__} > ----------------')
    pp.pprint(_rootLevelDataRecurive(w, root))
    print()
    print()
