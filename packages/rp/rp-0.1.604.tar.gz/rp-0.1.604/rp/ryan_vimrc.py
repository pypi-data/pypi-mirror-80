...
" ctrl+p will fuzzy-search for files, like in sublime
" \ctrl+p will fuzzy-search most recent files
" ctrl+down, ctrl+up adds a second cursor above or below
" ctrl+n (in normal mode) is like ctrl+d in sublime (selects next occurence with new cursor)
...
" g< and g> will swap the position of an argument. Example: On g<: print(hello,wor|ld) --> print(wor|ld,hello) 
...
" \c   when added to a search in vim, makes the query case-insensitive
...
" function StartObsession(timer)
"     nmap <F8>s Obsession ~/.Session.vim
" endfunction
" let timer=timer_start(0,'StartObsession')
