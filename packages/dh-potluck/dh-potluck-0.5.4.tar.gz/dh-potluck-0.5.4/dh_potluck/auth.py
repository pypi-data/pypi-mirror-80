...
def role_required(role):
    ...
    def decorator(func):
        ...
        def get_brand_role():
            ...
            # access the brand_role by locating the specified brand
            for brand in g.user.brands:
                if brand.id == int(brand_id):
                    # use the matching brand label to return the brand_role
                    return g.user.permissions.get('brand_roles', {}).get(brand.label)
...
def validate_token_using_api(token):
    ...
    res = requests.get(auth_api_url + 'self',
