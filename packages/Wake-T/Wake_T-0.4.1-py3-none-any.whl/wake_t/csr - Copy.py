...
#@jit(nopython=True)
def apply_csr(ds_csr, s_current, z, n_bins, bin_size, bin_center_0, bunch_hist, ref_traj, bunch_matrix, bunch_q, gamma):
    """
    ...
    #z = -bunch_matrix[4]
    #bunch_hist, bin_edges = np.histogram(z, n_bins, weights=bunch_q)
    ...
    K1 = _calculate_kernel(it_range[0], ref_traj, n_bins, bin_size, gamma)
    for it in it_range[1:]:
        K1 += _calculate_kernel(it, ref_traj, n_bins,
                                        bin_size, gamma)
    K1 /= n_iter
...
@jit(nopython=True)
def _calculate_kernel(i, traj, n_bins, bin_size, gamma):
    """
    Calculate CSR kernel.
    ...
    w, KS = _calculate_kernel_long_range(i, traj, w_range[0], gamma)

    # Calculate short-range interactions.
    w_min = np.min(w)
    if w_range[0] < w_min:
        m = np.where(w_range < w_min)[0][-1]
        KS2 = _calculate_kernel_short_range(
    ...
    K1 = np.diff(np.append(np.diff(np.append(KS, 0)),0)) / bin_size / four_pi_eps0
    return K1

@jit(nopython=True)
def _calculate_kernel_long_range(i, traj, wmin, gamma):
    """
    The implementation of this method is an adaptation of code from OCELOT 
    (https://github.com/ocelot-collab/ocelot) written by S. Tomin and
    M. Dohlus.
    ...
    #_n = traj[1:4, i]
    _n = np.ascontiguousarray(traj[1:4, i]).reshape((3,1))
    n = _n - traj[1:4, :i]
    ...
    x = np.sum(n * t, axis=0)
    #a = traj[4:, i]
    a = np.ascontiguousarray(traj[4:, i]).reshape((3,1))
    K = ((beta * (x - np.sum(n * a, axis=0)) -
            beta_sq * (1 - np.sum(t * a, axis=0)) -
            gamma_sq) / R -
            (1. - beta * x) / w * gamma_sq)

    # Integrate kernel
    K[:-1] += K[1:]
    K *= 0.5 * np.diff(np.append(s, 0))
    K = np.cumsum(K[::-1])[::-1]

    return w, K
 
@jit(nopython=True)
def _calculate_kernel_short_range(i, traj, w_range, gamma):
    """
...
class CSRCalculator():
    ...
    def apply_csr(self, bunch_matrix, bunch_q, gamma, element, element_pos):
        ...
        apply_csr(ds_csr, s_current, z, self._n_bins, bin_size, bin_center_0, bunch_hist, self._ref_traj,
                  bunch_matrix, bunch_q, gamma)
        

    def _apply_csr(self, bunch_matrix, bunch_q, gamma, element, element_pos):
        """
        Apply the CSR kick to the specified particle bunch at a certain
        lattice location.
