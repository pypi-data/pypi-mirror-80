...
def b_theta_0_driver(xi, r, xi_d, n_d, sz_d, sr_d):
    return (- n_d * np.exp(-(xi-xi_d)**2/(2*sz_d**2)) * sr_d**2/r * (1 - np.exp(-r**2/(2*sr_d**2)))
            - 25.2 * np.exp(-(xi-6.5)**2/(2*0.3**2)) * 0.1**2/r * (1 - np.exp(-r**2/(2*0.1**2))))
...
def equations_of_motion(xi, r, pr, gamma, q, xi_d, n_d, sz_d, sr_d, a_0, w_0, tau, xi_c):
    ...
    dpr = gamma * dr_psi_p / (1 + psi_p) - 0*b_theta_bar_p - b_theta_0_p + nabla_a / (4 * (1 + psi_p))
    dr = pr / (1 + psi_p)
...
def test_rk():
    ...
    psi_mesh = np.zeros((200, steps))
    a2_mesh = np.zeros((200, steps))
    ...
    for step in np.arange(steps):
        ...
        for i, r in enumerate(np.linspace(0, 4, 200)):
            psi_mesh[i, step] = psi(r_part, q_part, r)
            a2_mesh[i, step] = laser_a2(xi, r, a_0, w_0, tau, xi_c)
    ...
    plt.subplot(222)
    plt.imshow(np.gradient(psi_mesh, dxi)[1], aspect='auto')
    plt.subplot(223)
