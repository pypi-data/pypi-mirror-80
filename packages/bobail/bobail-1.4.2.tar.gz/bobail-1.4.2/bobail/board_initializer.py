...
class BoardInitializer:
	...
	def set_starting_pieces(self):
		...
		for key, row in self.board.position_layout.items():
			for key, position in row.items():
				player_number = 1 if position in player_starting_positions[1] else 2 if position in player_starting_positions[2] else 3 if position in player_starting_positions[3] else None
	...
	def create_piece(self, player_number, position):
		...
		piece.bobail = True if player_number == 3 else False
		return piece