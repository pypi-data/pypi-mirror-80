...
def prepare_auto_ai_model_to_publish_notebook_normal_scenario(
    ...
    request_json['content_location']['connection'] = result_connection.connection.to_dict()
...
class RunDetailsGenerator:
    ...
    def __init__(self, wml_client: 'APIClient', run_id: str, wait_time: float) -> None:
...
def try_import_joblib():
    ...
    def fallback_load(f):
        @wraps(f)
        def _f(self, *method_args, **method_kwargs):
            try:
                output = f(*method_args, **method_kwargs)

            except:
                import sklearn
                if '0.20' in str(sklearn.__version__):
                    raise LibraryNotCompatible(
                        reason=f"Installed scikit-learn version: {sklearn.__version__} is not compatible with "
                               f"version included in AutoAI pipeline. Please try to install another 0.23.2 version.")
                else:
                    raise LibraryNotCompatible(
                        reason=f"Installed scikit-learn version: {sklearn.__version__} is not compatible with "
                               f"version included in AutoAI pipeline. Please try to install another 0.20.3 version.")
    ...
    new_load = fallback_load(joblib.load)
    joblib.load = MethodType(new_load, joblib)
...
def get_sw_spec_and_type_based_on_sklearn(client: 'APIClient') -> Tuple[str, str]:
    """Based on user environment, check sklearn version and find apropriate sw spec.
    ...
    """
    import sklearn
    if '0.20.' in sklearn.__version__:
        sw_spec = client.software_specifications.get_id_by_name('autoai-kb_3.0-py3.6')
        model_type = 'scikit-learn_0.20'

    elif '0.23.' in sklearn.__version__:
        sw_spec = client.software_specifications.get_id_by_name('autoai-kb_3.1-py3.6')
