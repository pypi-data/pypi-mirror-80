...
class DataJoinPipeline:
    ...
    def __init__(self, preprocessed_data_connection: 'DataConnection', optimizer: 'RemoteAutoPipelines') -> None:
        self._obm_json = preprocessed_data_connection._download_obm_json_from_cos()
...
class OBMPipelineGraphBuilder:
    ...
    def get_feature_info(feature_info_json: dict) -> dict:
        ...
        results['columns'] = join_path.get("columns")
    ...
    def build_preprocessing_prefix(self) -> 'lale.operators.TrainableIndividualOp':
        ...
        for msg in self.trained_pipeline_json.get("feature_engineering_components")[0].get("obm"):
            ...
            join_keys = feature_info["columns"]
        ...
        for join_tables_str, join_info in joins.items():
            ...
            scan_conjunction = scan_table_nodes[0]
            for i in range(1, len(scan_table_nodes)):
                scan_conjunction = scan_conjunction & scan_table_nodes[i]
            ...
            if len(predicates_list) > 0:
                ...
                join_prefix = (scan_conjunction) >> join_node
            else:
                join_prefix = scan_conjunction
                        ...
                        if op in time_series_functions:
                            ...
                            if op == "ItemsetMI":
                                if pattern is not None:
                                    ...
                                    kwargs['value'] = ""
                                    print("Pattern missing for ItemsetMI")  # TODO: Raise an exception
            ...
            if len(ops_list) > 0:
                extractors_conjunction = ops_list[0]
                for i in range(1, len(ops_list)):
                    extractors_conjunction = extractors_conjunction & ops_list[i]
                extractors_prefix = join_prefix >> (extractors_conjunction)
                extractors_list.append(extractors_prefix)
        # Use ConcatFeatures on the extractors_prefix and then selectFeatures
        feature_extractors_conjunction = extractors_list[0]
        for i in range(1, len(extractors_list)):
            feature_extractors_conjunction = feature_extractors_conjunction & extractors_list[i]
        all_features = feature_extractors_conjunction >> ConcatFeatures()
        ...
        if self.categorical_column_names is not None:
            ...
            nested_pipeline = all_features >> string_indexer_encoding
        else:
            nested_pipeline = all_features
