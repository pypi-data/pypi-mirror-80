...
from ibm_watson_machine_learning.helpers.connections import (
    DataConnection, S3Location, FSLocation, CP4DAssetLocation, WMLSAssetLocation, CloudAssetLocation, WSDAssetLocation,
    DeploymentOutputAssetLocation)
from ibm_watson_machine_learning.utils.autoai.enums import (DataConnectionTypes)

from ibm_watson_machine_learning.utils.autoai.errors import DataLocationTypeNotSupported
    ...
    for data_connection in source_data_connections:
        ...
        if isinstance(data_connection.location, FSLocation):
            ...
            if workspace.wml_client.ICP:
                ...
                data_connection.location = WMLSAssetLocation(
                    asset_id) if workspace.WMLS else CP4DAssetLocation(asset_id)
        ...
        # note: set project id when we are working on CP4D and using Data Assets Location, space_id for WMLS
        if isinstance(data_connection.location, WMLSAssetLocation):
            data_connection.location.href = data_connection.location.href.format(
                option='space_id',
                id=workspace.wml_client.default_space_id)

        elif isinstance(data_connection.location, CP4DAssetLocation):
            if deployment:
                data_connection.location.href = data_connection.location.href.format(
                    option='space_id',
                    id=workspace.wml_client.default_space_id)
        ...
        elif isinstance(data_connection.location, CloudAssetLocation):
            if deployment:
                data_connection.location.href = data_connection.location.href.format(
                    option='space_id',
                    id=workspace.wml_client.default_space_id)
            else:
                raise DataLocationTypeNotSupported(value_name='CloudAssetLocation',
                                                   reason='Data asset type on Cloud is supported for Batch Deployment')
        # -- end note
...
def validate_deployment_output_connection(results_data_connection: Union['DataConnection', None],
    ...
    if isinstance(results_data_connection.location, (CP4DAssetLocation, WMLSAssetLocation)):
        if workspace.wml_client.default_space_id:
    ...
    if not isinstance(results_data_connection.location, (S3Location, CP4DAssetLocation, WMLSAssetLocation, DeploymentOutputAssetLocation)):
        raise TypeError('Unsupported results location type. Results reference can be stored only in '
                        'one of [S3Location, CP4DAssetLocation, WMLSAssetLocation, DeploymentOutputAssetLocation].')
    # -- end
    return results_data_connection
