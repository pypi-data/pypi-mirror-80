...
class Mail:
    ...
    def get_free_space(self, signature=None):
        ...
        with raisin.Printer("Get free space of '%s'..." % self.name, signature=signature) as p:# on annonce que l'on va faire une operation risque
            ...
            with imaplib.IMAP4_SSL(host=ingoing_serv, port=port) as imapserver:
                ...
                with raisin.Printer("Finding main directory..."):
                    ...
                    dossier = re.fullmatch(r"\(.*\)\s+\".*\"\s+(?P<dossier>.*)", dossiers[0].decode()).group("dossier")
                    p.show("It is '%s'." % dossier)
                with raisin.Printer("Getting quota..."):
    ...
    def send(self, address, subject, message, attachment=None, signature=None):
        ...
        assert re.fullmatch(r"[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]{2,}\.[a-z]{2,4}", address), "l'address %s n'est pas une address valide" % address
