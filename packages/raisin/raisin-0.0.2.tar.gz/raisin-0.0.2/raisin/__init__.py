...
try:
    ...
    warnings.warn("'numpy' failed to import. Some process will be slow.")
    numpy = None
try:
    import psutil
except ImportError:
    warnings.warn("'psutil' failed to import, no possibility to get the cpu temperature.")
    psutil = None
try:
    import regex as re
except ImportError:
    warnings.warn("'regex' failed to import. 're' is going to be used, but it is less efficient than regex.")
    import re
try:
    import sympy
    ...
    try:
        ...
        warnings.warn("'giacpy' failed to import. Sometimes sympy is not able to compute but 'giacpy'"
                      "knows how to do that, especially concerning implicit operators.")
        giacpy = None
except ImportError:
    warnings.warn("'sympy' failed to import. There is no symbolic computing.")
    sympy = None
    giacpy = None
    count_ops = None
...
def compress(obj, *, compresslevel=3, psw=None, copy_file=False, parallelization_rate=0, signature=None):
    ...
    with Printer("Compress...", signature=signature):
        return reader.serialization.compress(
                                obj,
...
def decompress(chaine, *, psw=None, parallelization_rate=0, signature=None):
    ...
    with Printer("decompress...", signature=signature):
        return reader.serialization.decompress(
                                chaine,
                                psw=psw,
                                parallelization_rate=parallelization_rate,
                                signature=signature)
...
def serialize(obj, *, buff=1024*1024, compresslevel=-1, psw=None, copy_file=True, parallelization_rate=1, signature=None):
    ...
    yield from reader.serialization.serialize(
                                        obj,
                                        signature=signature,
                                        buff=buff,
                                        compresslevel=compresslevel,
...
def deserialize(data, *, parallelization_rate=0, psw=None, signature=None):
    ...
    return reader.serialization.deserialize(data, parallelization_rate=parallelization_rate, psw=psw, signature=signature)
...
class Map(raisin.Map):
    ...
    def __init__(self,
        ...
        :param iterables: Chaque iterable associes a chacuns des arguments de lafonction.
