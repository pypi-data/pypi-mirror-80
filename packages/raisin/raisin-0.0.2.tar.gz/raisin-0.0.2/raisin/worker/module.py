...
def make_analyse():
    ...
    """
    with raisin.Printer("Updated information on modules..."):
...
def get_infos(modulename):
    ...
    with raisin.Printer("getting infos about '%s' ..." % modulename) as p:
        try:
...
def test_module(module, modulename):
    ...
    """
    with raisin.Printer("verification of the '%s' module..." % modulename) as p:
        if modulename == "abc":
            return None
...
def get_installed_modules():
    ...
    """
    with raisin.Printer("Recovery of the names of all installed modules..."):
...
def get_dependencies(modulename):
    ...
    with raisin.Printer("Recherche des dependance du module '%s'..." % modulename) as p:
        with raisin.Printer("Recherche de l'emplacement..."):
            ...
            if not emplacements:                    # si on a rien trouve
                ...
                return set()
        with raisin.Printer("Analysis of these files..."):
            depend = set()
...
def get_all_dependencies(modulename):
    ...
    with raisin.Printer("Recherche de toutes les dependances de '%s', meme les plus elognes..." % modulename):
...
def get_unmet_dependencies(modulename):
    ...
    missing = set()
    with raisin.Printer("Get unmet dependances..."):
...
def install(modulename):
    ...
    def install_tkinter():
        ...
        """
        if os.name == "posix":
            if os.system("sudo apt install python3-tk") == 0:
                return True
            if os.system("sudo packman -S python3-tk") == 0:
                return True
        return False
    ...
    def install_giacpy():
        ...
        on peu faire:
            ...
            gpg -a --export 998B1156 | sudo apt-key add -
            apt-get update
        """
        if os.name == "posix":
            if os.system("sudo apt install python3-giacpy") == 0:
                return True
            if os.system("sudo packman -S python3-tk") == 0:
                return True
            if os.system("sudo add-apt-repository 'deb http://www-fourier.ujf-grenoble.fr/~parisse/debian/ stable main'") == 0:
                with open("xcas_public_key.gpg", "wb") as f:
                    f.write(b'-----BEGIN PGP PUBLIC KEY BLOCK-----\n'
                            ...
                            b'tF/Durjx0F9r64\n=1H7u\n-----END PGP PU'
                            b'BLIC KEY BLOCK-----\n')
                os.system("sudo apt add xcas_public_key.gpg")
                os.remove("xcas_public_key.gpg")
                if os.system("sudo apt update") == 0:
                    if os.system("sudo apt install *giac*") == 0:
                        return True
        return False

    assert type(modulename) is str, "'modulename' doit etre le nom du module (str), or il s'agit d'un (%s)." % type(modulename)

    with raisin.Printer("Install '%s'..." % modulename) as p:
        ...
        # install with pip
        import pip
        if not raisin.security.root_privilages(): # on prend les droits administrateur
            p.show("Il se peut que l'installation capotte faute de droit suffisant.")
        with raisin.Printer("Install '%s' with 'pip'..." % modulename):
            res = pip.main(["install", modulename])
            if res == 0:
                validity, version, is_python = get_infos(modulename)
