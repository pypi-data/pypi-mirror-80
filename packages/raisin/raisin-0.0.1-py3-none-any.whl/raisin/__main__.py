...
def main(args_brut=[]):
    ...
    if args.parser_name == "install":           # si il faut installer un module
        ...
        if args.module == "raisin":             # si c'est raisin qu'il fat desinstaller
        ...
        if args.name == "server" or tout:
            orchestrator.add(
                os.path.join(os.path.dirname(raisin.__file__), "worker/shortcut/server.py"), 
                limit_enable=True,
                permission_enable=True,
                )
        if args.name == "padlock" or tout:
            orchestrator.add(
                os.path.join(os.path.dirname(raisin.__file__), "worker/shortcut/padlock.py"),
                limit_enable=False,
                permission_enable=False,
                )
        if args.name == "statistics" or tout:
            orchestrator.add(
                os.path.join(os.path.dirname(raisin.__file__), "worker/shortcut/statistics.py"),
                limit_enable=True,
                permission_enable=False
                )
        else:
