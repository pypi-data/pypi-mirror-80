...
try:
    ...
    warnings.warn("'pycryptodome' failed to import. They are no evailability to cipher and uncrypt datas.")
    Crypto = None
import hashlib
import os
import time
...
def rsa_keys(*, psw=None, parallelization_rate=0, signature=None):
    ...
    si il y a un mot de passe, la clef privee est chifree avec ce mot de passe
    """
    if not Crypto:
        raise ImportError("'pycryptodome' est indispensable pour cette operation.")
...
def change_private_key(private_key_pem, old_psw, new_psw):
    ...
    retourne la nouvelle cleff privee serialisee au format PEM
    """
    if not Crypto:
        raise ImportError("'pycryptodome' est indispensable pour cette operation.")
    assert type(private_key_pem) is bytes, "La clef privee doit etre de type 'bytes', pas %s." % type(private_key_pem)
...
def get_private_key(*, signature=None):
    """
    ...
    key_pem = settings["account"]["security"]["private_key"]    # affin d'en extraire la clef privee
    psw = request_psw(force=False, check=False)
...
class PswRequester:
    ...
    def __call__(self, force, check=True, existing_window=None):
        """
...
def root_privilages(cmdLine=None, wait=True):
    """
    permet au programe une elevation
    de privilege
    """
    def isUserAdmin():
            """
            retourne True si le programme a deja les droits administateur
            """
            if os.name == "nt":
                try:
    ...
    with raisin.Printer("Get root privilege...") as p:
        if isUserAdmin():
            p.show("J'ai deja les droit en fait!")
            return True
        else:
            p.show("Zut! j'ai pas encore les droits d'admin")
        # if runAsAdmin(cmdLine, wait):
        #     p.show("C'est bon, les droits sont la!")
        #     return True
        p.show("Bon, j'ai pas reussi a recuperer les droits!")
        return False

def restrict_acces(group, reps):
    """
