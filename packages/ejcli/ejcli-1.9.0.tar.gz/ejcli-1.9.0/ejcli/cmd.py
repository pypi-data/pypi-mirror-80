...
class BruteCMD(cmd.Cmd):
    ...
    def do_submit(self, cmd):
        ...
        if not sp[1].isnumeric():
            raise EJError("lang_id must be a number")
        try:
...
#           raise EJError("File is binary.")
    ...
    def onecmd(self, c):
        try:
            ...
            except (EJError, SystemExit): raise
            except BaseException:
                raise EJError(traceback.format_exc())
        except EJError as e:
            self.print_error(e)
    ...
    def __getattr__(self, cmd):
        ...
        cmd = cmd[3:]
        try: __import__('ejcli.commands.' + cmd)
        except ImportError: raise AttributeError(cmd)
        return type(self.__getattr__)(getattr(getattr(ejcli.commands, cmd), 'do_'+cmd), self)
    def get_names(self):
        dir0 = set(dir(type(self)))
        import pkgutil
        try: cmds = list(pkgutil.iter_modules(ejcli.commands.__path__))
        except: pass
...
#       try: __import__('ejcli.commands.' + cmd)
#       except ImportError:
        raise EJError('No such command: ' + cmd.split()[0])
#       return getattr(getattr(ejcli.commands, cmd), 'do_'+cmd)(self, arg)
    ...
    def do_geterror(self, cmd):
        ...
        if sp and sp[0] == '--binary':
            if not has_feature(self.url, self.cookie, 'compile_error', 'binary'):
                raise EJError('This system does not support --binary in geterror.')
            del sp[0]
            kwargs['binary'] = True
        if len(sp) >= 2 and sp[0] == '--kind':
            if not has_feature(self.url, self.cookie, 'compile_error', 'kind'):
                raise EJError('This system does not support --kind in geterror.')
            try: kwargs['kind'] = int(sp[1])
            except ValueError:
                raise EJError('Kind must be a number')
            del sp[:2]
        if len(sp) != 1:
            return self.do_help('geterror')
        try: sp[0] = int(sp[0])
        except ValueError:
            raise EJError('Submission ID must be a number')
