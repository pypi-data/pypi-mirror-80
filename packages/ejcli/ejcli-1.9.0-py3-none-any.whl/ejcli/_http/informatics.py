...
class Informatics(Ejudge):
    ...
    def __init__(self, url, login, password):
        ...
        for proto in ('http', 'https'):
            ...
            raise EJError("Not an old.informatics.msk.ru URL")
        self.url = url
...
#       data.append(b'"action_40"\r\n\r\nSend!')
    ...
    def download_file(self, prob_id, filename):
        raise EJError("File download doesn't exist on old.informatics.msk.ru")
    def clar_list(self):
        raise EJError("Clarifications don't exits on old.informatics.msk.ru")
    def submit_clar(self, *args):
        raise EJError("Clarifications don't exits on old.informatics.msk.ru")
    def read_clar(self, id):
        raise EJError("Clarifications don't exits on old.informatics.msk.ru")
    def get_samples(self, subm_id):
        raise EJError("Can't get samples on old.informatics.msk.ru")
    def scoreboard(self):
        raise EJError("Scoreboard is not supported on old.informatics.msk.ru")
