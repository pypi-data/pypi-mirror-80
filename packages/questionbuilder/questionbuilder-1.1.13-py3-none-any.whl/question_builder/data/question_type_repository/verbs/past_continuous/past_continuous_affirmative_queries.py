...
CALL apoc.cypher.run("
    ...
    MATCH (content)-[:HAS_WORD{position: pattern_rel.position - 1}]-(auxiliaryverb:WORD)
...
MATCH (interest) - [:HAS_CONTENT] -> (content:CONTENT) - [pattern_rel:HAS_PATTERN] -> (:PATTERN{code:'PACC'})
MATCH (content)-[:HAS_WORD{position: pattern_rel.position - 2}]-(subject:WORD)
MATCH (content)-[:HAS_WORD{position: pattern_rel.position - 1}]-(auxiliaryverb:WORD)
MATCH (content)-[rel_target_word:HAS_WORD{position: pattern_rel.position}]-(target_word:WORD)
