...
class FuturePerfectNegativeChoiceQuestionCreator(QuestionCreator):
    ...
    @staticmethod
    def _get_combined_verbtowill_and_negation(negation):
        if negation == NOT:
            return f"{WILL} {NOT}"
        else:
            return WONT
    ...
    @staticmethod
    def _get_correct_answer(target_verb, subject, verbtowill_and_negation):
        return f"{subject} {verbtowill_and_negation} {HAVE} {target_verb}"

    @staticmethod
    def _get_baits(target_verb, subject):
