...
class SimplePresentNegationChoiceQuestionCreator(QuestionCreator):
    ...
    @staticmethod
    def _get_do_with_negation(negation_adverb):
    ...
    @staticmethod
    def _get_correct_answer(target_verb, subject, do_with_negation):
        return f"{subject} {do_with_negation} {target_verb}"

    @staticmethod
    def _get_baits(
        target_verb, subject, do_with_negation, target_verb_present_continuous
    ):
