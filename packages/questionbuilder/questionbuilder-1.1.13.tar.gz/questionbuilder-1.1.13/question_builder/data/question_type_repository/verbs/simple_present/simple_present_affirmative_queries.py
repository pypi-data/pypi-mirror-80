...
CALL apoc.cypher.run("
    ...
    MATCH (interest) - [:HAS_CONTENT] -> (content:CONTENT) - [pattern_rel:HAS_PATTERN{lemma:{lemma}}] -> (:PATTERN{code:'PTC'})
    MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 1}]-(subject:WORD)
...
MATCH (interest) - [:HAS_CONTENT] -> (content:CONTENT) - [pattern_rel:HAS_PATTERN] -> (:PATTERN{code:'PTC'})
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 1}]-(subject:WORD)
MATCH (content)-[rel_target_word:HAS_WORD{position : pattern_rel.position}]-(target_word:WORD)
