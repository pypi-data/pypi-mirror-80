...
class Model(nn.Module):
    ...
    def __init__(self, model_cfg='yolov5m.yaml', ch=3, nc=None):
        ...
        if type(model_cfg) is dict:
            ...
            with open(model_cfg) as f:
                self.md = yaml.load(f, Loader=yaml.Loader)  # model dict

        # Define model
        if nc and nc != self.md['nc']:
            logger.info("[Overriding {} nc={} with nc={}]".format(model_cfg.split('\\')[-1], self.md['nc'], nc))
            self.md['nc'] = nc
        self.ch = ch
        self.model, self.save = parse_model(self.md, ch=[self.ch])  # model, savelist, ch_out
    ...
    def restore_model(self, model_dict=None, **kwargs):
        ...
        """
        self.model, _ = parse_model(self.md, ch=[self.ch])
...
def parse_model(md, ch):
    ...
    for i, (f, n, m, args) in enumerate(md['backbone'] + md['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, Bottleneck, SPP, Focus, BottleneckCSP]:
            c1, c2 = ch[f], args[0]
