...
from azureml.contrib.automl.dnn.vision.object_detection.common.constants \
    import training_settings_defaults
...
class TestModelWrappers:
    ...
    def test_wrappers_export_onnx(self):
        ...
        input = torch.rand(1, 3, 600, 800, device=device, requires_grad=False)
        ...
        r50 = FasterRCNNResnet50FPNWrapper(20)
        r50_file = 'FasterRCNNResnet50FPNWrapper.onnx'
        r50.export_onnx_model(file_path=r50_file, device=device)
        check_exported_onnx_model(r50_file, r50, input, device, get_model_output, False)
