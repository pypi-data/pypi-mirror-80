...
def preview_video(cam_id, settings = None, on_key = None, on_mouse = None):
    ...
    def wrapped_on_mouse(event, x, y, flags, params):
        if on_mouse is not None and event == cv2.EVENT_LBUTTONDOWN:
            on_mouse(x,y, img_to_send[0], is_left=True)
        elif on_mouse is not None and event == cv2.EVENT_RBUTTONDOWN:
            on_mouse(x,y, img_to_send[0], is_left=False)
    ...
    while(True):
        ...
        if settings is not None:
            if 'skintone_low' in settings:
                mask = ARTable.detect_color_hsv(color_img, settings['skintone_low'], settings['skintone_high'])
...
def find_skintone(settings):
    ...
    settings['skintone_low'] = (0,0,0)
    settings['skintone_high'] = (180,255,255)
    settings['has_pressed'] = False
    def on_mouse(x, y, img, is_left = False):
        # Get the selected pixel
        '''(h,w,c) = img.shape
        if not (x >= 0 and x < w and y >= 0 and y < h):
            return
        bgr_val = img[y,x]
        rgb = (bgr_val[2] / 255.0, bgr_val[1] / 255.0, bgr_val[0] / 255.0)
        # convert to hsv
        hsv_val = colorsys.rgb_to_hsv(*rgb)
        # Back to uint's
        hsv_rounded = tuple([min(max(0, round(v * 255)), 255) for v in hsv_val])
        # Now we have an hsv pixel, set the hsv skintone
        '''
        hsv_img = ARTable.to_hsv(img)
        hsv_rounded = tuple(map(int, hsv_img[y,x]))
        if is_left:
            settings['skintone_low'] = hsv_rounded
        else:
            settings['skintone_high'] = hsv_rounded
        print(settings['skintone_low'], settings['skintone_high'])
    
    print("Find your skintone by clicking it.")
    print("Left click for lower bound, Right click for upper.")
    print(" P to pause is useful, here")
    preview_video(settings['webcam'], settings, None, on_mouse)
...
def find_skintone2(settings):
    settings['skintone_low'] = (0, 120, 30)
    settings['skintone_high'] = (33, 255, 255)
    print("Using pre-calibrated skintone")
    return True
...
def main(ui_path):
    ...
    clear_screen()
    print_divider(7)
    skintone_success = find_skintone2(settings)
