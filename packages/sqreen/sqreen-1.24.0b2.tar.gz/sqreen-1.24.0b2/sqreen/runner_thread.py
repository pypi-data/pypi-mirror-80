...
def start():
    ...
    if RUNNER_THREAD is None:
        try:
            ...
            done = RUNNER_THREAD.unblock_main_thread.wait(CONFIG["INSTRUMENTATION_TIMEOUT"])
            if done is not True:
                LOGGER.critical(
...
class RunnerThread(Thread):
    ...
    def __init__(self, runtime_infos, queue=None, observations_queue=None,
        ...
        self.reactive = reactive or Reactive(self.observations_queue, self.queue)
        self.runner = None
        self.started = False
        self.pid = None
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                session = get_session(
                    session_url, token, app_name, proxy_url=get_proxy_url(),
