    ...
    for callback in callbacks:
        try:
            if isinstance(callback, RuleCallback):
                ...
                if runner.reactive is not None:
                    runner.reactive.add_callback(callback)
                else:
                    raise RuntimeError("reactive engine is disabled, cannot load {!r}".format(callback))
            else:
...
def instrumentation_remove(runner, params=None):
    ...
    if runner.reactive is not None:
        runner.reactive.remove_callbacks()
    return True
