...
class BaseRequest(object):
    ...
    @property
    def request_id(self):
        if self._request_uuid is None:
            self._request_uuid = uuid.uuid4()
        return self._request_uuid.hex
    ...
    def request_rate(self):
        if self._request_uuid is None:
            self._request_uuid = uuid.uuid4()
        return self._request_uuid.int / self.MAX_UUID
            ...
            for key, value in self.raw_headers.items():
                ...
                if not key.startswith("HTTP_"):
                    if key == "CONTENT_LENGTH" and value:
                        ...
                        continue
                self._request_headers[key] = to_unicode_safe(value)
