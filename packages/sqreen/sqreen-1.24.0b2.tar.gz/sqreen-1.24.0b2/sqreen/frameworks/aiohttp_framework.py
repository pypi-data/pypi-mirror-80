...
class AioHTTPRequest(BaseRequest):
    ...
    def form_params(self):
        ...
        if self._form_params is None:
            ...
            if post_params is not None:
                ...
                # It is important to call this only after _form_params
                # was set because it could reenter this method.
                self.on_request_body_read()
        ...
        if self._json_params is None:
            try:
                ...
                if self.request._read_bytes is not None:
                    ...
                    self._json_params = json.loads(data)
            except Exception:
                LOGGER.debug("couldn't get request.json from the framework",
                             exc_info=True)
                self._json_params = {}
            else:
                # It is important to call this only after _json_params
                # was set because it could reenter this method.
                self.on_request_body_read()
    ...
    def on_request_body_read(self):
        """Method called when either the JSON or POST parameters are available.

        Do not change this method name as it might be used by some rule.
        """
        pass
