...
from ..utils import HAS_TYPING, is_unicode, now
...
class WAFCBMixin(object):
    ...
    def execute(self, waf_context, params, budget):
        ...
        if self.metrics:
            # Record metrics only if the request is sampled
            ...
            if self.metrics_sampling.should_sample(random_value=random_value):
                at = now()
                self.record_observation("sq.internal.powerwaf.full", None, ret.perf_total_runtime / 1000.0, at=at)
                self.record_observation("sq.internal.powerwaf.cache.resolved", None, ret.perf_cache_hit_rate, at=at)
                perf_data = json.loads(ret.perf_data.decode("utf-8", errors="replace"))
                for name, value in perf_data.get("topRuleRuntime"):
                    self.record_observation("sq.internal.powerwaf.{}".format(name), None, value / 1000.0, at=at)

        if ret.report:
            waf_data = json.loads(ret.data.decode("utf-8", errors="replace"))
            self.record_waf_attack(waf_data)
    ...
    def record_waf_attack(self, waf_data):
        """ Record a WAF attack.
        """
        self.record_attack({"waf_data": waf_data})
...
class WAFCB(WAFCBMixin, RuleCallback):
    ...
    def record_waf_attack(self, waf_data):
        ...
        self.record_attack({"waf_data": waf_data, "ba_type": get_agent_type()})
