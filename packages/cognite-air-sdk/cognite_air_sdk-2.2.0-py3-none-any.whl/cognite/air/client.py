...
class AIRClient:
    ...
    def retrieve_fields(self, field_names: List[str], ignore_unknown_field_names: bool = False) -> List[str]:
        ...
        for id, field in zip(field_names, fields):
            field_type = self._config.data_fields_defs[id]["type"]
            ret_val = parse_json_if_json(field)
            if isinstance(ret_val, list):  # this means multiple=True
    ...
    def convert_type_info(dct):
        ...
        dct["type"] = legal_types[dct["type"]]
        return dct

    def create_data_field_dct(self, json_str: Optional[str]):
        lst_of_field_defs = parse_json_if_json(json_str) or []
