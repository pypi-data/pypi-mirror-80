...
from cognite.air.constants import (
    ...
    AIR_EVENTS_META_KEY_ACKNOWLEDGED,
    AIR_EVENTS_META_KEY_DASHBOARD_ID,
    AIR_EVENTS_META_KEY_MODEL,
    AIR_EVENTS_META_KEY_MODEL_VERSION,
    AIR_EVENTS_META_KEY_ORIGINAL_MODEL_VERSION,
    AIR_EVENTS_META_KEY_PROJECT_NAME,
    AIR_EVENTS_META_KEY_SA_EXT_ID,
    AIR_EVENTS_META_KEY_SKIP_NOTIFICATION,
    AIR_EVENTS_META_KEY_SYSTEM_ID,
    EVENT_ASSET_IDS,
    EVENT_DATA_SET_ID,
    EVENT_DATA_SET_IDS,
    EVENT_EXT_ID,
...
from cognite.air.utils import is_string_truthy, valfilter
...
class AIREventsAPI(BaseAPIClient):
    RESERVED_AIR_EVENT_META_KEYS = set(
        [
            AIR_EVENTS_META_KEY_MODEL,
            AIR_EVENTS_META_KEY_MODEL_VERSION,
            AIR_EVENTS_META_KEY_ORIGINAL_MODEL_VERSION,
    ...
    RESERVED_PARAMS_DCT = MappingProxyType(
        {
            ...
            EventEndpoints.CREATE: set([EVENT_EXT_ID, EVENT_TYPE, EVENT_SUBTYPE, EVENT_ASSET_IDS, EVENT_DATA_SET_ID]),
        }
    )
        ...
        return {
            ...
            EVENT_METADATA: {
                ...
                **self.air_event_required_meta,
                AIR_EVENTS_META_KEY_DASHBOARD_ID: self._config.schedule_asset.metadata["dashboardId"],
                AIR_EVENTS_META_KEY_SYSTEM_ID: self._config.schedule_asset.metadata["systemId"],
                AIR_EVENTS_META_KEY_ORIGINAL_MODEL_VERSION: self._config.model_version_stripped,
                AIR_EVENTS_META_KEY_PROJECT_NAME: os.environ["COGNITE_PROJECT"],
                AIR_EVENTS_META_KEY_SKIP_NOTIFICATION: str(not notification),
                AIR_EVENTS_META_KEY_ACKNOWLEDGED: str(not notification),
            },
        }
    ...
    def _make_events_air_compatible_inplace(self, events: List[Event], *, is_alert) -> None:
        for ev in events:
            ...
            attr_dct = _attr_fn(meta=ev.metadata or {})
            attr_dct.update(
    ...
    def _validate_event_input(self, event: Union[Event, List[Event]], *, is_alert) -> Tuple[List[Event], bool]:
    ...
    def create(self, event: Union[Event, List[Event]]) -> Union[Event, EventList]:
        items, single_item = self._type_check_event_input(event, is_alert=False)
    ...
    def create_alert(self, event: Union[Event, List[Event]]) -> Union[Event, EventList]:
        items, single_item = self._type_check_event_input(event, is_alert=True)
        return self.client.events.create(items[0] if single_item else items)
