...
def _tagname_from_header(tag):
    ...
    tagname =  hdr.find_next("span", class_="mw-headline").attrs["id"]
    print(tagname)
    return tagname
...
def scrape_primary_features(url=OSM_FEATURES_URL, up_to="Additional_properties", use_cache=False):
    r = requests.get(url)
    soup = BeautifulSoup(r.text, 'html.parser')
    # Get the tables and taglists _above_ the 'Additional Properties' items
    # that tail the data in the doc
    adp = soup.find_all("span", class_="mw-headline", id=up_to).pop()
    ...
    next_tags = adp.find_all_next('div', class_='taglist')
    prev_tags = adp.find_all_previous('div', class_='taglist')
    tags = next_tags + prev_tags

    tbld = defaultdict(lambda: defaultdict(list))
    next_tbls = adp.find_all_next('table', class_='wikitable')
    prev_tbls = adp.find_all_previous('table', class_='wikitable')
    tbls = next_tbls + prev_tbls
