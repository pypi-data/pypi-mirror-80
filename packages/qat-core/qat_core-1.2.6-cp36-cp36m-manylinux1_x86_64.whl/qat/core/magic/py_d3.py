...
<g id="d3-cell-''' + str(self.max_id) + '''">
        ...
        cell = sub(
            r'd3.select\((?!this)',
            "d3.select" + str(self.max_id) + "(",
            cell
        )
        cell = sub(
            r'd3.selectAll\((?!this)',
            "d3.selectAll" + str(self.max_id) + "(",
            cell
        )
    ...
    def qatd3(self, line="", cell=None):
        ...
        if self.src is not None:
            if _src != self.src and not initialized:
                msg = "The first source of D3 used in this notebook is" \
                    + " %s. You can't use different versions of" % self.src \
