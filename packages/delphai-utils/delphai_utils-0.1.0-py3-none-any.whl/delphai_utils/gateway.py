...
class GatewayServer(threading.Thread):
  ...
  def get_http_status(grpc_code: StatusCode):
  ...
  async def request_handler(self, request):
    ...
    try:
      output = self.client.request(handler['service'], handler['method'], input)
  ...
  def run(self) -> None:
    ...
    app = Starlette(debug=True, routes=routes, exception_handlers={HTTPException: self.http_exception})
    config = Config()
