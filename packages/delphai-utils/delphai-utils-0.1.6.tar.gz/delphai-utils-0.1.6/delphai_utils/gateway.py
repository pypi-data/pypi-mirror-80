...
class AccessLogMiddleware(BaseHTTPMiddleware):
  async def dispatch(self, request, call_next):
    ...
    if response.status_code < 400:
      logging.info(f'[{response.status_code}] {request.method} {request.url}')
    return response
...
class GatewayServer(threading.Thread):
  ...
  async def request_handler(self, request):
    ...
    try:
      ...
      output = MessageToDict(raw_output, including_default_value_fields=True, preserving_proto_field_name=True)
  ...
  async def http_exception(self, request, exc):
    if 'favicon.ico' not in str(request.url):
      logging.error(f'[{exc.status_code}] {request.method} {request.url} - {exc.detail}')
