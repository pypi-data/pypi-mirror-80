...
from pydent.exceptions import log_depr_msg
...
from pydent.loggers import session_logger as logger
...
class AqSession(SessionABC):
    ...
    def __init__(
        ...
        """
        self.name: str = name  #: optional name of the session
        self._aqhttp: AqHTTP = None  #: requests interface
        ...
        self._interface_class: Union[QueryInterface, BrowserInterface] = QueryInterface
        self._initialize_interfaces()
        self._browser: Optional[Browser] = None  #: the sessions browser
        self._using_cache: bool = False
        self.init_cache()
        self.parent_session: Optional[AqSession] = (
            None  #: the parent session, if derived from another session
        )

    @property
    def log(self):
        warn(log_depr_msg, DeprecationWarning)
        return logger

    @property
    def interface_class(self) -> Union[QueryInterface, BrowserInterface]:
        """Returns the session's interface class."""
        return self._interface_class

    @interface_class.setter
    def interface_class(self, c: Union[QueryInterface, BrowserInterface]):
    ...
    def current_user(self) -> "ModelBase":
        ...
        if self._current_user is None:
            user = self.User.where({"login": self._aqhttp.user})
            if not user:
                return None
            self._current_user = self.User.where({"login": self._aqhttp.user})[0]
    ...
    @overload
    def model_interface(
        self, model_name: str, interface_class: BrowserInterface = None
    ) -> BrowserInterface:
        ...

    def model_interface(
        self, model_name: str, interface_class: Optional[QueryInterface] = None
    ) -> QueryInterface:
    ...
    def query(
        ...
        page_callback: Optional[Callable[[int, List], Any]] = None,
    ) -> List[ModelBase]:
        ...
        .. versionchanged::
            0.1.5a23 `query` key now changed to `__query__`

        .. versionchanged::
            0.1.4 Adds page_size (override) and page callback
