...
from pydent.loggers import aqhttp_logger as logger
...
class AqHTTP:
    ...
    def request(
        ...
        **kwargs,
        ...
        logger.info(self._format_response_info(response))
    ...
    def _response_to_json(self, response: requests.Response) -> dict:
        ...
        try:
            ...
            msg += "\nMessage:\n" + response.text
            logger.error(self._format_response_info(response))
            raise TridentRequestError(msg, response)
        if response_json:
    ...
    def __repr__(self):
        return "<{}(user='{}', url='{}')>".format(
            self.__class__.__name__, self.user, self.aquarium_url
        )
