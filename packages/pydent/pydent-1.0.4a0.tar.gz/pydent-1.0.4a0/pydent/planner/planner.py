...
from pydent.loggers import planning_logger as logger
...
class Planner(AFTMatcher):
    ...
    def create_operation_by_type(
        ...
        logger.debug("{} created".format(ot.name))
        return op
    ...
    def get_wire(self, fv1: FieldValue, fv2: FieldValue) -> Union[None, Wire]:
        for wire in self.plan.wires:
            if self._model_are_equal(wire.source, fv1) and self._model_are_equal(
                ...
                logger.debug("found wire from {} to {}".format(fv1.name, fv2.name))
                return wire
    ...
    def remove_wire(self, fv1: FieldValue, fv2: FieldValue) -> Wire:
        ...
        if wire:
            logger.debug("removing wire from {} to {}".format(fv1.name, fv2.name))
    ...
    def chain(
        ...
        *op_or_otnames: Tuple[UnresolvedWireTarget, ...],
        ...
        """
        logger.debug("QUICK CREATE CHAIN {}".format(op_or_otnames))
        targets = [
    ...
    def add_wire(self, fv1: FieldValue, fv2: FieldValue) -> Wire:
        ...
        if wire is None:
            ...
            logger.debug("wired {} to {}".format(fv1.name, fv2.name))
        return wire
    ...
    def get_available_items(
        ...
        if item_preference == self.ITEM_SELECTION_PREFERENCE.RESTRICT_TO_ONE_ON_SERVER:
            ...
            logger.debug("{} items are reserved".format(x - len(available_items)))
    ...
    def optimize(self, merge_missing_samples=False) -> "Planner":
        ...
        optimizer = PlanOptimizer(
            self, inherit_logger=logger, merge_missing_samples=merge_missing_samples,
        )
