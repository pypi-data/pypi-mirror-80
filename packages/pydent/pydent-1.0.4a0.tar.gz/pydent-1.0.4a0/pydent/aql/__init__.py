...
def _aql(
    ...
    returned_models = interface.where(
        new_query, opts=opts, page_size=page_size, page_callback=page_callback
    )
...
def aql(
    ...
    .. versionchanged:: 0.1.5a23
        `query` key now changed to `__query__`

    .. versionchanged:: 0.1.4
        Adds page_size (override) and page_callback
    ...
    :param page_size: Optional override for page_size in the query
    :param page_callback: Optional callback function to use upon finding each page.
    :return:
    """
    validate_aql(data)
    with session.with_cache(using_models=use_cache) as sess:
        return _aql(sess, data, page_size=page_size, page_callback=page_callback)
