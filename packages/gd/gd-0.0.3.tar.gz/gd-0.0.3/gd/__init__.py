...
class ExportJob:
    def __init__(self, token, status_uri, gd, done=False):
...
class GoodData:
    ...
    def __init__(self, user, password, domain, pid, nocache=False):
        ...
        Args:
            ...
            pid (str): project id
            nocache (bool, optional): If True, discard any possible existing cache. Defaults to False.
        """
        ...
        cache = {}
        if not self.pid in cache:
            self.metadata = {}
            self.dataset_members = {}
        else:
            self.metadata = cache[self.pid]["metadata"]
            self.dataset_members = cache[self.pid]["dataset_members"]
    ...
    def list_dashboards(self, nocache=False):
        """List all dashboards

        Returns:
            list: List of dictionaries containing dashboard metadata
        """
        if not self.dashboards or nocache:
            self.login()
    ...
    def list_kpidashboards(self, nocache=False):
        """List all KPI Dashboards

        Returns:
            list: List of dictionaries containing dashboard metadata
        """
        if not self.kpidashboards or nocache:
            self.login()
            url = _url(
                self.domain,
    ...
    def list_reports(self, nocache=False):
        """List all reports

        Returns:
            list: List of dictionaries containing report metadata
        """
        if not self.reports or nocache:
            self.login()
    ...
    def execute_afm(self, afm: dict):
        ...
        response = self.session.post(url, json=afm)
    ...
    def partial_export_async(self, object_id):
        ...
        return ExportJob(job_info["token"], job_info["status"]["uri"], self)
    ...
    ## TODO: Is there a way to monitor status? I don't have a project to test this in right now
    def partial_import_async(self, token, target_pid=None):
        """Create partial import job and return (before job completes)
        Returns True on 200 status code, False on any other
