...
if _swig_python_version_info >= (2, 7, 0):
    def swig_import_helper():
        ...
        mname = '.'.join((pkg, '_proshade')).lstrip('.')
        try:
            return importlib.import_module(mname)
        except ImportError:
            return importlib.import_module('_proshade')
    _proshade = swig_import_helper()
    ...
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_proshade', [dirname(__file__)])
        except ImportError:
            import _proshade
            return _proshade
        try:
            _mod = imp.load_module('_proshade', fp, pathname, description)
        finally:
            if fp is not None:
                fp.close()
        return _mod
...
def readInMapHeaderFroms(map, xFrom, yFrom, zFrom):
    return _proshade.readInMapHeaderFroms(map, xFrom, yFrom, zFrom)
readInMapHeaderFroms = _proshade.readInMapHeaderFroms
...
class ProSHADE_data(_object):
    ...
    def computeGroupElementsForGroup(self, settings, allCSyms, grPosition):
        return _proshade.ProSHADE_data_computeGroupElementsForGroup(self, settings, allCSyms, grPosition)

    def getGroupElementsLength(self, settings, grPosition):
        return _proshade.ProSHADE_data_getGroupElementsLength(self, settings, grPosition)

    def getGroupElementsPython(self, settings, groupElements, grPosition):
        return _proshade.ProSHADE_data_getGroupElementsPython(self, settings, groupElements, grPosition)
