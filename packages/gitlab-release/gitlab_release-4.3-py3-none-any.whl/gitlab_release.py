...
def main():
    ...
    if private_token:
        ...
        raise SystemExit("Neither PRIVATE-TOKEN nor CI_JOB_TOKEN available, must be in env var 'PRIVATE_TOKEN' or provided as arg")
    ...
    if not link_in_desc:
        data["assets"] = dict(
                links=[
                    dict(name=name, url=url) for name, url in assets
                ]
            )
