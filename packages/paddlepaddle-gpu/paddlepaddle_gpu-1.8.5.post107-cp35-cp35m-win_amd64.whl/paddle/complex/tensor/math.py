...
def trace(input, offset=0, dim1=0, dim2=1, name=None):
    ...
    real = layers.trace(input.real, offset, dim1, dim2, name)
    imag = layers.trace(input.imag, offset, dim1, dim2, name)
...
def sum(input, dim=None, keep_dim=False, name=None):
    ...
    real = layers.reduce_sum(input.real, dim=dim, keep_dim=keep_dim, name=name)
    imag = layers.reduce_sum(input.imag, dim=dim, keep_dim=keep_dim, name=name)
    return ComplexVariable(real, imag)
