...
def get_rates(metrics: dict, **kwargs):
    ...
    for metric_name, metric_params in metrics.items():
        try:
            access_repository(user=user[metric_name]["user"],
                              password=user[metric_name]["password"],
                              branch=user[metric_name]["branch"],
                              repository=metric_params[metric_name]["repository"],
                              chdir=metric_params[metric_name]["chdir"])
