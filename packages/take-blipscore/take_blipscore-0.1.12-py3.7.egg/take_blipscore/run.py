...
from take_blipscore.data_extraction.get_rates import get_rates, METRIC_ERROR
...
def transform(params: tp.Dict[str, str], rates: tp.Dict[str, float]) -> tp.Tuple[np.ndarray, np.ndarray]:
    ...
    w_list = list()
    x_list = list()
    for metric_name, metric_param in params["metrics"].items():
        x_item = rates[metric_name]
        if x_item != METRIC_ERROR:
