...
class Sensor(SensorBase):
    ...
    def definition(self) -> dict:
        ...
        http_def_group_target = SensorDefinitionGroup(
            name="targetinfo", caption="Target Information"
        )
        ...
        http_def_group_target.add_field(
            ...
            caption="POST Data",
            default="",
            help="Enter HTTP Headers as dictionary. e.g. {'my header':'my value'}. Only used when Request Type is POST",
        )
        http_def_group_target.add_field(
            field_type="edit",
    ...
    async def work(self, task_data: dict, q: asyncio.Queue) -> None:
        ...
        try:
            ...
            if not task_data["acceptable_status_codes"] == "":
                acceptable_status_codes = [
                    int(i) for i in task_data["acceptable_status_codes"].split(",")
                ]
            if not task_data["headers"] == "":
                try:
                    headers = json.loads(task_data["headers"])
                except JSONDecodeError:
                    raise
            if not task_data["post_data"] == "" and task_data["request_type"] == "POST":
                try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                response = await client.send(
                    request=request, timeout=int(task_data["timeout"])
                )

            if (
                len(acceptable_status_codes)
                and response.status_code not in acceptable_status_codes
            ):
                raise httpx.HTTPStatusError(
                    "Status Code not in Filter list", request=request, response=response
                )
            ...
            http_data.message = f"HTTP target returned {response.status_code}, Request Type: {task_data['request_type']}"

        except httpx.HTTPStatusError:
            http_data.error = "Exception"
            http_data.error_code = 1
            http_data.message = (
                f"Status Code {response.status_code} not in acceptable list"
            )
