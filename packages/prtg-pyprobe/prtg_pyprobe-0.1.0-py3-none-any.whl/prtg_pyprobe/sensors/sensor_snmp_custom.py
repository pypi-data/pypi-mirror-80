...
class Sensor(SensorSNMPBase):
    ...
    def definition(self) -> dict:
        ...
        snmp_custom_def_group_sensor_specific = SensorDefinitionGroup(
    ...
    async def work(self, task_data: dict, q: asyncio.Queue, snmp_engine: SnmpEngine):
        ...
        if task_data["snmp_version"] == "0" or task_data["snmp_version"] == "1":
            creds = CommunityData(
                task_data["community"], mpModel=int(task_data["snmp_version"])
            )
        try:
            ...
            for result in results:
                ...
                if v == "":
                    raise PySnmpError(
                        "Returned value empty. Probably no such value/object"
                    )
            ...
            snmp_custom_data.error = "Exception"
            snmp_custom_data.error_code = 1
            snmp_custom_data.message = (
                "The SNMP Query did return a not supported value type."
            )
