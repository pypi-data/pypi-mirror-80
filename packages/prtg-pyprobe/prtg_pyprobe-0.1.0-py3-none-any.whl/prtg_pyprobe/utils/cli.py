...
from prtg_pyprobe.utils.config import configuration_questions, ProbeConfig
from prtg_pyprobe.utils.defaults import *
...
def check_systemd():
    result = subprocess.run(
        ["ps", "--no-headers", "-o", "comm", "1"], capture_output=True
    )
...
def dstop():
    ...
    for process in running_procs:
        if any("bin/pyprobe" in string for string in process.cmdline()):
            process.terminate()
    print("pyprobe daemon terminated")
    ...
    for process in running_procs:
        if any("bin/pyprobe" in string for string in process.cmdline()):
            if not any("status" in string for string in process.cmdline()):
                run_since = datetime.datetime.fromtimestamp(
                    process.create_time()
                ).strftime("%Y-%m-%d %H:%M:%S")
                print(f"--- Status {process.name()} ---")
                print(f"PID: {process.pid}")
                print(f"Running as: {process.username()}")
                print(f"Running since: {run_since}")
...
def configure():
    ...
    try:
        ...
        if os.path.isfile(f"{CONFIG_PATH}{config_name}"):
            click.echo(
                "\033[93m Completing this config wizard will overwrite your current config! \033[93m"
            )
            cont = prompt(
                {
                    "type": "confirm",
        ...
        answers = prompt(configuration_questions)
    except KeyboardInterrupt:
        print("Exiting configuration wizard")
        sys.exit(0)
    if answers != {}:
        answers["probe_protocol_version"] = "1"
        answers["probe_access_key_hashed"] = hash_access_key(
            answers["probe_access_key"]
        )
        probe_cfg = ProbeConfig(path=f"{CONFIG_PATH}{config_name}")
