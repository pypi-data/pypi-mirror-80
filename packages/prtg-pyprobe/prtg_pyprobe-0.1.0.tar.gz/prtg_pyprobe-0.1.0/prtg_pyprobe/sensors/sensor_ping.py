...
class Sensor(SensorBase):
    ...
    def definition(self) -> dict:
        ...
        ping_def_group_settings = SensorDefinitionGroup(
            name="pingsettings", caption="Ping Settings"
        )
        ...
        try:
            ...
            for i in range(int(task_data["ping_count"])):
                ...
                if task_data["ping_type"] == "tcp":
                    ping_future = asyncio.open_connection(
                        host=task_data["host"], port=task_data["target_port"]
                    )
                    start = time.time()
                    reader, writer = await asyncio.wait_for(
                        ping_future, int(task_data["timeout"])
                    )
                    writer.close()
                    ...
                    if not self._is_admin():
                        raise PermissionError(
                            "The pyprobe must run as root/administrator to use ICMP Ping"
                        )
                    single_ping = await ping(
            ...
            logging.exception(
                "ICMP Ping uses RAW sockets. You must be root to use them."
            )
