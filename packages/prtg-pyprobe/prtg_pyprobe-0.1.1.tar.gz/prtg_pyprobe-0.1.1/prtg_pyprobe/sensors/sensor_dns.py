...
class Sensor(SensorBase):
    ...
    def definition(self) -> dict:
        ...
        dns_def_group_dns_spec = SensorDefinitionGroup(
            name="dns_specific", caption="DNS Specific"
        )
    ...
    async def work(self, task_data: dict, q: asyncio.Queue) -> None:
        ...
        try:
            ...
            if task_data["filter"]:
                for string in split_strings:
                    if string not in msg:
                        logging.debug(
                            f'{msg} did not contain filtered string "{string}"'
                        )
                        raise NoMatchException(
                            string_filter=string, sensor_last_message=msg
                        )
            ...
            dns_data.error_code = 1
            dns_data.message = (
                f"Filtered value '{e.string_filter}' not found in last message '{e.last_message}'."
