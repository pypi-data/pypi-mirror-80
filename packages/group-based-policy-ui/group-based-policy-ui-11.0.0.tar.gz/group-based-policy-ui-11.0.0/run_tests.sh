...
included_dirs="gbpui/openstack_dashboard gbpui/horizon"
...
function run_jshint {
  echo "Running jshint ..."
  jshint gbpui/horizon/static/horizon/js
  jshint gbpui/horizon/static/horizon/tests
}
...
function run_tests_all {
  echo "Running GBPUI application tests"
  export NOSE_XUNIT_FILE=gbpui/horizon/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='gbpui_horizon_nose_results.html'
  fi
  if [ $with_coverage -eq 1 ]; then
    ${command_wrapper} python -m coverage.__main__ erase
    coverage_run="python -m coverage.__main__ run -p"
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test gbpui/horizon --settings=horizon.test.settings $testopts
  # get results of the GBPUI tests
  HORIZON_RESULT=$?

  echo "Running openstack_dashboard tests"
  export NOSE_XUNIT_FILE=gbpui/openstack_dashboard/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='gbpui_dashboard_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test gbpui/openstack_dashboard --settings=openstack_dashboard.test.settings $testopts
  ...
  if [ $with_coverage -eq 1 ]; then
    ...
    ${command_wrapper} python -m coverage.__main__ xml -i --include="gbpui/horizon/*,openstack_dashboard/*" --omit='/usr*,setup.py,*egg*,.venv/*'
    ${command_wrapper} python -m coverage.__main__ html -i --include="gbpui/horizon/*,openstack_dashboard/*" --omit='/usr*,setup.py,*egg*,.venv/*' -d reports
  fi
...
function run_integration_tests {
  ...
  echo "Running gbpui integration tests..."
  if [ -z "$testargs" ]; then
      ${command_wrapper} nosetests gbpui/openstack_dashboard/test/integration_tests/tests
  else
...
function run_compilemessages {
  ...
  rm -vf gbpui/horizon/locale/en/LC_MESSAGES/django*.mo
  rm -vf gbpui/openstack_dashboard/locale/en/LC_MESSAGES/django.mo
