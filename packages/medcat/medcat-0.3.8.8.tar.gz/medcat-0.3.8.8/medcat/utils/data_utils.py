...
def check_differences(data_path, cat, cntx_size=30, min_acc=0.2):
    data = load_data(data_path, require_annotations=True)
    for pid, project in enumerate(data['projects']):
        ...
        for did, doc in enumerate(project['documents']):
            ...
            for id, t_ann in enumerate(t_anns_norm):
                if t_ann not in p_anns_norm:
                    # Change is it correct
                    ann = doc['annotations'][id]
                    if '_verified' not in ann:
                        ...
                        print("SNIPPET: {} <<<{}>>> {}".format(b, m, e))
                        print(ann['cui'], " | ", cat.cdb.cui2pretty_name.get(ann['cui'], ''))
                        ...
                        d = str(input("###Change to (or empty for skip): 1-Correct, 2-Incorrect: "))
