    ...
    def __iter__(self):
        ...
        for idx in self.sampler:
            try:
                ...
                # filter sample
                if self.sample_filter is None or self.sample_filter(_return_data):
                    if not isinstance(_return_data, tuple):
                        _return_data = _return_data,
                    batch_data.append(_return_data)

            except Exception as e:
                print(e)

            if len(batch_data) == self.batch_size:
                returnData = list(zip(*batch_data))
                for i in range(len(returnData)):
                    returnData[i] = np.asarray(list(returnData[i]))
