...
if get_backend()=='pytorch':
    ...
    from trident.backend.pytorch_ops import to_numpy,to_tensor,arange,shuffle,cast,clip,sqrt,int_shape,argmax,softmax,any_abnormal_number
    ...
    def on_optimization_step_start(self, training_context):
        if get_backend()=='pytorch':
            ...
            if  (training_context['current_epoch'] * training_context['total_batch'] + training_context['current_batch']) % self.batch_inteval == 0:
                ...
                grads_and_vars=[gv for gv in grads_and_vars if gv[1].trainable and tf.reduce_any(tf.not_equal(gv[0],0))]
