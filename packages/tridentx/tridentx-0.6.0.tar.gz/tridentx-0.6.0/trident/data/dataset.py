...
class Dataset(List):
    ...
    def __add__(self, other):
        if other is not None and hasattr(other, '__iter__'):
            for item in other:
                if isinstance(item, (int, float, str, np.ndarray,list)):
                    super().append(item)
            ...
            for i in range(len(corpus)):
                ...
                if  item=='\n' and  corpus[i - 1]!='\n' and len(section)>0:
                    ...
                    if i<len(corpus)-1 and corpus[i + 1] == '\n':
                        ...
                        section.append('<start/>')
                elif self.section_delimiter!='\n\n' and len(self.section_delimiter)==2 and len(section)>0and item==self.section_delimiter[0] and i<len(corpus)-1  and corpus[i-1]==self.section_delimiter[1]:
                    section.append('<end/>')
                    if i < len(corpus) - 1 and corpus[i + 1] == '\n':
                        new_corpus.append(section)
                        section=[]
