    ...
    def _initial_graph(self, inputs=None, input_shape=None,output=None,initializer=None):
        ...
        if inputs is None:
            ...
            for k, v in inputs.items():
                if is_tensor(v)or isinstance(v,np.ndarray):
                    self.inputs[k] =  list(int_shape(v))[self.batch_index+1:]
        ...
        if isinstance(output, (Layer, nn.Module)):
            ...
            if  output.built and hasattr(output,'_output_shape') and  output._output_shape is not None:
                ...
                if isinstance(out, torch.Tensor):
                    self._outputs['output'] = to_list(out.size())[self.batch_index+1:]
                    self._targets['target'] = to_list(out.size())[self.batch_index+1:]
                else:
                    for i in range(len(out)):
                        self._outputs['output_{0}'.format(i)] = to_list(out[i].size())[self.batch_index+1:]
                        self._targets['target_{0}'.format(i)] = to_list(out[i].size())[self.batch_index+1:]
            self._model.signature = get_signature(self._model.forward,'model')
            self._model.signature.inputs = copy.deepcopy(self.inputs)
            self._model.signature.outputs=copy.deepcopy(self._outputs)
            self._signature=self._model.signature
            ...
            for i in range(len(output_list)):
                self._outputs['output_{0}'.format(i)] = list(int_shape(output_list[i]))[self.batch_index+1:]
                self._targets['target_{0}'.format(i)] = list(int_shape(output_list[i]))[self.batch_index+1:]
            self._model.signature = get_signature(self._model.forward,'model')
            self._model.signature.inputs = copy.deepcopy(self.inputs)
            self._model.signature.outputs = copy.deepcopy(self._outputs)
            self._signature = self._model.signature
            ...
            self._outputs['output'] = to_list(out.size())[self.batch_index + 1:]
            self._targets['target'] = to_list(out.size())[self.batch_index + 1:]
            self._model.signature =Signature()
            self._model.signature.outputs = copy.deepcopy(self._outputs)
            self._signature = self._model.signature
        else:
            ...
            try:
                ...
                if train_data is not None:
                    ...
                    for arg in self._model.signature.inputs.key_list:
                        if  arg in data_feed and arg in available_fields:
