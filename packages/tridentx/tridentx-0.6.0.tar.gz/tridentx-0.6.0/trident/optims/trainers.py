                    ...
                    if len(trainingitem.signature.inputs) + len(trainingitem.signature.outputs) == len(
                        ...
                        if len(trainingitem.signature.outputs) == 1 and len(trainingitem.signature.inputs) == 1:
                            ...
                            if 1 <= len(data_loader.signature.outputs) <= 2:
                                ...
                                print('data_feed for {0} :{1}'.format(trainingitem.name,data_feed))  # elif len(outputs)==1
                    elif len(trainingitem.signature.inputs)  <= len(data_loader.signature.outputs) :
                        for inp in trainingitem.signature.inputs.key_list:
                            if inp in data_loader.signature.outputs.key_list:
                                data_feed[inp]=inp
                        if len(data_feed.key_list)==0:
                            for i in range(len(trainingitem.signature.inputs)):
                                data_feed[trainingitem.signature.inputs.key_list[i]] = data_loader.signature.outputs.key_list[i]
                        trainingitem.training_context['data_feed'] = data_feed
