...
from qgis.core import (QgsApplication, QgsFeature, QgsField, QgsMapLayer,
                       QgsPoint, QgsProject, QgsVectorLayer, QgsWkbTypes)
...
def add_layers(canvas: QgsMapCanvas, combo_box: QComboBox, *args: QComboBox,
               filter_: Callable[[QgsMapLayer], bool] = None,
...
def _process_divcon(line_string: List[Union[Point2D, Point3D]],
    ...
    return split_line(line_string, segments)
...
def process_string_defs(layer: QgsVectorLayer, id_field: QgsField, mesh: Mesh,
                        precision: float = 0.0, *, feedback: Feedback = None
                        ) -> Dict[str, List[int]]:
