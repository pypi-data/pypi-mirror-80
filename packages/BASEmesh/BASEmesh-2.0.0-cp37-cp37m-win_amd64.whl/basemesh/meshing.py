...
def elevation_mesh(nodes: Iterable[triangle.PSLGNode],
                   segments: Iterable[triangle.PSLGSegment] = None, *,
                   move_tempfiles: str = None, **kwargs: Any) -> Mesh:
    """Triangulate the given input vertices and break lines.

     Generate a mesh using the given vertices. No mesh quality
    contraints will be applied. The output mesh will contain elevation
    information.
    ...
    Parameters
    ----------
    nodes : Iterable[triangle.PSLGNode]
        The set of nodes to mesh
    segments : Iterable[triangle.PSLGSegment], optional
    ...
    for node in mesh.nodes:
        node.move(z=node.attributes.pop(_Z_ATTR))
    return mesh


def quality_mesh(nodes: Iterable[triangle.PSLGNode],
                 segments: Iterable[triangle.PSLGSegment] = None,
                 holes: Iterable[triangle.HoleMarker] = None,
                 regions: Iterable[triangle.RegionMarker] = None, *,
                 conforming_delaunay: bool = True,
                 preserve_boundary: bool = False,
                 min_angle: float = None,
                 max_area: float = None,
                 move_tempfiles: str = None,
                 **kwargs: Any) -> Mesh:
...
def _mesh_from_triangle(
        node_file: str, ele_file: str, node_attrs: List[str] = None,
    ...
    for pslg_ele in pslg_elements:
        ...
        # Add element
        element_nodes = (nodes[n.id_] for n in pslg_ele.nodes)
        matids = () if not use_matid else (pslg_ele.matid,)
        mesh.add_element(element_nodes, pslg_ele.id_, *matids)
    if delete_input:
        os.remove(node_file)
        os.remove(ele_file)
    return mesh
