...
class MeshElement(SupportsSpatial):
    ...
    def interpolate(self, point: Tuple[float, ...]) -> float:
        ...
        float
            ...
            element
        """
        return interpolate_triangle(point[:2], self.points)
...
class Mesh(ElevationSource):
    ...
    def add_node(self, point: Point3D, id_: int = None,
                 **kwargs: Any) -> MeshNode:
    ...
    def elements_by_polygon(self, polygon: Polygon2D,
        ...
        if check_midpoints:
            return [e for e in self.elements
                    if point_in_polygon_concave(e.center, polygon)]
    ...
    def save(self, filename: str, num_materials: int = None) -> None:
        ...
        with py2dm.Writer(filename) as mesh:
            ...
            for element in elements:
                node_ids = [transform_ids[n.id_] for n in element.nodes]
            ...
            for name, nodes in self.node_strings.items():
                node_ids = [transform_ids[n.id_] for n in nodes]
