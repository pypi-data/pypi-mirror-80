...
class TemplateMixin:
    def get_template_names(self):
        keyword = self.template_name_suffix.split("_")[-1]
        self.template_name = getattr(self.site, "%s_template_name" % keyword)
...
class BreadcrumbMixin:
    ...
    def get_breadcrumb_text(self, keyword):
        text = getattr(self.site, "breadcrumb_%s_text" % keyword)
    ...
    def get_form_breadcrumbs(self):
        ...
        breadcrumbs.append((self.get_breadcrumb_text(self.keyword), "#"))
        return breadcrumbs
    ...
    def get_detail_breadcrumbs(self):
        ...
        url_name = self.site.get_url_name(self.keyword)
        breadcrumbs = self.get_list_breadcrumbs()
        breadcrumbs.append(
            (
                self.get_breadcrumb_text(self.keyword) or str(self.object),
    ...
    def get_breadcrumbs(self):
        attr = getattr(self, "get_%s_breadcrumbs" % self.keyword)
        return attr()
