        ...
        while index <= end_index:
            ...
            if curr_ds == MeshSegment.BG_REGION_DS:
                ...
                if curr_ds <= next_ds:
                    ...
                    new_segment = MeshSegment(
                        0.5 * (segment.s0 + segment.s1), segment.s1,
                        curr_ds, segment.var_ds_ratio)
        ...
        while i < len(self.segment_list):
            ...
            if legacy_mesh_segment_list[0].s0 == segment.s0 and \
                ...
                if len(legacy_mesh_segment_list) == 1:
                    if self.update_next_segment(segment, i, legacy_segment):
                        i = - 1
                ...
                if segment.has_increasing_ds() and segment.s1 != self.upper_bnd:
                    ...
                    # Next segment
                    adjacent_segment = self.segment_list[i + 1]
                    ...
                    if not passes_mult_segment and \
                        ...
                        if adjacent_segment.s1 != self.upper_bnd and adjacent_segment.is_fixed_length() \
                            ...
                            if self.segment_list[i + 2].is_fixed_length() and \
                                    self.segment_list[i + 2].has_decreasing_ds():
                                if self.segment_list[i + 2].s1 != self.upper_bnd:
                                    ''''
                                    ...
                                    adjacent_segment.s1 = self.segment_list[i + 2].s1
                                    adjacent_segment.var_ds_ratio = segment.DEFAULT_BG_REGION_RATIO
                                    adjacent_segment.ds1 = self.segment_list[i + 2].ds1
                                    self.remove_mesh_segment(i + 2)
                                    self.resolve_gap_mid_mesh(adjacent_segment, i + 1, ds_begin=ds,
                ...
                if segment.has_decreasing_ds() and segment.s0 != self.lower_bnd:
                    ...
                    if not passes_mult_segment and \
                        ...
                        if adjacent_segment.s0 != self.lower_bnd and adjacent_segment.is_fixed_length() \
                            ...
                            if self.segment_list[i - 2].is_fixed_length() and \
                                    self.segment_list[i - 2].has_increasing_ds():
                                '''
                                    Undershoot case mid-mesh. Calculate new intersection point. 
                                    Calling resolve gap again. 
                                    ds0      ds1     ds0       ds1
                                    ...
                                    '''
                                # print("Recalculate resolve gap. ")
                                self.segment_list[i - 2].s1 = adjacent_segment.s1
                                self.segment_list[i - 2].var_ds_ratio = segment.DEFAULT_BG_REGION_RATIO
                                self.segment_list[i - 2].ds1 = adjacent_segment.ds1
                                self.remove_mesh_segment(i - 1)
                                self.resolve_gap_mid_mesh(self.segment_list[i - 2], i - 2,
                                                          ds_begin=self.segment_list[i - 2].ds0, ds_end=ds)
