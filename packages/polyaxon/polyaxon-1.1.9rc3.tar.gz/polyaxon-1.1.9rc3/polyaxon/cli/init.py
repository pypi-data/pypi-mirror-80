...
@click.option(
    ...
    default=False,
    show_default=False,
    help="Purge the local cache and all current configs.",
)
def init(project, git_connection, git_url, polyaxonfile, polyaxonignore, purge):
    """Initialize a new local project and cache directory."""
    if not any([project, git_connection, git_url, polyaxonfile, polyaxonignore, purge]):
    ...
    if purge:
        ...
        Printer.print_success("Local Polyaxon configurations cleaned.")
