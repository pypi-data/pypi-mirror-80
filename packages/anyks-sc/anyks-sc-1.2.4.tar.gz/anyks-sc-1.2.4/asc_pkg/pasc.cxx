		...
		void saveIndex(const string & filename, const string & password = "", const u_short aes = 128, function <void (const wstring &, const u_short)> status = nullptr) noexcept {
			...
			if(!filename.empty()){
				...
				if((vocabInfo.first > 0) && (vocabInfo.second > 0)){
					...
					(almV2 ? alm2->setSize(toolkit.getSize()) : alm1->setSize(toolkit.getSize()));
					// Выполняем извлечение языковой модели
					toolkit.saveArpa([&](const vector <char> & buffer, const u_short rate){
						// Устанавливаем данные языковой модели
						(almV2 ? alm2->setBin(buffer) : alm1->setBin(buffer));
						// Выводим индикатор загрузки
						if(status != nullptr) status(L"Read language model", rate);
					}, true);
