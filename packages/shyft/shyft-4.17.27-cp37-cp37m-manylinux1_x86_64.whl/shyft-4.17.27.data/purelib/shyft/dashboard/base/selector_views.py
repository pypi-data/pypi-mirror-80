...
class FilterMultiSelect(SelectorViewBase):
    ...
    def __init__(self,
        ...
        if not filter_after_enter_is_pressed:
            if version.parse(bokeh.__version__) >= version.parse('1.3.0'):
                value = 'value_input'
            else:
                self.logger.warning(f"FilterMultiSelect: cannot set `filter_after_enter_is_pressed=False` rquiresbokeh version >= 1.3.0, current bokeh version: {bokeh.__version__}")
                value = 'value'
...
class AutocompleteInput(BokehViewBase):
    ...
    def __init__(self,
                 *,
                 ...
                 show_state_title: bool = True,
                 **bkwargs) -> None:
        bkwargs.update(dict(placeholder=placeholder))
        ...
        self._bokeh_view.on_change('value', self._on_change_select)
        self._update_select = self.update_value_factory(self._bokeh_view, 'value')
        self._bokeh_view.placeholder = placeholder
...
class GroupBokehViewBase(BokehViewBase):
    ...
    def __init__(self, *,
        ...
        text_height = text_height or constants.text_height
        layout_width = calculate_layout_width(width, padding)
        self._title_div = bokeh.models.Div(text=title, height=text_height)
