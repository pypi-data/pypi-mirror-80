...
class AbstractPropertiesManagerTest(unittest.TestCase):
    ...
    def test_is_connector_modified_exceptions(self):
        pm = ControlPropertyManager('test_abstract_properties', root_keys=[], knowledge_keys=[])
        with self.assertRaises(ModuleNotFoundError) as context:
            pm.is_connector_modified('control')
        self.assertTrue("The connector 'control' has not been set" in str(context.exception))
        pm.set_connector_contract(connector_name='control', connector_contract=self.connector)
        with self.assertRaises(FileExistsError) as context:
            pm.is_connector_modified('control')
        self.assertTrue("The connector 'control' sourcing file does not exist" in str(context.exception))
    ...
    def test_file_pattern(self):
        ...
        result = pm.file_pattern(connector_name='source', name_suffix="_special")
        self.assertEqual('hadron_control_task_source_special.pickle', result)
