...
class AbstractPropertyManager(ABC):
    ...
    def is_connector_modified(self, connector_name) -> bool:
        ...
        if not self.has_connector(connector_name):
            raise ModuleNotFoundError(f"The connector '{connector_name}' has not been set")
        if not self.get_connector_handler(connector_name).exists():
            raise FileExistsError(f"The connector '{connector_name}' sourcing file does not exist")
    ...
    def file_pattern(self, connector_name: str, path: [str, list]=None, prefix: str=None, name_suffix: str=None,
                     suffix: str=None, file_type: str=None, versioned: bool=None, stamped: str=None) -> str:
        ...
        :param name_suffix: (optional) a suffix to put at the end of name portion of the file before the file_type
        ...
        name_suffix = name_suffix if isinstance(name_suffix, str) else ''
        file_type = file_type if isinstance(file_type, str) else 'pickle'
        _pattern = "_".join(["hadron", self.manager_name(), self.task_name, connector_name])
        _pattern = f"{prefix}{_pattern}{name_suffix}"
