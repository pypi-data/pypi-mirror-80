...
Max fit RIPTiDe tests all minimum objective flux fractions over the provided range and returns only the model with the best Spearman correlation between context-specific flux for reactions and the associated transcriptomic values. Search is terminated is a subsequent iteration has a lower correlation coefficient than the last, however this could be a local maxima and must be considered if an exhaustive analysis is preferred.
