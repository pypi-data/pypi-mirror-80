        ...
        if intent not in ner_intents:
            ...
            for e in entities:
                if e['extractor'] in ['DucklingHTTPExtractor', 'DucklingExtractor']:
                    ...
                    if e['entity'] == 'B-account':
                        ...
                        if xNormalizer.tr_normalize(e['value']).lower().strip() in ['bakiye']:
                            continue
            ...
            for e in entities:
                if e['extractor'] in ['DucklingHTTPExtractor', 'DucklingExtractor']:
                    ...
                    if e['entity'] in ['B-account', 'B-cardtype', 'B-CARDTYPE']:
                        ...
                        if xNormalizer.tr_normalize(e['value']).lower().strip() in [
                            'kredi', 
                            'kart', 'kartim', 'kartımın', 'kartlarımı', 'kartimin', 'kartlarimi', 'kartlarim',
                            'kredi kart', 'kredi kartı', 'kredi kartimda', 'kredi karti', 'kredi kartim',
                            'vadeli', 'vadesiz']:
                            continue
            ...
            for e in entities:
                if e['extractor'] in ['DucklingHTTPExtractor', 'DucklingExtractor']:
                    ...
                    if e['entity'] in ['B-account', 'B-cardtype', 'B-CARDTYPE']:
                        ...
                        if xNormalizer.tr_normalize(e['value']).lower().strip() in ['kredi', 'kart', 'kredi kart', 'vadeli', 'vadesiz', 'maas']:
                            continue
                        output['_CARD_TYPE_TR_'].append(e['value'])
                        checks['_CARD_TYPE_TR_'] = True
            ...
            for e in entities:
                if e['extractor'] ==  'RegexEntityExtractor':
                    ...
                    if not checks['_ACCOUNT_TR_'] and e['entity'] == 'account':
                        output['_ACCOUNT_TR_'].append(e['value'])
