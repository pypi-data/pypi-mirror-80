...
class DucklingExtractor(EntityExtractor):
    ...
    def _url(self) -> Optional[Text]:
        ...
        logging.error("Duckling url selection.")
        # if os.environ.get("SOFIE_DUCKLING_URL"):
        logging.error(os.environ)
        if os.getenv('SOFIE_DUCKLING_URL'):
            logging.error("SOFIE_DUCKLING_URL exists")
            url = os.environ["SOFIE_DUCKLING_URL"]
            logging.error("INPUT URL")
            logging.error(url)
            if url[:7] != 'http://' and url[:8] != 'https://':
                url = 'http://' + url
            logging.error("UPDATED INPUT URL")
            logging.error(url)
            return url
    ...
    def _duckling_parse(self, text: Text, reference_time: int) -> List[Dict[Text, Any]]:
        ...
        try:
            ...
            logging.error("RESPONSE FROM API")
