...
#  the action is either 'update' or 'select':
    ...
    if args.action == 'update':
       ...
       #and not args.select:
        ...
        #print("Call with field=%s use_index=%s index=%s value=%s" % (field, use_index, index, value))


        ##if args.from_yml:
            #if field =

                    #print("Read from config: obj_field=%s type=%s" % (obj_field, type(obj)))
    ...
    #for k,v in scnl_filter.__dict__.items():
        #print("%s : %s" % (k,v))
