...
class Opt:
    ...
    @staticmethod
    def ogr_geom_type(input,
                      format='num'):
        """
        Method to return OGR geometry type from input string
        :param input: String to convert to OGR geometry type code
        :param format: Which format should be output? (default: 'num'; options: 'num', 'name')
        :return: OGR geometry type code
        """

        if format == 'num':
            comp_str = input.lower()
            comp_dict = {
                'point': 1,
                'line': 2,
                'polygon': 3,
