...
def tryfunc(func, args, on_error=None):
    try:
        return func(*args)
    except:
        return on_error
...
def now(date_too=True):
    'return current time (format = "YYYY-mm-dd HH:MM:SS" if dat else "HH:MM:SS"'
...
def minmon(argv):
    ...
    while True:
        ...
        m = mem().percent
        c = cpu()
        t = tryfunc(max, ([x.current for xx in temp().values() for x in xx],), 0)
