...
class BaseAI(object):
    def __init__(self, token=None):
        self._token = token
        self._url = "https://dobotlab.dev.ganguomob.com/api/ai/tencent-transmit"
    ...
    def _format(self, data):
        try:
            status = json.loads(data.content.decode())['status']
            if status == 'error':
                loggers.get('DobotEDU').error(data.content.decode())
            else:
                d = json.loads(data.content.decode())['data']
                f_data = json.loads(d)
...
class Speech(BaseAI):
    ...
    def asr(self, voice):
        ...
        r = requests.post(self._url,
                          headers=self._headers,
                          data=json.dumps(payload))
        asr_data = self._format(r)["Result"]
        return asr_data
    ...
    def synthesis(self, text, language):
        ...
        r = requests.post(self._url,
                          headers=self._headers,
                          data=json.dumps(payload))
        res = self._format(r)['Audio']
        voice_data = base64.b64decode(res)
        return voice_data
...
class Ocr(BaseAI):
    ...
    def basic_general(self, image):
        ...
        res = self._format(r)
        ocr_data = ""
        for i in range(len(res["TextDetections"])):
            ocr_data = ocr_data + res["TextDetections"][i][
                'DetectedText'] + "\n"
        return ocr_data
...
class Nlp(BaseAI):
    ...
    def topic(self, text):
        ...
        r = requests.post(self._url,
                          headers=self._headers,
                          data=json.dumps(payload))
        data = self._format(r)['Classes'][0]['FirstClassName']
        return data
...
class Tmt(BaseAI):
    ...
    def translation(self, source_text, source, target):
        ...
        r = requests.post(self._url,
                          headers=self._headers,
                          data=json.dumps(payload))
        data = self._format(r)["TargetText"]
        return data
