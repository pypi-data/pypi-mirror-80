...
class SHCAPI:
    ...
    def set_temperature(self, device, celsius: float):
                ...
                "id": [
                    {
                        "data": device.data,
                        "sIT600TH": {
                            "SetHeatingSetpoint_x100": int((round(celsius * 2) / 2) * 100),
                            "SetHoldType": 2
                        }
                    }
                ]
    ...
    def set_climate_device_mode(self, device, mode: str):
        api_url = f"{self._api_root}/write"
        data = {
                "requestAttr": "write",
                "id": [
                    {
                        ...
                        "sIT600TH": {
                            "SetHoldType":
                                7 if mode == HVAC_MODE_OFF
                                else 2 if device.previous_mode == PRESET_PERMANENT_HOLD
                                else 7 if device.previous_mode == PRESET_OFF
                                else 0
                        }
                    }
                ]
    ...
    def set_climate_device_preset(self, device, preset: str):
        api_url = f"{self._api_root}/write"
        data = {
                "requestAttr": "write",
                "id": [
