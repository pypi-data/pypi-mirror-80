...
class MultiTaskIntent_Model(MultiInputBaseModel):
    ...
    def __init__(self, embeddings=None, sequence_length=None, hyper_parameters=None,
        ...
        if embeddings is None:
            embeddings=[]
            self.embedding_layer = DefaultEmbedding(embedding_size=self.hyper_parameters['word_embedding_size'], name='word_input')
            self.char_embedding_layer = DefaultEmbedding(embedding_size=self.hyper_parameters['char_embedding_size'], input_length=self.hyper_parameters['word_length'], name='char_input')
    ...
    def build_model_arc(self):
        ...
        char_embeddings = tf.keras.layers.TimeDistributed(tf.keras.layers.Bidirectional(L.LSTM(self.hyper_parameters['char_lstm_dim']),name='bilistm_1'), name='time_distrinuted')(char_embeddings)
        ...
        second_bilstm_layer = tf.keras.layers.Bidirectional(L.LSTM(self.hyper_parameters['tagger_lstm_dims'], return_sequences=True), name='bidirectional')(combined_features)
        ...
        self.tf_model = tf.keras.Model(inputs=[embeddings[0].input_tensor, embeddings[1].input_tensor], outputs=[intents, labels])
