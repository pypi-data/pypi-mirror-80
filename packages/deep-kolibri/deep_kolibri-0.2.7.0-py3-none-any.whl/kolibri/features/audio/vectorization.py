...
def vectorize_raw(audio, vectorizer='mfcc', sr=16000, window_samples=1600,hop_samples=800, num_filt=20, fft_size=512, num_coeffs=13):
    """Turns audio into feature vectors, without clipping for length"""
    if len(audio) == 0:
        raise Exception('Cannot vectorize empty audio!')
    if vectorizer=='mel':
        return extract_mel_fearures(audio, sr=sr, window_samples=window_samples,hop_samples=hop_samples, num_filt=num_filt, fft_size=fft_size)
    else:
        return extract_mfcc_fearures(audio, sr=sr, window_samples=window_samples,hop_samples=hop_samples, num_filt=num_filt, fft_size=fft_size, num_coeffs=num_coeffs)
...
def vectorize(audio,  vectorizer='mfcc', max_samples=24000, n_features=29,sr=16000,window_samples=1600, hop_samples=800, num_filt=20, fft_size=512, num_coeffs=13):
    """
    ...
    features = vectorize_raw(audio, vectorizer=vectorizer, sr=sr, window_samples=window_samples,hop_samples=hop_samples, num_filt=num_filt, fft_size=fft_size, num_coeffs=num_coeffs)
