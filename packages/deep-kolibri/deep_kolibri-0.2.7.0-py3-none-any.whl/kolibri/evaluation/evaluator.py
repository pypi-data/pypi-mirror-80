...
from . import plot
from . import metrics
from sklearn.metrics import accuracy_score, f1_score, recall_score, jaccard_score, balanced_accuracy_score, log_loss
...
class ClassifierEvaluator(object):
    ...
    def __init__(self, estimator, X, y, labels=None, folds=4, ratio_to_use=1, n_jobs=4, learning_curve=False):
        ...
        if self.compute_learning_curve:
            ...
            self.train_sizes, self.train_scores, self.test_scores = sk_learning_curve(estimator, X, y, cv=folds, n_jobs=n_jobs, train_sizes=train_sizes)
        self._y_pred=self.cross_validate_predict(estimator, X, y, folds=folds, ratio_to_use=ratio_to_use, n_jobs=n_jobs)
    ...
    def confusion_matrix(self, y_true=None, y_pred=None, target_names=None):
        ...
        if y_true is not None and y_pred is not None:
            ...
            cm = pd.DataFrame(metrics.confusion_matrix(self.y_true, self.y_pred, self.target_names))
    ...
    def cross_validate_predict(self, classifier, X, y, folds=4, ratio_to_use=1, n_jobs=4):
        ...
        try:
            return cross_val_predict(classifier, X, y, cv=cv, n_jobs=n_jobs, method='predict_proba')
        except:
    ...
    @classmethod
    def get_performance_report(cls, y_true=None, y_pred=None, target_names=None):
        report={}

        report['confusion_matrix']=cls.confusion_matrix(cls, y_true, y_pred, target_names).to_dict()
        cr=cls.classification_report(cls, y_true, y_pred, target_names)
        ...
        report['recall_score_weighted']=recall_score(y_true=y_true, y_pred=y_pred, labels=target_names, average='weighted')
        report['jaccard_score_micro']= jaccard_score(y_true=y_true, y_pred=y_pred, labels=target_names, average='micro')
        report['jaccard_score_macro'] = jaccard_score(y_true=y_true, y_pred=y_pred, labels=target_names, average='macro')
        report['jaccard_score_weighted'] = jaccard_score(y_true=y_true, y_pred=y_pred, labels=target_names, average='weighted')
        report['balanced_accuracy_score']=balanced_accuracy_score(y_true=y_true, y_pred=y_pred)
        report['adjusted_balanced_accuracy_score']=balanced_accuracy_score(y_true=y_true, y_pred=y_pred, adjusted=True)
