...
class hyper_ellipse():
    """ Finds a hyper ellispe which bounds the provided data, and inflates it
    ...
    def fit_hyper_ellipse(self, data=None):
        ...
        the Uncertainty quantifcation of Sliced Normal Distributions by Scaling
    ...
    def he_obj_fun(self, q):
        ...
        return -(
            2*np.sum(
                np.log(np.diag(np.linalg.cholesky(nearest_psd(q[1:, 1:]))))
                )
            )+self.f_beta(q)
    ...
    def f_beta(self, q):
        ...
        beta = np.min(
            np.sum(
                -np.column_stack([np.ones(np.shape(self.data)[0]), self.data]).T
                *(
                    q@np.column_stack(
                        [np.ones(np.shape(self.data)[0]), self.data]
                        ).T
                )/2, 0)
            )
        return beta
