...
Steps='''Steps to Get Android Activity File :
...
def creategDataDF(File=''):
    ...
    global Steps, ReportName
   
    def unzip(file):
        DefaultFileLoc='/Takeout/My Activity/Android/' 
        extractDir=file.split('.zip')[0]

        if not os.path.exists(extractDir):
            with zipfile.ZipFile(file,"r") as zip_ref:
                zip_ref.extractall(extractDir)
        return [extractDir+DefaultFileLoc+File for File in os.listdir(extractDir+DefaultFileLoc)][0]
    ...
    if File:
        if '.json' in File.lower(): # If Default : json file
            ...
            File = (File if '/' is File[-1] else File+'/').replace(os.sep,'/')
            ...
            ActivityFile=unzip(latestfiles) 
    else:
        ...
        if RecentTakeoutFile:
            ...
            ActivityFile=unzip(RecentTakeoutFile)
        else:
...
def appUsageData(topApps, AppUsage, Yearly=0, All=0): # Works on AppUsage DataFrame
    ...
    if not Yearly and not All:
        print('\n\n   -- Generating App-Usage Report ..',end='')  #  else print(' .',end='')
    elif Yearly and not All:
            print('\n      - Year : '+str(AppUsage['Time'].iloc[-1].year)+'-'+str(AppUsage['Time'].iloc[0].year)+' ', end='')
    ...
    for n in range(len(ApUsgMthChange)-1):
        ...
        for index,appName in enumerate(TopAPPS):
            ...
            elif Yearly and not n%5:
                    print('.', end='') 
            elif All and not n%3:
                    print('.', end='')
...
def scatterplot(xsleeptime,ywakeuptime,xdata, Title, plotName, yearly=0):
    from matplotlib import pyplot as plt
    import matplotlib.transforms as transforms
    import matplotlib.font_manager as Fontmanager
    
    import math
    global IdlSleepT, showMarkerday, ReportName
...
def addTableHTML(TableData,TableHeading='', HeadingFmt=''):
    ...
    if isinstance(TableData,dict): # If DataFrame is in a Dictionary
        ...
        TableHeading = HeadingFmt.replace('_TITLE_',TableHeading) if '_TITLE_' in HeadingFmt else TableHeading
...
def generateAppUsageHTMLTabs(AppAllTimeUsage, Plots):
    global TopAppsN, ReportName, htmlTABcss, htmlTABjs
    HTML=''
    Title=str(TopAppsN)+' Top-Apps Average Usage during '+list(AppAllTimeUsage.keys())[0]
    
    HTML+=addTableHTML(AppAllTimeUsage[list(AppAllTimeUsage.keys())[0]],TableHeading=Title)+'<br><br>' # All Years
    htmlTAB='<div class="tab">'
    for i,Yr in enumerate(Plots):
        htmlTAB+='''<button class="tablinks" onclick='openYear(event, "'''+Yr+'''")' '''+('' if i else 'id="defaultOpen"')+'''  >'''+Yr+'''</button>'''
    HTML+=htmlTAB+'</div>'+htmlTABcss


    for Yr in Plots:
        HTML+='<div id="'+Yr+'" class="tabcontent">'+addTableHTML(AppAllTimeUsage[int(Yr)],TableHeading='In '+Yr)
        HTML+=addImageColumnHTML(Plots[Yr], plotpath='App-Usage'+'/', title='Average App Usage ')+'</div>'
    HTML+=htmlTABjs+'</body></html>'  # Add JS at the end 
...
def generateSleepReportHTML(SleepYearlyMeanDF,SleepMonthlyMeanDF, barplots, scatterplots):
    ...
    monthPlots={}
    for pltYr in barplots['monthly']: # Create dictionary to reverse month plots for each year
    ...
    # Sleep Routine
    monthPlots={}
    for pltYr in scatterplots['monthly']:
       monthPlots.setdefault(pltYr.split('/')[0],[]) ; monthPlots[pltYr.split('/')[0]].append(pltYr)
    for yr in monthPlots:
...
def generateSleepReportHTMLtabs(SleepYearlyMeanDF,SleepMonthlyMeanDF, barplots, scatterplots):
    global htmlCSS, htmlTABcss, htmlTABjs, ReportName
    FilePath='Sleep-Routine'+'/'
    ...
    Period=barplots['yearly'][-1].split('/')[0]+'-'+barplots['yearly'][0].split('/')[0]
    HTML+=addTableHTML(SleepYearlyMeanDF,TableHeading='Sleep Taken during '+Period)


    ### Yearly Plots :
    HTML+=addDesc('Sleep Time and Sleep Routine during '+Period)+'''<div class="columnpadding"></div>'''
    monthPlots={}; scmonthPlots={}
    for pltYr in barplots['monthly']:
       monthPlots.setdefault(pltYr.split('/')[0],[]) ; monthPlots[pltYr.split('/')[0]].append(pltYr)

    for pltYr in scatterplots['monthly']: # Create dictionary to reverse month plots for each year
       scmonthPlots.setdefault(pltYr.split('/')[0],[]) ; scmonthPlots[pltYr.split('/')[0]].append(pltYr)       
       
    htmlTAB='<div class="tab">'
    for i,Yr in enumerate(monthPlots):
      #if not i:
        htmlTAB+='''<button class="tablinks" onclick='openYear(event, "'''+Yr+'''")' '''+('' if i else 'id="defaultOpen"')+'''  >'''+Yr+'''</button>'''
    htmlTAB+='</div></div>' # End First Row and Column
    HTML+=htmlTAB+htmlTABcss

    for i,Yr in enumerate(monthPlots):
      #if not i:
        HTML+='<div id="'+Yr+'" class="tabcontent"><div class="row">' +addTableHTML(SleepMonthlyMeanDF[Yr], TableHeading='In '+Yr)+'<div class="columnpadding"></div><div class="imgcolumn">'
        ## Yearly Plots
        HTML+='''<div class="gallery">
                <img src="'''+getb64Image(FilePath+barplots['yearly'][i])+'''"  alt="Sleep Time in '''+barplots['yearly'][i]+'''" width="1200" height="800"></div>'''

        HTML+='''<div class="gallery">
                <img src="'''+getb64Image(FilePath+scatterplots['yearly'][i])+'''"  alt="Sleep Time in '''+scatterplots['yearly'][i]+'''" width="1200" height="800"></div>'''
        ...
        for plot in monthPlots[Yr][::-1]:
            HTML+='''<div class="gallery">
                 <img src="'''+getb64Image(FilePath+plot)+'''"  alt="Sleep Time in '''+plot+'''" width="1200" height="800"></div>'''
        HTML+='</div></div>' # End  Previous row and Image Column

        # Sleep Routine
        HTML+=addDesc('Sleep Routine in '+Yr) 
        HTML+='<div class="row">'+'<div class="imgcol2">' # New Row for Each Year
        for plot in scmonthPlots[Yr][::-1][::2]:
            HTML+='''<div class="gallery">
                 <img src="'''+getb64Image(FilePath+plot)+'''"  alt="Sleep Routine in '''+plot+'''" width="1200" height="800"></div>''' 
        HTML+='</div> <div class="imgcol2">' # End First Column & Add Second Column
        
        for plot in scmonthPlots[Yr][::-1][1::2]:
            HTML+='''<div class="gallery">
                 <img src="'''+getb64Image(FilePath+plot)+'''"  alt="Sleep Routine in '''+plot+'''" width="1200" height="800"></div>'''         
        HTML+='</div></div></div>' # Close Last row, Img column and current tab
    HTML+=htmlTABjs+'</body></html>'  # Add JS at the end 
...
######### Main Program ########
def androidReport(file='MyActivity.json', apps=12, timezone='in', excludeapps=['com.miui.home' ], idealsleeptime=8, inlineimg=1, showmarkerday=0, output=1, yeartabs=1 ,verbose=0):
    '''
    ...
    Example : By Default, Looks for the Latest Takeout Zip in the current working directory
    ...
    sleepReport = generateSleepReportHTMLtabs(SleepYearlyMeanDF, SleepMonthlyMeanDF, barplots, scatterplots) if yeartabs else generateSleepReportHTML(SleepYearlyMeanDF, SleepMonthlyMeanDF, barplots, scatterplots) # Generates Sleep Report HTML File
    '''
    ...
    AppUsageReport = generateAppUsageHTMLTabs(AppAllTimeUsage, Plots) if yeartabs else generateAppUsageHTML(AppAllTimeUsage, Plots)
...
if __name__ == '__main__' :
    reports = androidReport(file='E:/PY/activity/datafile')
