...
class Sample(PathExAttMap):
    ...
    def to_yaml(self, path):
        ...
        def _obj2dict(obj, name=None, to_skip=(SUBSAMPLE_DF_KEY, SAMPLE_DF_KEY)):
            ...
            if name == PRJ_REF:
                ...
                return {k: _obj2dict(v, name=k) for k, v in list(prj_data.items())}
            if isinstance(obj, list):
                return [_obj2dict(i) for i in obj]
            if isinstance(obj, AttMap):
                return {k: _obj2dict(v, name=k) for k, v in list(obj.items())
    ...
    def derive_attribute(self, data_sources, attr_name):
        ...
        def _format_regex(regex, items):
            ...
            attr_lens = [len(v) for k, v in list(items.items())
        ...
        try:
            vals = _format_regex(regex, dict(list(self.items())))
    ...
    def __reduce__(self):
        return (
            ...
            (None, {}),
            iter([]),
            iter(list({PRJ_REF: self[PRJ_REF]}.items()))
        )
    ...
    def __str__(self, max_attr=10):
        ...
        pub_attrs = {k: v for k, v in list(self.items()) if not k.startswith("_")}
        maxlen = max(list(map(len, list(pub_attrs.keys())))) + 2
        attrs = ""
        counter = 0
        for k, v in list(pub_attrs.items()):
            attrs += "\n{}{}".\
