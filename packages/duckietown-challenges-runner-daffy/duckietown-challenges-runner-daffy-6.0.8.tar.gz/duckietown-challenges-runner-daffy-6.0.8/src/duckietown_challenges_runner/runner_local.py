...
from duckietown_challenges import CHALLENGE_PREVIOUS_STEPS_DIR, ChallengeResults, traceback
            ...
            if os.path.exists(wd_final) and os.path.exists(params):
                if open(params).read() == evaluation_parameters_str:
                    ...
                    if cr.status == SUCCESS:
                        logger.info(
                            'Not redoing step "%s" because it is already completed.' % challenge_step_name
                        )
