...
class Queue:
    ...
    def __init__(self, host='0.0.0.0', port=6379, db=0, module=None):
        ...
        self.queue = RQ(connection=connection.redis)
        self.module = module
    ...
    def _retry_on_failure(self, job, *exc_info):
        ...
        The retry() decorator in src.ext.task.decorators is provided to flag
        any method as retry-able.
    ...
    def cancel_job(self, job):
        ...
        cancel_job(job.id, connection=connection.redis)
        job.delete()

    def cancel_jobs(self, job_ids):
        """Cancel of bunch of jobs given their IDs."""
        for job_id in job_ids:
            self.cancel_job(self.get_job(job_id))
