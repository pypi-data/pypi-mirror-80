...
class CachedResponseBody(Base, AutoName):
    ...
    """
    region_id = Column(col_t.SmallInteger, ForeignKey('cache_region.id'), primary_key=True)
    region = sqlalchemy.orm.relationship('CacheRegion')
...
class CachedRequest(Base, AutoName):
    ...
    ForeignKeyConstraint(
        ...
        ('cached_response_body.region_id',
         'cached_response_body.scrape_timestamp',
         'cached_response_body.request_fingerprint')
    )
...
class CachedResponseMeta(Base, AutoName):
    ...
    content_extraction_method_id = Column(
        col_t.SmallInteger,
        ForeignKey('content_extraction_method.id'),
        nullable=True
    )
    ...
    response_body_text_number_of_bytes = Column(col_t.Integer, nullable=False)
