...
class SNScontent:
        ...
        if type(content) == str:
            ...
            try:
                ...
                for content_parameter in content_parameters:
                    ...
                    if param_type not in _interaction_content:
                        _interaction_content[param_type] = []

                    _interaction_content[param_type].append(param_value)
    ...
    def __str__(self):
        return '\r\n\r\n'.join(['\r\n'.join([([key + ": " + e2 for e2 in value]).join('\r\n') for key, value in e1.items()]) for e1 in self.content]) 