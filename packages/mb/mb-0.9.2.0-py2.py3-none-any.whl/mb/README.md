...
**Online Media Metadata (OMM)** is a succinct online media referencing syntax. The media unit is **mediabyte**. Think MIDI for online media.
...
![OMM syntax highlight example](http://v1d.dk/omm/sunday_mixtape.png)
...
Check out the OMM [documentation](https://github.com/taext/mediabyte/blob/master/user_guide/README.md) for details and examples.
...
General interface `omm()` or `input()` to parse any mediabyte string, Mixtape file or Mixtape link.
...
**Yota** and **Mixtape** keyword search using YouTube auto-generated subtitles. (pro version)
...
`mediabyte.hash` referencing: `o.diu5ir4jkk3` (full hash) or `input('o.diu')` (uniquely match).
