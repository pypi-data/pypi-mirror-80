# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 10:40:23 2017
...
class MeasurementScore():
    #Score for the different measurements (e.g. FRET, PIFE, quenching)
    file_tag = None
    measurement_score = {}
...
#            self.identifier = identifier
            ...
            self.structure[tag],self.model[tag] = pdbhelper.load_pdb(identifier,file_path)
        else:
            raise ValueError("tag must be 'apo' or 'holo'") 
    ...
    def save_csv(self,tag='short'):
        ...
        if tag=='short':
            ...
            filepath = os.path.join(os.path.dirname(self.file_path), identifiers + r'_' + self.file_tag + '.csv')
        else:
            data = self.measurement_score_long
            filepath = os.path.join(os.path.dirname(self.file_path), identifiers + r'_' + self.file_tag + 'long.csv')
