...
class Points:
    def __init__(points, positions, color, size):
        points.points = [list(point) for point in positions]
        points.color = color
        points.size = size
    ...
    def draw(points):
        arcade.draw_points(points.points, points.color, points.size)
        
    def collide(points, other):
        return collision(points.boundary(), other.boundary())
        
    def boundary(points):
        return {'left': points.left, 'top': points.top, 'right': points.right, 'bottom': points.bottom, 'type': 'rect'}
    
    def angle(points, other):
        return Angle(points.x, points.y, other.x, other.y)

    def distance(points, other):
        return Distance(points.x, points.y, other.x, other.y) 
    
    def getx(points):
        return find_center_x_of_points(points.points)
    def setx(points, value):
        difference = value - find_center_x_of_points(points.points)
        for point in points.points:
            point[0] += difference
    x = property(getx, setx)
            
    def gety(points):
        return find_center_y_of_points(points.points)
    def sety(points, value):
        difference = value - find_center_y_of_points(points.points)
        for point in points.points:
            point[1] += difference
    y = property(gety, sety)
    
    def getwidth(points):
        return points.right - points.left
    def setwidth(points, value):
        if not value == points.right - points.left:
            center_x = find_center_x_of_points(points.points)
            x_scale = value / (points.right - points.left)
            scale_x(points.points, center_x, x_scale)
    width = property(getwidth, setwidth)
            
    def getheight(points):
        return points.top - points.bottom
    def setheight(points, value):
        if not value == points.top - points.bottom:
            center_y = find_center_y_of_points(points.points)
            y_scale = value / (points.top - points.bottom)
            scale_y(points.points, center_y, y_scale)
    height = property(getheight, setheight)
                  
    def getrotation(points):
        return points._rotation
    def setrotation(points, value):
        if not value == points._rotation:
            center_x = find_center_x_of_points(points.points)
            center_y = find_center_y_of_points(points.points)
            rotate(points.points, value - points._rotation, center_x, center_y)
            points._rotation = value
    rotation = property(getrotation, setrotation)
    
    def gettop(points):
        return find_top_of_points(points.points) + points.size / 2
    def settop(points, value):
        distance = value - (find_top_of_points(points.points) + points.size / 2)
        for point in points.points:
            point[1] += distance
    top = property(gettop, settop)
    
    def getbottom(points):
        return find_bottom_of_points(points.points) - points.size / 2
    def setbottom(points, value):
        distance = value - (find_bottom_of_points(points.points) - points.size / 2)
        for point in points.points:
            point[1] += distance
    bottom = property(getbottom, setbottom)
    
    def getleft(points):
        return find_left_of_points(points.points) - points.size / 2
    def setleft(points, value):
        distance = value - (find_left_of_points(points.points) - points.size / 2)
        for point in points.points:
            point[0] += distance
    left = property(getleft, setleft)
    
    def getright(points):
        return find_right_of_points(points.points) + points.size / 2
    def setright(points, value):
        distance = value - (find_right_of_points(points.points) + points.size / 2)
        for point in points.points:
            point[0] += distance
    right = property(getright, setright)
...
class Lines:
    def __init__(lines, points, color, thickness):
        lines.points = [list(point) for point in points]
        lines.color = color
    ...
    def draw(lines):
        arcade.draw_lines(lines.points, lines.color, lines.thickness)
        
    def collide(lines, other):
        return collision(lines.boundary(), other.boundary())
        
    def boundary(lines):
        return {'left': lines.left, 'top': lines.top, 'right': lines.right, 'bottom': lines.bottom, 'type': 'rect'}
    
    def angle(lines, other):
        return Angle(lines.x, lines.y, other.x, other.y)

    def distance(lines, other):
        return Distance(lines.x, lines.y, other.x, other.y)
    
    def getx(lines):
        return find_center_x_of_points(lines.points)
    def setx(lines, value):
        difference = value - find_center_x_of_points(lines.points)
        for point in lines.points:
            point[0] += difference
    x = property(getx, setx)

    def gety(lines):
        return find_center_y_of_points(lines.points)
    def sety(lines, value):
        difference = value - find_center_y_of_points(lines.points)
        for point in lines.points:
            point[1] += difference
    y = property(gety, sety)    
    
    def getwidth(lines):
        return lines.right - lines.left
    def setwidth(lines, value):
        if not value == lines.right - lines.left:
            center_x = find_center_x_of_points(lines.points)
            x_scale = value / (lines.right - lines.left)
            scale_x(lines.points, center_x, x_scale)
    width = property(getwidth, setwidth)

    def getheight(lines):
        return lines.top - lines.bottom
    def setheight(lines, value):
        if not value == lines.top - lines.bottom:
            center_y = find_center_y_of_points(lines.points)
            y_scale = value / (lines.top - lines.bottom)
            scale_y(lines.points, center_y, y_scale)
    height = property(getheight, setheight)    
                  
    def getrotation(lines):
        return lines._rotation
    def setrotation(lines, value):
        if not value == lines._rotation:
            center_x = find_center_x_of_points(lines.points)
            center_y = find_center_y_of_points(lines.points)
            rotate(lines.points, value - lines._rotation, center_x, center_y)
            lines._rotation = value
    rotation = property(getrotation, setrotation)
    
    def gettop(lines):
        return find_top_of_points(lines.points)
    def settop(lines, value):
        distance = value - find_top_of_points(lines.points)
        for point in lines.points:
            point[1] += distance
    top = property(gettop, settop)
    
    def getbottom(lines):
        return find_bottom_of_points(lines.points)
    def setbottom(lines, value):
        distance = value - find_bottom_of_points(lines.points)
        for point in lines.points:
            point[1] += distance
    bottom = property(getbottom, setbottom)
    
    def getleft(lines):
        return find_left_of_points(lines.points)
    def setleft(lines, value):
        distance = value - find_left_of_points(lines.points)
        for point in lines.points:
            point[0] += distance
    left = property(getleft, setleft)
    
    def getright(lines):
        return find_right_of_points(lines.points)
    def setright(lines, value):
        distance = value - find_right_of_points(lines.points)
        for point in lines.points:
            point[0] += distance
    right = property(getright, setright)    
...
class Sprite:
    def __init__(sprite, x, y, **names):
        ...
        sprite._texture = getattr(sprite, next(iter(names)))
        sprite.x = x
        sprite.y = y
        sprite.rotation = 0  
        sprite.width = sprite._texture.width
        sprite.height = sprite._texture.height
        
    def draw(sprite):
        arcade.draw_texture_rectangle(sprite.x, sprite.y, sprite.width, sprite.height, sprite._texture, sprite.rotation)
        
    def getimage(sprite):
        return getattr(sprite, '_texture')
    def setimage(sprite, name):
        sprite._texture = name
        sprite.width = sprite._texture.width
        sprite.height = sprite._texture.height
    image = property(getimage, setimage)
    
class image():
    def __init__(sprite, name, **flip):
        sprite.name = name
        sprite.flip = flip
