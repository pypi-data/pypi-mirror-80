...
download_svhn_train_cropped = lambda path: download(path, TRAIN_32x32_FILENAME, root_url=ROOT_URL)
download_svhn_test_cropped = lambda path: download(path, TEST_32x32_FILENAME, root_url=ROOT_URL)
download_svhn_extra_cropped = lambda path: download(path, EXTRA_32x32_FILENAME, root_url=ROOT_URL)

download_svhn_cropped = lambda path: download(
  path,
  TRAIN_32x32_FILENAME, TEST_32x32_FILENAME,
  root_url=ROOT_URL
)

download_svhn_full_cropped = lambda path: download(
  path,
