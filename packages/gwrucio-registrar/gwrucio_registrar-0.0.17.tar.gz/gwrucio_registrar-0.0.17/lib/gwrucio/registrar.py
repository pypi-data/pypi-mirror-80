...
def get_lfn_meta(path, common_metadata):
    ...
    if metadata['extension'] == "sft":
        if not all(key in gwrucio.utils.REQUIRED_SFT_KEYS for key in
                   common_metadata.keys()):
            raise KeyError("File naming requires metadata:",
        ...
        if self.replicas:
            ...
            for replica in self.replicas:
                try:
                    ...
                    # TODO: force re-calculation of checksums?
            ...
            # Add replicas
            if new_replicas:
                CLIENT.add_replicas(rse=self.rse, files=new_replicas,
