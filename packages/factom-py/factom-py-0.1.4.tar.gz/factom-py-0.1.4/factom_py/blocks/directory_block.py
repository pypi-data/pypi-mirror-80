...
import logging
import factom_py
from factom_py.utils import merkle
...
class DirectoryBlockBody:
    ...
    def merkle_root(self):
        ...
        body_elements = [
            factom_py.blocks.AdminBlockHeader.CHAIN_ID,
            self.admin_block_lookup_hash,
            factom_py.blocks.EntryCreditBlockHeader.CHAIN_ID,
            self.entry_credit_block_header_hash,
            factom_py.blocks.FactoidBlockHeader.CHAIN_ID,
    ...
    def marshal(self):
        buf = bytearray()
        buf.extend(factom_py.blocks.AdminBlockHeader.CHAIN_ID)
        buf.extend(self.admin_block_lookup_hash)
        buf.extend(factom_py.blocks.EntryCreditBlockHeader.CHAIN_ID)
        buf.extend(self.entry_credit_block_header_hash)
        buf.extend(factom_py.blocks.FactoidBlockHeader.CHAIN_ID)
    ...
    def unmarshal_with_remainder(cls, raw: bytes, block_count: int):
        ...
        assert admin_block_chain_id == factom_py.blocks.AdminBlockHeader.CHAIN_ID
        admin_block_lookup_hash, data = data[:32], data[32:]
        ec_block_chain_id, data = data[:32], data[32:]
        assert ec_block_chain_id == factom_py.blocks.EntryCreditBlockHeader.CHAIN_ID
        entry_credit_block_header_hash, data = data[:32], data[32:]
        factoid_block_chain_id, data = data[:32], data[32:]
        assert factoid_block_chain_id == factom_py.blocks.FactoidBlockHeader.CHAIN_ID
