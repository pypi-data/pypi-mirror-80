...
# ----------------    -------    --------    -----------
# 2020.09.29 13:38    shengyang      v0.1        creation
import math
...
def compute_model_complexity(model, input_size, verbose=True, only_conv_linear=False):
    ...
    Examples::
        ...
        >>> num_params, flops = compute_model_complexity(model, (1, 3, 224, 224), verbose=True)
    """
    registered_handles = []
    layer_list = []
...
def demo():
    flops, params = compute_model_complexity(resnet50(), (1, 3, 224, 224), verbose=True)
    print(f"resnet50 flops:{flops / 1e9:.2f} GFLOPs")
    print(f"resnet50 params:{params / 1e6:.2f} M")

    flops, params = compute_model_complexity(resnet152(), (1, 3, 224, 224), verbose=True)
    print(f"resnet152 flops:{flops / 1e9:.2f} GFLOPs")
    print(f"resnet152 params:{params / 1e6:.2f} M")
