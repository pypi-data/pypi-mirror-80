...
def generate_ambassador_host_yaml(spell_domain_name, is_public):
  ...
  is_public: is ambassador accessible publicly?
  """
    # If ambassador is not publicly accessible, disable auto-HTTPS. Otherwise, applying this
    # configuration appears to fail because an SSL certificate cannot be obtained from LetsEncrypt.
    if is_public:
        acme_provider = r""
    else:
        acme_provider = r"""
  acmeProvider:
    ...
    return ambassador_http_host_yaml.format(
        acme_provider=acme_provider, spell_domain_name=spell_domain_name
    )
