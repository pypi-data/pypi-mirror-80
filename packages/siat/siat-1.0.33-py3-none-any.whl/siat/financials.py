...
def compare_snapshot(tickers,itemk,datatag=True,tag_offset=0.01):
    """
    ...
    for t in tickers:
        ...
        row=pd.Series({'ticker':t,'item':item,'value':value,'name':name3})
...
def calc_DebtToAsset(fsdf):
    ...
    fsdf1=fsdf.copy()
    try:
        fsdf1['Debt to Asset']=round(fsdf1['TotalDebt']/fsdf1['TotalAssets'],4)
    except:
        print("#Error(get_DebtToAsset): divided by zero in calculating DebtToAsset")
    try:
        fsdf1['Debt to Equity']=round(fsdf1['TotalDebt']/(fsdf1['TotalAssets']-fsdf1['TotalDebt']),4)    
    except:
        print("#Error(get_DebtToAsset): divided by zero in calculating DebtToEquity")    
...
def calc_fin_rates(fsdf):
    ...
    #短期偿债能力指标
    #流动比率：流动资产 / 流动负债
    fs['Current Ratio']=round(fs['CurrentAssets']/fs['CurrentDebt'],2)
    #速动比率：（流动资产-存货） / 流动负债
    fs['Quick Ratio']=round((fs['CurrentAssets']-fs['Inventory'])/fs['CurrentDebt'],2)
    #现金比率: （现金+现金等价物） / 流动负债
    fs['Cash Ratio']=round(fs['CashAndCashEquivalents']/fs['CurrentDebt'],2)
    #现金流量比率：经营活动现金流量 / 流动负债
    fs['Cash Flow Ratio']=round(fs['OperatingCashFlow']/fs['CurrentDebt'],2)
    
    #####长期偿债能力指标
    #资产负债率：负债总额 / 资产总额
    fs['Debt to Asset']=round(fs['TotalDebt']/fs['TotalAssets'],2)
    #股东权益比率：股东权益总额 / 资产总额
    fs['Equity to Asset']=round(fs['StockholdersEquity']/fs['TotalAssets'],2)
    #权益乘数：资产总额 / 股东权益总额
    fs['Equity Multiplier']=round(fs['TotalAssets']/fs['StockholdersEquity'],2)
    #负债股权比率：负债总额 / 股东权益总额
    fs['Debt to Equity']=round(fs['TotalDebt']/fs['StockholdersEquity'],2)
    #有形净值债务率：负债总额 / （股东权益-无形资产净额）
    fs['netIntangibleAsset']=fs['TotalAssets']-fs['NetTangibleAssets']
    fs['Debt to Tangible Net Asset']=round(fs['TotalDebt']/(fs['StockholdersEquity']-fs['netIntangibleAsset']),2)
    #偿债保障比率：负债总额 / 经营活动现金净流量
    fs['Debt Service Coverage']=round(fs['TotalDebt']/fs['OperatingCashFlow'],2)
    ...
    fs['Capital Accumulation']=round(fs['StockholdersEquity'].pct_change(),2)
    ...
    #净资产报酬率：净利润 / 平均净资产    
    fs['avgTotalEquity']=(fs['StockholdersEquity']+fs['StockholdersEquity'].shift(1))/2.0  
