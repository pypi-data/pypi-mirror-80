...
def secondary_fund_support(func):
    ...
    def wrapper(order_book_ids, *args, **kwargs):
        ...
        parent_ids = list(set(parent_ids))  # drop duplicate
        ret = func(parent_ids, *args, **kwargs)
        if ret is None or ret.empty:
            return ret
        # ensure has unique index
        index_len = ret.index.size
        unique_index = [np.arange(index_len).tolist()]
        ret.index = pd.MultiIndex(ret.index.levels + unique_index,
                                  codes=ret.index.codes + unique_index,
                                  names=ret.index.names + ['unique_index'])

        def _add_children_data(df, _obids, _main_map):
            """添加次级基金数据， 次级基金与主基金的数据一致

            :param df: 母基金api数据，index.name = ["order_book_id", "datetime"]
            :param _obids: 需要返回的order_book_id，包括主基金，次级基金
            :param _main_map: 次级基金和主基金的映射
            :return:
            """

            children_map = defaultdict(list)
            for obid in _obids:
                parent_id = _main_map.get(obid)
            ...
            for children_group in children_groups.values():
                parents = [_main_map.get(cid) for cid in children_group]
                children_df = df.reindex(parents, level=0)
                children_df.index = pd.MultiIndex([children_group] + children_df.index.levels[1:],
                                                  codes=children_df.index.codes)
                df = pd.concat([df, children_df])
            return df.reindex(_obids, level=0)

        ret = _add_children_data(ret, order_book_ids, main_code_map)
        # drop the unique index
        ret.index = ret.index.droplevel('unique_index')
        return ret
...
def get_units_change(order_book_ids, date=None, market="cn"):
    ...
    df = pd.DataFrame(df, columns=["subscribe_units", "redeem_units", "units", "order_book_id", "datetime", "net_asset"])
    df["datetime"] = pd.to_datetime(df["datetime"])
    return df.set_index(["order_book_id", "datetime"]).sort_index()
...
def get_fee(order_book_ids, fee_type=None, charge_type="front", date=None, market_type="otc", market="cn"):
    ...
    result = get_client().execute("fund.get_fee_v2", order_book_ids, fee_type, charge_type, date, market_type, market=market)
    if not result:
        return
    columns = [
...
def get_transaction_status(order_book_ids, start_date=None, end_date=None, fields=None, investor="institution",
    ...
    def process(x):
        if len(x) <= 1:
            return x
        status = ['Close', 'Suspended', 'Limited', 'Open']
        for stat in status:
            _df = x[x['redeem_status'] == stat]
            if not _df.empty:
                return _df

    result = pd.DataFrame(result)
    result = result.groupby('datetime').apply(process)
    result.set_index(["order_book_id", "datetime"], inplace=True)
