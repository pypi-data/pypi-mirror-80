...
class I18nMixin:
    ...
    def _get_language(self):
        ...
        self._override_context_language = None
        languages = self.context.get('request').META.get('HTTP_ACCEPT_LANGUAGE', None)
        server_languages = self._get_server_languages()

        if languages is not None:
            languages = [i[0] for i in parse_accept_lang_header(languages)]
