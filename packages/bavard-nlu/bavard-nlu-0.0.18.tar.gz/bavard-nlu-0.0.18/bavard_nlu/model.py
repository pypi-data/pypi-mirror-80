...
class NLUModel:
    ...
    def get_embedder(*, trainable: bool) -> hub.KerasLayer:
        return hub.KerasLayer("https://tfhub.dev/tensorflow/bert_multi_cased_L-12_H-768_A-12/2", trainable=trainable)

    @staticmethod
    def get_tokenizer(embedder: hub.KerasLayer = None, *, trainable: bool = False) -> FullTokenizer:
        if embedder is None:
            embedder = NLUModel.get_embedder(trainable=trainable)
        vocab_file = embedder.resolved_object.vocab_file.asset_path.numpy()
        do_lower_case = embedder.resolved_object.do_lower_case.numpy()
        tokenizer = FullTokenizer(vocab_file, do_lower_case)
        return tokenizer
    
    @staticmethod
    def get_embedder_and_tokenizer(*, trainable: bool) -> Tuple[hub.KerasLayer, FullTokenizer]:
        embedder = NLUModel.get_embedder(trainable=trainable)
        ...
        if self.save_model_dir is not None:
            callbacks.append(
                tf.keras.callbacks.ModelCheckpoint(
                    ...
                    save_best_only=True if val_data else False,
    ...
    def predict(self, text: str, tokenizer: FullTokenizer):
        text = text.lower()
