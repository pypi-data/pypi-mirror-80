        ...
        try:
            with Utility.lock:
                ...
                if os.path.exists(record_field_details_path):
                    ...
                    if Initializer.get_initializer().auto_refresh_fields and not Utility.new_file and not Utility.get_modified_modules and (Constants.FIELDS_LAST_MODIFIED_TIME not in record_field_details_json or Utility.force_refresh or (time.time() * 1000 - record_field_details_json[Constants.FIELDS_LAST_MODIFIED_TIME]) > 3600000):
                    ...
                    elif not Initializer.get_initializer().auto_refresh_fields and Utility.force_refresh and not Utility.get_modified_modules:
                        Utility.get_modified_modules = True
                        Utility.modify_fields(record_field_details_path, last_modified_time)
                        Utility.get_modified_modules = False
            ...
            if record_field_details_path is not None and os.path.exists(record_field_details_path):
                try:
                    ...
                    Utility.logger.info(Constants.EXCEPTION + ex.__str__())
