...
class JSONConverter(Converter):
    ...
    def is_not_record_request(self, request_instance, class_name, class_detail, instance_number, class_member_detail):
        ...
        if class_member_detail is not None:
            ...
            skip_mandatory = class_member_detail[Constants.SKIP_MANDATORY] if Constants.SKIP_MANDATORY in member_detail else False
        ...
        for member_name, member_detail in class_detail.items():
            ...
            if Constants.REQUIRED_IN_UPDATE in member_detail:
                required_in_update = member_detail[Constants.REQUIRED_IN_UPDATE]

            if Constants.PRIMARY in member_detail and member_detail[Constants.PRIMARY] and required_in_update:
            ...
            if modification is not None and modification != 0:
                ...
                if self.value_checker(class_name=class_name, member_name=member_name, key_details=member_detail,
                    ...
                    if field_value is None and not skip_mandatory:
                        self.mandatory_check(request_instance, member_name, member_detail, lookup, skip_mandatory)

                    if key_name in required_keys:
                        required_keys.remove(key_name)

                    if key_name in primary_keys:
                        primary_keys.remove(key_name)
        ...
        if self.common_api_handler.mandatory_checker is not None and self.common_api_handler.mandatory_checker and not skip_mandatory:
            if self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_CREATE:
                if lookup:
                    if len(primary_keys) > 0:
                        error = {
                            Constants.FIELD: class_member_name,
                            Constants.TYPE: request_instance.__class__.__name__,
                            Constants.KEYS: str(primary_keys),
                            Constants.INSTANCE_NUMBER: instance_number
                        }
                        raise SDKException(Constants.MANDATORY_VALUE_MISSING_ERROR, Constants.PRIMARY_KEY_MISSING_ERROR,
                                           error)
                    ...
                    error = {
                        Constants.FIELD: class_member_name,
                        Constants.TYPE: request_instance.__class__.__name__,
                        Constants.KEYS: str(required_keys),
                        Constants.INSTANCE_NUMBER: instance_number
                    }
                    raise SDKException(Constants.MANDATORY_VALUE_MISSING_ERROR, Constants.MANDATORY_KEY_MISSING_ERROR,
                                       error)

            if self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_UPDATE:
                if len(primary_keys) > 0:
                    error = {
                        Constants.FIELD: class_member_name,
                        Constants.TYPE: request_instance.__class__.__name__,
        ...
        elif lookup and self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_UPDATE and not skip_mandatory:
            if len(primary_keys) > 0:
                error = {
                    Constants.FIELD: class_member_name,
                    Constants.TYPE: request_instance.__class__.__name__,
                ...
                raise SDKException(Constants.MANDATORY_VALUE_MISSING_ERROR, Constants.PRIMARY_KEY_MISSING_ERROR, error)
    ...
    def is_record_request(self, record_instance, class_detail, instance_number, member_detail):
        ...
        request_json, module_detail, unique_values = {}, {}, {}
        module_api_name = self.common_api_handler.module_api_name
        ...
        for key_name in key_modified.keys():
            ...
            if len(key_detail) > 0:
                ...
                if self.value_checker(class_name, key_name, key_detail, key_value, unique_values, instance_number):
                    if key_value is None:
                        self.mandatory_check(record_instance, key_name, key_detail, lookup, skip_mandatory)

                    if Constants.PRIMARY in key_detail and key_detail[Constants.PRIMARY]:
                        is_primary_key_added = True
            ...
            if self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_CREATE:
                if lookup:
                    if len(primary_keys) > 0:
                        error = {
                            Constants.FIELD: class_member_name,
                            Constants.TYPE: record_instance.__class__.__name__,
                            Constants.KEYS: str(primary_keys),
                            Constants.INSTANCE_NUMBER: instance_number
                        }
                        raise SDKException(Constants.MANDATORY_VALUE_MISSING_ERROR, Constants.PRIMARY_KEY_MISSING_ERROR,
    ...
    def set_data(self, member_detail, field_value):
        if field_value is not None:
            ...
            if data_type == Constants.LIST_NAMESPACE:
                ...
                return self.is_record_request(field_value, self.__get_module_detail_from_user_spec_json(member_detail[Constants.MODULE]), 0, member_detail)

            elif Constants.STRUCTURE_NAME in member_detail:
                return self.form_request(field_value, member_detail[Constants.STRUCTURE_NAME], 0, member_detail)
    ...
    def mandatory_check(self, instance, member_name, member_detail, lookup, skip_mandatory):
        is_required = member_detail[Constants.REQUIRED] if Constants.REQUIRED in member_detail else False
        is_primary = member_detail[Constants.PRIMARY] if Constants.PRIMARY in member_detail else False

        if not skip_mandatory:
            if not lookup:
                if is_required and self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_CREATE:
                    error = {
                        Constants.FIELD: member_name,
                        Constants.TYPE: instance.__class__.__name__
                    }
                    raise SDKException(Constants.MANDATORY_VALUE_NULL_ERROR, Constants.MANDATORY_KEY_NULL_ERROR, error)

                if is_required and self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_UPDATE:
                    error = {
                        Constants.FIELD: member_name,
                        Constants.TYPE: instance.__class__.__name__
                    }
            ...
            if is_primary and self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_UPDATE and self.common_api_handler.mandatory_checker is not None and self.common_api_handler.mandatory_checker:
                error = {
                    Constants.FIELD: member_name,
                    Constants.TYPE: instance.__class__.__name__
                }
