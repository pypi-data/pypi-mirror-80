...
class InteractionDatasetABC(ABC):
    ...
    def select(self, query, copy=True):
        ...
        """
        if query in self._select_cache:
            return self._select_cache[query]
        self._select_cache[query] = self._select(query, copy=copy)
        return self._select_cache[query]
        #pass

    @abstractmethod
    def _select(self, query, copy=True):
        pass
    ...
    def apply(self, column, function):
        ...
        self._apply(column, function)

    @abstractmethod
    def _apply(self, column, function):
        pass
