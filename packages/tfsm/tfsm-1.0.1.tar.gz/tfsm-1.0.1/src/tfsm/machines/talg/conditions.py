...
class CallAlpha(fsm.Condition):
    def yes(self, i):
        case = not i.claim(p.ALPHA_ALIAS) and i.algtime() < p.ALPHA_TIME_ENTER_BEFORE
...
class CallBeta(fsm.Condition):
    def yes(self, i):
        case = not i.claim(p.BETA_ALIAS) and i.algtime() <= p.TIME_STOP_CALL
...
class CallCharlie(fsm.Condition):
    def yes(self, i):
        case = not i.claim(p.CHARLIE_ALIAS) and i.algtime() <= p.TIME_STOP_CALL
