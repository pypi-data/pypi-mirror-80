...
class Brokerage:
    ...
    def live(self, symbol, mock=None): 
        ...
        response = {
            "symbol": symbol,
            "time": datetime.datetime.now(tz=settings.TIME_ZONE),
...
class TfsmInput(fsm.Input):
    ...
    def __lt__(self, other): return self.algdt < other.algdt
    def __gt__(self, other): return self.algdt > other.algdt
    def __le__(self, other): return self.algdt <= other.algdt
    def __ge__(self, other): return self.algdt >= other.algdt
    def __eq__(self, other): return self.algdt == other.algdt
    def __ne__(self, other): return self.algdt != other.algdt
    ...
    def load(self):
        ...
        self.algdt = datetime.datetime.now(self.timezone)
        self.sysdt = self.algdt.replace(microsecond=0)
    ...
    def clock(self, tick=False):
        ...
        if tick: 
            self.algdt = self.stash["time"]
        else: self.sysdt = self.sysdt + datetime.timedelta(seconds=settings.PERIOD_OF_DATA)

        return now, self.sysdt
    ...
    # == Utility ==
    def now(self): return datetime.datetime.now(self.timezone)
    def algtime(self): return self.algdt.time()
    def systime(self): return self.sysdt.time()
