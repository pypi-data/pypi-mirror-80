...
CALL apoc.cypher.run("
    ...
    MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 2}]-(auxiliaryverb:WORD)
    MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 1}]-(auxiliaryverb1:WORD)
    ...
    WITH {
        ...
        verbgames_pattern_items:{
                                ...
                                conjugated_auxiliaryverb: auxiliaryverb.text
                                }
...
MATCH (interest) - [:HAS_CONTENT] -> (content:CONTENT) - [pattern_rel:HAS_PATTERN] -> (:PATTERN{code:'PCIPT'})
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 3}]-(subject:WORD)
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 2}]-(auxiliaryverb:WORD)
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 1}]-(auxiliaryverb1:WORD)
MATCH (content)-[rel_target_word:HAS_WORD{position : pattern_rel.position}]-(target_word:WORD)
