...
class GoingToFutureNegativeChoiceQuestionCreator(QuestionCreator):
    ...
    def create(self, data_question: DataQuestion, user_id: str) -> Question:
        ...
        question.baits = self._get_baits(subject, target_lemma)
    ...
    def _get_baits(self, subject: str, target_lemma: str) -> List[str]:

        return random.sample(
            [
                f"{subject} is don't going to {target_lemma}",
                f"{subject} not is going to {target_lemma}",
                f"{subject} is no going to {target_lemma}",
            ],
            2,
        )
