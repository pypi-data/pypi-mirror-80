...
class PresentContinuousFullWordTypingQuestionCreator(QuestionCreator):
    ...
    def _get_correct_answer(self, pattern: str, subject: str) -> str:
    ...
    def _get_pattern_substitution(self, pattern: str, subject: str) -> str:
        final_pattern = pattern
        words_to_remove = pattern.replace(subject, "").replace("'", "").strip()
        for word in words_to_remove.split():
            final_pattern = self._remove_word(final_pattern, word)
        return final_pattern

    def _get_phrase(self, original_phrase: str, pattern: str, subject: str) -> str:
