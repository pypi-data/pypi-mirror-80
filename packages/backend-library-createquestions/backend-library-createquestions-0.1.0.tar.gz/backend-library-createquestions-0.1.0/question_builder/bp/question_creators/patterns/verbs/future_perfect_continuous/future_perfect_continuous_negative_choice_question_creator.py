...
class FuturePerfectContinuousNegativeChoiceQuestionCreator(QuestionCreator):
    ...
    def create(self, data_question: DataQuestion, user_id: str) -> Question:
        ...
        question.baits = self._get_baits(subject, target_lemma, verb_present_continuous)
    ...
    def _get_baits(self, subject: str, target_lemma: str, verb_present_continuous: str) -> List[str]:
        return random.sample(
            [
                f"{subject} {WILL} have {BEEN} no {verb_present_continuous}",
                f"{subject} don't {WILL} have {BEEN} {target_lemma}",
                f"{subject} {WILL} not have {BEEN} {verb_present_continuous}",
            ],
            2,
        )
