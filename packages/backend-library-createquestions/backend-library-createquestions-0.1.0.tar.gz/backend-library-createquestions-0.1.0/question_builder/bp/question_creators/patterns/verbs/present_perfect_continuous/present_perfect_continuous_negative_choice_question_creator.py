...
class PresentPerfectContinuousNegativeChoiceQuestionCreator(QuestionCreator):
    ...
    def _get_baits(self, subject: str, target_lemma: str, verb_present_continuous: str) -> List[str]:
        return random.sample(
            [
                f"{subject} don't have {BEEN} {verb_present_continuous}",
                f"{subject} not have {BEEN} {verb_present_continuous}",
                f"{subject} have no {BEEN} {verb_present_continuous}",
            ],
            N_BAITS,
        )
