...
CALL apoc.cypher.run("
    ...
    MATCH (c)-[:HAS_WORD{position : pattern_rel.position - 2}]-(auxiliaryverb1:WORD)
    MATCH (c)-[:HAS_WORD{position : pattern_rel.position - 1}]-(auxiliaryverb2:WORD)
...
MATCH (interest) - [:HAS_CONTENT] -> (content:CONTENT) - [pattern_rel:HAS_PATTERN] -> (:PATTERN{code:'PRPCC'})
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 3}]-(subject:WORD)
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 2}]-(auxiliaryverb1:WORD)
MATCH (content)-[:HAS_WORD{position : pattern_rel.position - 1}]-(auxiliaryverb2:WORD)
MATCH (content)-[rel_target_word:HAS_WORD{position: pattern_rel.position}]-(target_word:WORD)
