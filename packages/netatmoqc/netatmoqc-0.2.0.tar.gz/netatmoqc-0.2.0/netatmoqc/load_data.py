...
import logging
import humanize
from .dtgs import epoch2datetime, datetime2epoch, Dtg
...
def gen_netatmo_fpaths_for_dates(dates, rootdir):
    """
    Returns an generator for the paths of the NetAtmo CSV files that need
...
def read_netatmo_data_for_dates(
    dates,
    rootdir,
    dropna=True,
    fillna=dict(sum_rain_1=0.0),
    ...
    """
    Returns a pd dataframe with data from NetAtmo files for given dates
    """
    data_from_all_files = {}
    for f in gen_netatmo_fpaths_for_dates(dates, rootdir):
        data_from_all_files[f.stem] = read_netatmo_csv(
            f,
            dropna=dropna,
            fillna=fillna,
            recover_pressure_from_mslp=recover_pressure_from_mslp,
            drop_mslp=drop_mslp,
        )
    if len(data_from_all_files) > 0:
        ...
        raise DataNotFoundError(
            "Could not find data for date(s)={} under {}".format(
                dates, rootdir
            )
        )
...
def remove_duplicates_within_cycle(df, dtg):
    ...
    df = df.groupby(["id"], as_index=False, sort=False).first()
    return df
...
def read_netatmo_data_for_dtg(
    ...
    if len(data_from_all_files) == 0:
        raise DataNotFoundError(
            "Could not find data for DTG={} under {}".format(dtg, rootdir)
        )
