...
from typer import colors, confirm, secho
...
class Export:
    ...
    # TODO CHECK DATA EXISTS, RETURN USER-FRIENDLY MESSAGE
    ...
    def export_to_sheets(self, filename: str):
        """
        Export to sheets.
        """
        ...
        browser_open = confirm("Do you want to open sheets in your browser?")
        if browser_open:
            try:
                webbrowser.open(r.json()["sheetURL"])
            except KeyError:
                secho("Something went terribly wrong!", bold=True, fg=colors.BRIGHT_RED)
        else:
            try:
                secho(
