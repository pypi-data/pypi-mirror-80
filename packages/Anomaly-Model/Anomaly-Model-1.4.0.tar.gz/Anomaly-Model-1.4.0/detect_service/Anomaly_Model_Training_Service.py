...
def read_training_validation_from_COS(model_id,icos_client,bucketName,bucketName_model,testData_date_list,dateRange=''):
    ...
    for weldDay in testData_date_list:
        feature_file_name = 'Production_Lincoln_features_data/'+deviceId+'_LincolnFANUC_'+weldDay+'_welding_stable_data_weldid_'+weldId_with_toolID+'_feature.csv'
        body = icos_client.get_object(Bucket=bucketName,Key=feature_file_name)['Body']
        # add missing __iter__ method, so pandas accepts body as file-like object
        if not hasattr(body, "__iter__"): body.__iter__ = types.MethodType( __iter__, body )
...
#    testData_join = pd.concat([testData_join,validate_df.loc[validate_df.label == 1]], sort=True).reset_index(drop=True)
    testData_join = pd.concat([testData_join,validate_df.loc[validate_df.label == 1]], axis = 0).reset_index(drop=True)
...
#----------- anomaly score -----------------------
def get_anomaly_score(model_id,icos_client,bucketName,pipeline,selected_features,testData_date,localPath='./Production_Anomaly_data/'):
    deviceId = model_id.split('_weld')[0] # Formet_FR4_STA60_LH_R1_weld32_toolB
    weldId_with_toolID =  model_id.split('_weld')[1]
    feature_file_name = 'Production_Lincoln_features_data/'+deviceId+'_LincolnFANUC_'+testData_date+'_welding_stable_data_weldid_'+weldId_with_toolID+'_feature.csv'
    body = icos_client.get_object(Bucket=bucketName,Key=feature_file_name)['Body']
    # add missing __iter__ method, so pandas accepts body as file-like object
    if not hasattr(body, "__iter__"): body.__iter__ = types.MethodType( __iter__, body )
    feature_data = pd.read_csv(body)
...
#-----------------------------------------
#------------------add TSA components
def add_TSA_trend_and_residual(feature_data,freq=250):
    # feature list
    ...
    for tsa_feature in features_select_TSA:
        # original data
        res = sm.tsa.seasonal_decompose(feature_data[tsa_feature] , freq=250)
