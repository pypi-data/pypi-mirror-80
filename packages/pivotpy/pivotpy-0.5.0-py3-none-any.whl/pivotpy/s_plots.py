...
def quick_rgb_lines(path_evr    = None,
    ...
    """
    - Returns axes object and plot on which all matplotlib allowed actions could be performed. In this function,orbs,orblabels,elements all have list of length 3. Inside list, sublists or strings could be any length but should be there even if empty.
    - **Parameters**
        - path_evr   : path/to/vasprun.xml or output of `export_vasprun`. Auto picks in CWD.
        - ax         : Matplotlib axes object, if not given, one is created.
        - skipk      : Number of kpoints to skip, default will be from IBZKPT.
        ...
        - orblabels  : List [str,str,str] of orbitals labels. empty string should exist to maintain shape.
...
def quick_color_lines(path_evr  = None,
    ...
    """
    - Returns axes object and plot on which all matplotlib allowed actions could be performed. If given, axes,elements,orbs colors, and orblabels must have same length. If not given, zeroth ion is plotted with s-orbital.
    - **Parameters**
        - path_evr   : Path/to/vasprun.xml or output of `export_vasprun`. Auto picks in CWD.
        ...
        - colors     : Defualt is auto pick. List ['red',] of orbitals colors (str,rgb,rgba), len(colors)==len(orbs) must hold.
    ...
    for key in vr_keys:
        ...
        if(len(elements)!=len(orbs)):
            ...
            return
        # Fix elements and orblabels length
        if(len(elements)!=len(orblabels)):
            raise ValueError("elements and orblabels expect same length even if their entries are empty.")
            return
...
def collect_dos(path_evr      = None,
    ...
    """
    - Returns lists of energy,tdos, pdos and labels. If given,elements,orbs and orblabels must have same length. If not given, zeroth ions is collected with s-orbital.
    - **Parameters**)
        - path_evr   : Path/to/vasprun.xml or output of `export_vasprun`. Auto picks in CWD.
        - elim       : [min,max] of energy range.
        - E_Fermi    : If not given, automatically picked from `export_vasprun`.
...
def quick_dos_lines(path_evr      = None,
        ...
        - **Parameters**)
            ...
            - orbs       : List [[0],] lists of indices of orbitals, could be empty.
            - orblabels  : List [str,] of orbitals labels. len(orblabels)==len(orbs) must hold.
            - colors     : Auto assign colors if None. List ['red',] of orbitals colors (str,rgb,rgba), len(colors)==len(orbs) must hold.
        ...
        import matplotlib as mpl
        import matplotlib.pyplot as plt
        en,tdos,pdos,labels,vr=None,None,None,None,None # Placeholders for defining. must be here.
        cl_dos=sp.collect_dos(path_evr=path_evr,elim=elim, elements=elements, orbs=orbs, orblabels=orblabels, \
                                E_Fermi=E_Fermi, spin=spin, interpolate=interpolate, n=n, k=k)
        try:
