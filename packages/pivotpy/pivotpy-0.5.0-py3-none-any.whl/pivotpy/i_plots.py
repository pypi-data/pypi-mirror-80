...
__all__ = ['get_rgb_data', 'rgb_to_plotly', 'plotly_to_html', 'plotly_rgb_lines', 'plotly_dos_lines', 'iplotfromtxt']
...
# Cell
def rgb_to_plotly(rgb_data=None,mode='markers',max_width=5,showlegend=False,name='',labels=['s','p','d']):
    """
    ...
    if(rgb_data):
        ...
        if(mode=='markers'):
            ...
            data.append(go.Scatter(x=k,y=en,mode='markers', name=name,
                        marker=dict(color=colors,size=lws),line=dict(width=0.001,
        ...
        if(mode=='bands'):
            for i,values in enumerate(zip(en,colors,lws,h_text)):
                ...
                data.append(go.Scatter(x=k,y=e,mode='markers+lines',
                            ...
                            marker=dict(color=c,size=w),line=dict(width=0.001,
...
def plotly_rgb_lines(path_evr    = None,
    ...
    """
    - Returns plotly's figure object, takes care of spin-polarized calculations automatically. `elements`,`orbs` and `orblabels` are required to be one-to-one lists of size 3 where each item in list could be another list or integer.
    - **Parameters**
    ...
    for key in vr_keys:
        ...
        if _elements_inds and max(_elements_inds) > elem_inds-1:
        ...
        if(ISPIN==2):
            ...
            data1=rgb_to_plotly(rgb_data=rgb_lines1,mode=mode,showlegend=showlegend,labels=labels,name='B<sup>↑</sup>',max_width=max_width)
            new_args2=dict(kpath=K, evals_set=En2, pros_set=Pros2,**args_dict)
            rgb_lines2=get_rgb_data(**new_args2)
            data2=rgb_to_plotly(rgb_data=rgb_lines2,mode=mode,showlegend=showlegend,labels=labels,name='B<sup>↓</sup>',max_width=max_width)
