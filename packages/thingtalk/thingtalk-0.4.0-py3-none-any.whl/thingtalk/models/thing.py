...
from .message_node import MsgNode
...
class Thing(MsgNode):
    ...
    def set_href_prefix(self, prefix: typing.Optional[str] = None):
        """
        Set the prefix of any hrefs associated with this thing.
        prefix -- the prefix
        """
        self.href_prefix = prefix if prefix else f"/things/{self.id}"
    ...
    async def has_property(self, property_name: str) -> bool:
        """
    ...
    async def add_subscriber(self, ws):
        ...
        if id(ws) not in self.subscribers:
            logger.info(f"subscriber {ws.url} added")
    ...
    async def remove_subscriber(self, ws):
        ...
        if id(ws) in self.subscribers:
            logger.info(f"subscriber {ws.url} poped")
            self.subscribers.pop(id(ws))

        for name in self.available_events:
            await self.remove_event_subscriber(name, ws)

    async def add_env_subscriber(self, ws):
        """
        Add a new websocket subscriber.
        ws -- the websocket
        """
        if id(ws) not in self.env_subscribers:
            logger.info(f"env subscriber {ws.url} added")
            self.env_subscribers[id(ws)] = ws

    async def remove_env_subscriber(self, ws):
        """
        Remove a websocket subscriber.
        ws -- the websocket
        """
        if id(ws) in self.env_subscribers:
            logger.info(f"env subscriber {ws.url} poped")
            self.env_subscribers.pop(id(ws))

        for name in self.available_events:
            await self.remove_event_subscriber(name, ws)

    async def remove_env_subscriber_by_id(self, ws_id):
        """
        Remove a websocket subscriber.
        ws -- the websocket
        """
        if ws_id in self.env_subscribers:
            self.env_subscribers.pop(ws_id)

        for name in self.available_events:
            await self.remove_event_subscriber_by_id(name, ws_id)
    ...
    async def remove_event_subscriber_by_id(self, name, ws_id):
        """
        Remove a websocket subscriber from an event.
        name -- name of the event
        ws -- the websocket
        ...
        if (
                name in self.available_events
                and ws_id in self.available_events[name]["subscribers"]
        ):
            self.available_events[name]["subscribers"].pop(ws_id)

    async def property_notify(self, property_name, value_):
        """
    ...
    async def property_action(self, property_name, value_):
        """
