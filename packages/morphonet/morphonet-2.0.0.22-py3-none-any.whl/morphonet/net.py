...
from morphonet.tools import addslashes,tryParseInt,strblue,strred,strgreen,nodata,ss,getObjectIn,isObjectIdentifcal,getObject,getLastCuration
...
class Net:
    ...
    def _getURLDecompress(self,data):
        ...
        if data is not None:
            try:
                ...
                if "url" in data_str:
                    dict_str = ast.literal_eval(data_str)
                    if "url" in dict_str:
                        url=dict_str["url"]
                else:
                    url=data_str
    ...
    def getMesh(self,t,quality=0,channel=0):
        ...
        if self._isDataSet():
            ...
            obj=self._getURLDecompress(data)
            if obj is None:
                print(" --> No mesh found")
        return obj
    ...
    def getInfoByName(self,name):
        ...
        if self._isDataSet():
            ...
            print(data)
            info=self._getURLDecompress(data)
        return info
        ...
        for line in infos:
            if len(line)>0 and line[0]!="#":
                if line.find("type")==0:
                    ...
                    tab=line.split(":")
                    if tab[0] in objects: #Multiple times the same value (we put in list)
                        val1=objects[tab[0]]
                        if type(val1)!=list :
                            objects[tab[0]]=[]
                            objects[tab[0]].append(val1)
                        objects[tab[0]].append(tab[1])
                    else:
                        objects[tab[0]]=tab[1]
        return objects
    def _getType(self,infos):
    ...
    def getInfoCurated(self,id_info):
        ...
        if self._isDataSet():
            ...
            if info is not None:
                info_objets=self._getObjects(info)
                curation=self.getCuration(id_info)
                if curation is not None:
                    curation_objets=self._getObjects(curation)
                    ...
                    info_curated+="type:"+self._getType(info)+"\n"

                    for o in info_objets:
                        isCurated=getObjectIn(o,curation_objets)
                        info_curated+=str(o)+":"
                        if isCurated is not None:
                            oCurated=getLastCuration(isCurated)
