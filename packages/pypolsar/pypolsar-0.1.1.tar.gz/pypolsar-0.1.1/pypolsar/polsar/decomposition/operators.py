...
def PolarimetricCoherencyTMatrix(k_vector):
    """
    ...
    for ix, iy in np.ndindex(pol_coherency_matrix.shape[2:]):
        pol_coherency_matrix_filtered[:, :, ix, iy] = signal.convolve(pol_coherency_matrix[:, :, ix, iy],
                                                                            mean_filter, mode='same')
    return pol_coherency_matrix, pol_coherency_matrix_filtered

def PolarimetricCovarianceCMatrix(omega_vector):
    """
    ...
    for ix, iy in np.ndindex(pol_covariance_matrix.shape[2:]):
        # print(i)
        pol_covariance_matrix_filtered[:, :, ix, iy] = signal.convolve(pol_covariance_matrix[:, :, ix, iy],
                                                                             mean_filter, mode='same')
