...
# Need to modify in order to handle cases where a table has more than
# one sex. Currently, plot_pop() only correctly deals with cases where
# there is only one type of sex in a table.
def plot_pop(
    poptbl, 
    ...
    filtered by `age_range`. Set `alpha` for transparency.

    Usage
    -----
    `plot_pop(poptbl[, age_range, alpha])`
    ...
    * `alpha`: a float, .5 by default; the argument to control transparency
    ...
    pop_age = poptbl.get_pop_groups()[1]
