...
class Chinjmap(object):
    ...
    def getpdf(self, years, num, pdfRoot):
        ...
        # 封面
        fengmian = html.xpath("//p[@align='center']/a/@href").extract_first("")
        if fengmian != "":
            f_url = "http://www.chinjmap.com" + fengmian
            title = "{}封面".format(line)
            pdfFilePath = Parent.makedir(self.journal, years, str(num), title, pdfRoot)
            fig = Parent.getPdf(f_url, title, pdfFilePath, hdrs)
            if fig == 1:
