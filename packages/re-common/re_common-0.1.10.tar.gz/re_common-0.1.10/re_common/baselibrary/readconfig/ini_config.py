...
class IniConfig(object):
    ...
    def builder(self, configmoudle: ConfigParserMoudle = None):
        if configmoudle:
            self.moudle = configmoudle
    ...
    def read_config(self, encoding="utf-8"):
        """
        读取配置文件 这样cf本身就是一个字典
        :param encoding:
        :param configPath:
        ...
        assert self.cf, AttributeError("还没有调用get_configparser为其赋值 请先调用该函数")
        try:
            self.cf.read(self.configpath, encoding=encoding)
        except:
            print('读取配置失败:' + traceback.format_exc())
            sys.exit(-1)
        return self

    def read_config_string(self, encoding="utf8"):
        """
        读取配置文件 这样cf本身就是一个字典
        :param cfstr:
        :return:
        ...
        try:
            with open(self.configpath, mode='rb') as f:
                ...
                if content.startswith(b'\xef\xbb\xbf'):  # 去掉 utf8 bom 头
                    content = content[3:]
            self.cf.read_string(content.decode(encoding))
        except:
            print('读取配置失败:' + traceback.format_exc())
            sys.exit(-1)
        return self
    ...
    def conf_to_dict(self) -> OrderedDict:
        """
        读取文件转dict
        :param cf:
        :return:
    ...
    def get_conf_dict(self, configfile=None, encoding='utf-8', keep_keys_case=True):
        """
        获取config的dict对象
        :param configfile: 是否使用原来的文件
        :param encoding: 编码
        :param keep_keys_case: 是否保持配置文件大小写(这里只能保证配置文件的建大小写)
        :return:
        """
        if configfile:
            self.set_config_path(configfile)
        self.builder()
        if keep_keys_case:
            self.set_keep_keys_case()
        return self.read_config(encoding).conf_to_dict()
