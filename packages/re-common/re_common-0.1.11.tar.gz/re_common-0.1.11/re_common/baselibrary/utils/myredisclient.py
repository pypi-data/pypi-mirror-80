...
class MyRedis(object):
    def __init__(self, configpath="", sesc="", encoding="utf-8"):
        self.configpath = configpath
        self.sesc = sesc
        self.encoding = encoding
        self.is_conn_or_pipe = True
    ...
    def set_redis_from_config(self):
        ...
        dictsall = IniConfig(self.configpath).get_conf_dict(encoding=self.encoding)
    ...
    def builder(self, configpath=None, sesc="proxy", encoding="utf-8", conn_or_pipe=True):
        """
        一次性设置好所有变量，builder模式
        :param configpath:
        :param sesc:
        :param encoding:
        :param conn_or_pipe:
        ...
        """
        if configpath:
            self.set_configpath(configpath=configpath)
        self.set_redis_from_config(sesc=sesc, encoding=encoding)
        self.conn_redis()
        self.get_pipeline()
        self.set_conn_or_pipe(conn_or_pipe)
