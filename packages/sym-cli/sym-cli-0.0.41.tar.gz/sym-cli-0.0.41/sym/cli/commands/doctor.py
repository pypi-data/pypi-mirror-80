...
def doctor(
    ...
    if not did_update and (version := self_update()):
        click.secho(f"Updating to {version}!", fg="yellow")
        return subprocess.run([*sys.argv, "--did-update"])
    else:
        click.secho("No updates found!", fg="green")
    ...
    with push_envs({"SYM_LOG_DIR": str(doctor_dir / "logs"), "SYM_DEBUG": "true"}):
        if mode == "ansible":
            run_ansible_doctor(client, doctor_dir, inventory, playbook)

    click.echo("Zipping it all up...")
    zip = zip_logs(doctor_dir)
    output = Path(output_dir) / zip.name
    zip.link_to(output)
