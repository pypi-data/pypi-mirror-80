...
def test_saml2aws(
    ...
    capture_command.enqueue_outputs("AWS_REGION=foobar\nAWS_FOOBAR=baz")
    ...
    capture_command.assert_command(
        f"saml2aws --config {saml2aws.config_file} --idp-account sym --skip-prompt exec -- env",
...
def test_saml2aws_get_creds(
    ...
    capture_command.enqueue_outputs(
        "AWS_REGION=foobar\nAWS_FOOBAR=baz", "", "AWS_REGION=foobar\nAWS_FOOBAR=baz"
    )
    with test_context(debug=False) as saml2aws:
        with capture_command():
            assert saml2aws.get_creds() == {"AWS_REGION": "foobar"}
