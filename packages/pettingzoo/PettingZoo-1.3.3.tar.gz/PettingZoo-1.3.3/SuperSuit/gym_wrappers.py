...
class color_reduction_v0(BasicObservationWrapper):
    def __init__(self,env,mode='full'):
        super().__init__(env,basic_transforms.color_reduction_v0,mode)
...
class clip_actions_v0(ActionWrapper):
    def __init__(self, env):
        super().__init__(env)
        assert isinstance(self.action_space, Box), "clip_actions_v0 only works for Box action spaces"
...
class clip_reward_v0(RewardWrapper):
