...
class TmxReaderBilingualText:
    ...
    def read(self, segs_by_locale):
        if self._tmx_src_locale is None:
            src_locales = [locale for locale in segs_by_locale if any(src_lang in Locale.to_lang(locale) for src_lang in self._src_lang)]
            ...
            tgt_locales = [locale for locale in segs_by_locale if any(tgt_lang in Locale.to_lang(locale) for tgt_lang in self._tgt_lang)]
