    ...
    if automl_settings.drop_column_names is not None:
        ...
        if automl_settings.grain_column_names is not None:
            if drop_set.intersection(grain_set):
                raise DropSpecialColumn("Time series identifier column cannot be dropped. "
    ...
    if automl_settings.n_cross_validations is None and X_valid is None:
        ...
        if X.shape[0] < min_points:
            ...
            pii_message = (
                ...
                "horizon, the number of cross validations, lags or rolling window size.".format(
...
def _check_grain_min_points(number_of_data_points: int,
    ...
    """
    if (hasattr(automl_settings, TimeSeries.SHORT_SERIES_HANDLING) and
...
def _validate_timeseries_train_valid_tsdf(tsdf_train: TimeSeriesDataFrame,
    ...
    if len(grain_difference) > 0:
        ...
        if len(grain_in_train_not_in_valid) > 0:
            error_msg_list.append(
                "Time series identifier {} found in training data but not in validation data.".format(
                    ",".join(["[{}]".format(grain) for grain in grain_in_train_not_in_valid])
                )
            )
        if len(grain_in_valid_not_in_train) > 0:
            error_msg_list.append(
                "Time series identifier {} found in validation data but not in training data.".format(
                    ",".join(["[{}]".format(grain) for grain in grain_in_valid_not_in_train])
                )
            )
        raise GrainAbsent(
            exception_message="Training and validation datasets contain different sets of time series identifiers",
    ...
    for grain, tsdf in train_grain_data_dict.items():
        ...
        if has_lookback_features and tsdf.time_index.max() + tsdf.infer_freq() != tsdf_valid.time_index.min():
            error_message = "Training and validation data are not contiguous in time series identifier."
        ...
        if tsdf_valid.shape[0] > 1:
            if tsdf.infer_freq() != tsdf_valid.infer_freq():
                error_message = "For time series identifier {}, "
                "training data and validation data have different frequency."
