...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    AutoMLInternal, InsufficientMemory, MaxHorizonExceeded, PowerTransformerInverseTransform, QuantileRange,
...
from azureml.automl.core.shared.exceptions import (AutoMLException,
                                                   ...
                                                   PredictionException,
                                                   TransformException,
                                                   UntrainedModelException, ValidationException, ResourceException)
...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    def _do_check_max_horizon(self,
        ...
        # Exceeding maximum horizon is no longer an error.
            ...
            if de.error_code == MaxHorizonExceeded().code:
                return True
            raise
        return False
        ...
        if X_pred is not None:
            ...
            for column in X_copy.columns:
                if column not in special_columns and \
                    ...
                    self._warn_or_raise(TimeseriesDfContainsNaN, ignore_data_errors)
                    break
        ...
        for grain, df_one in X.groupby(self.grain_column_names):
            ...
            if all(pd.isnull(y) for y in df_one[constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN]) and\
               not is_reported and hasgap and self._lag_or_rw_enabled() and not self.is_grain_dropped(grain):
                ...
                self._warn_or_raise(TimeseriesNoDataContext, ignore_data_errors)
                is_reported = True
            if self._ts_transformer.dict_latest_date.get(grain) is None:
    ...
    def _recursive_forecast_one_grain(self,
        ...
        while origin_time <= df_pred[self.time_column_name].max():
            ...
            try:
                ...
                if err.error_code == TimeseriesNothingToPredict().code:
    ...
    def _check_data_rolling_evaluation(self,
        ...
        if y_pred_unknown:
            ...
            self._warn_or_raise(TimeseriesMissingValuesInY, ignore_data_errors=False)
    ...
    def _get_last_y_one_grain(self,
        ...
        if nan_indexes[0].size == 0:
            if not is_absent_grain:
                self._warn_or_raise(TimeseriesNothingToPredict, ignore_data_errors)
        ...
        if nan_indexes[0].size > 0:
            ...
            if not all([pd.isnull(x) for x in y[first_nan:]]):
                if not is_absent_grain:
                    self._warn_or_raise(TimeseriesNonContiguousTargetColumn, ignore_data_errors)
