...
class ExplainedVariance(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if np.isnan(ret) or np.isinf(ret):
            return constants.REGRESSION_RANGES[constants.EXPLAINED_VARIANCE][0]
        return ret
...
class R2(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        ret = sklearn.metrics.r2_score(
            ...
            sample_weight=self._sample_weight, multioutput='uniform_average')
        if np.isnan(ret) or np.isinf(ret):
            return constants.REGRESSION_RANGES[constants.R2_SCORE][0]
        return np.clip(ret, *constants.REGRESSION_RANGES[constants.R2_SCORE])
...
class MeanAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        ret = sklearn.metrics.mean_absolute_error(
            self._y_test, self._y_pred,
            sample_weight=self._sample_weight, multioutput='uniform_average')
        return np.clip(ret, *constants.REGRESSION_RANGES[constants.MEAN_ABS_ERROR])
...
class NormMeanAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if self._y_min == self._y_max:
            self._logger.warning("Skipping NormMeanAbsoluteError calculation. All targets are equal.")
            return constants.REGRESSION_RANGES[constants.NORM_MEAN_ABS_ERROR][1]
        ...
        return np.clip(norm_ret, *constants.REGRESSION_RANGES[constants.NORM_MEAN_ABS_ERROR])
...
class MedianAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        return np.clip(ret, *constants.REGRESSION_RANGES[constants.MEDIAN_ABS_ERROR])
...
class NormMedianAbsoluteError(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        if self._y_min == self._y_max:
            self._logger.warning("Skipping NormMedianAbsoluteError calculation. All targets are equal.")
            return constants.REGRESSION_RANGES[constants.NORM_MEDIAN_ABS_ERROR][1]

        ret = sklearn.metrics.median_absolute_error(self._y_test, self._y_pred)
        if np.isinf(ret):
            return constants.REGRESSION_RANGES[constants.NORM_MEDIAN_ABS_ERROR][1]
        norm_ret = ret / np.abs(self._y_max - self._y_min)  # type: ignore
        return np.clip(norm_ret, *constants.REGRESSION_RANGES[constants.NORM_MEDIAN_ABS_ERROR])
...
class RMSE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if np.isnan(ret) or np.isinf(ret):
            return constants.REGRESSION_RANGES[constants.RMSE][1]
        return np.clip(ret, *constants.REGRESSION_RANGES[constants.RMSE])
...
class NormRMSE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        if self._y_min == self._y_max:
            self._logger.warning("Skipping NormRMSE calculation. All targets are equal.")
            return constants.REGRESSION_RANGES[constants.NORM_RMSE][1]
        ...
        if np.isnan(ret) or np.isinf(ret):
            return constants.REGRESSION_RANGES[constants.NORM_RMSE][1]
        norm_ret = ret / np.abs(self._y_max - self._y_min)  # type: ignore
        return np.clip(norm_ret, *constants.REGRESSION_RANGES[constants.NORM_RMSE])
...
class RMSLE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        if self._has_negatives:
            self._logger.warning("Skipping RMSLE calculation. y_test/y_pred contain negative values.")
            return np.nan
        ...
        ret = np.sqrt(sklearn.metrics.mean_squared_log_error(
            ...
            sample_weight=self._sample_weight, multioutput='uniform_average'))
        if np.isnan(ret) or np.isinf(ret):
            return constants.REGRESSION_RANGES[constants.RMSLE][1]
        return ret
...
class NormRMSLE(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        if self._has_negatives:
            self._logger.warning("Skipping NormRMSLE calculation. y_test/y_pred contain negative values.")
            return np.nan

        if self._y_min == self._y_max:
            self._logger.warning("Skipping NormRMSLE calculation. All targets are equal.")
            return constants.REGRESSION_RANGES[constants.NORM_RMSLE][1]
        ...
        return np.clip(norm_ret, *constants.REGRESSION_RANGES[constants.NORM_RMSLE])
