...
class _MultiGrainForecastBase:
    ...
    def fit(self,
        ...
        if num_par_process > 1:
            ...
            # Parse results received from Parallel
...
class SeasonalNaive(_LowCapacityModelFitMixin, _MultiGrainForecastBase):
    ...
    def __init__(self, **kwargs):
        ...
        :param seasonality:
            the seasonal period that is defined by recurring patterns in the data.
        """
        timeseries_param_dict = kwargs[constants.TimeSeriesInternal.TIMESERIES_PARAM_DICT]
        super().__init__(timeseries_param_dict)
        seasonality = timeseries_param_dict.get('seasonality')
...
class SeasonalAverage(_LowCapacityModelFitMixin, _MultiGrainForecastBase):
    ...
    def __init__(self, **kwargs):
        ...
        timeseries_param_dict = kwargs[constants.TimeSeriesInternal.TIMESERIES_PARAM_DICT]
        super().__init__(timeseries_param_dict)
        window_size = timeseries_param_dict.get('seasonality')
        if window_size is None:
            # We will need to set this to the length of the training data when we receive the data.
