...
class R2(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        # r2 is a degenerate metric when passed a single value. scikit-learn <=0.20 would
        # handle this by setting the metric to 0 or 1. scikit-learn >0.20 now returns nan and raises a warning.
        # See: https://github.com/scikit-learn/scikit-learn/pull/12855 and
        # https://scikit-learn.org/stable/whats_new/v0.21.html#changelog. In order to maintain previous
        # functionality, if the length of y_test is 1, we will return 0 if the values differ, else 1.
        if self._y_test.shape[0] == 1:
            ret = float(self._y_test == self._y_pred)
