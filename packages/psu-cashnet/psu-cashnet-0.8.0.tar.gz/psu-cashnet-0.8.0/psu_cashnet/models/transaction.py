...
class Transaction(models.Model):
    ...
    status_code = models.CharField(
        ...
        blank=False, null=False, default='N'
    )
    ...
    def cashnet_url(self):
        if utility_service.is_non_production():
            if utility_service.get_setting("CASHNET_SIMULATE_TRANSACTION"):
                url = reverse('cashnet:fake_cashnet') + '?'
            elif utility_service.is_development():
                url = reverse('cashnet:fake_cashnet') + '?'
            else:
                url = f"https://train.cashnet.com/{self.merchant_code}?"
        else:
            url = f"https://commerce.cashnet.com/{self.merchant_code}?"
    ...
    def cashnet_parameters(self):
        ...
        def add_one_item(seq, fee):
            ...
            if item.catalog_item.gl:
                parameters.update({f"gl{ii}": '{0:.2f}'.format(item.catalog_item.gl)})
