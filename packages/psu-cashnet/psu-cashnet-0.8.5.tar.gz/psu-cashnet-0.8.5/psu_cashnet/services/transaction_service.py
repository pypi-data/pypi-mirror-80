    ...
    try:
        ...
        try:
            if type(success_url) is tuple:
                tx.success_url = reverse(success_url[0], args=list(success_url[1:]))
            else:
        ...
        try:
            if type(failure_url) is tuple:
                tx.failure_url = reverse(failure_url[0], args=[list(failure_url[1:])])
            else:
...
def record_cashnet_response(request):
    ...
    payment_completed = result is not None and int(result) == 0
...
def confirm_cashnet_response(tx_id):
    ...
    try:
        ...
        if tx and tx.customer_username != user.username:
            tx.internal_error_message = f"Authenticated user ({user.username}) is not the customer"
            tx.status_code = 'E'
            tx.save()
            return tx

        # If recorded transaction belongs to current user
        if tx and user.username == tx.customer_username and tx.is_recorded():
...
def get_processed_transaction():
    ...
    if tx_id:
        return Transaction.objects.get(pk=tx_id)
    else:
        return None
