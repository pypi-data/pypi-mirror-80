...
class Depro():
    ...
    def estimate(self,method="parallel",scree_plot=False,**kwargs):
        ...
        X = self.data.get_data()["X"] # 因子数推定時にはmirrorは考慮せず, 単にrawからzをとる. sphereは考慮する.  
    ...
    def decompose(self,initialize="principal_component",rotate="varimax",nfactor=None,**kwargs):
        ...
        if self.get_state()["mirror"]:
            outs = self.data.get_processed()["outlier"]
            nsample = int((X.shape[1] + len(outs))/2)
            L = L[:,:nsample]
            F = F[:nsample,:nsample]
            C = C[:nsample]
