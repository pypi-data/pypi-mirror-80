...
class MagneticStirrer(Runnable):
    ...
    def __init__(self,
        ...
        self.state_columns = [self.time_column_heading,
        ...
        self.command_columns = [self.time_column_heading,
                                self.time_s_column_heading,
                                self.time_min_column_heading,
                                self.time_hour_column_heading,
                                self.command_column_heading]
    ...
    def set_up_save_files(self, save_path: Path):
        ...
        index = 1
        while self.state_csv_path.exists():
            self.save_path = self.save_path.parent.joinpath(f'{file_name}_copy_{index}')
            new_file_name = self.save_path.name
            self.state_csv_path = self.save_path.with_name(f'{new_file_name}_state_data.csv')
            self.command_csv_path = self.save_path.with_name(f'{new_file_name}_command_data.csv')
            index += 1
        self.state_data: pd.DataFrame = None
        self.command_data: pd.DataFrame = None
        self.set_up_data()
    ...
    def set_up_command_data(self, start_time) -> None:
        ...
        :return: None
        """
        df = pd.DataFrame(columns=self.command_columns)
        add_data = {self.time_column_heading: start_time,
                    self.time_s_column_heading: 0,
                    self.time_min_column_heading: 0,
                    self.time_hour_column_heading: 0,
                    self.command_column_heading: 'START',
                    }
        df = df.append(add_data, ignore_index=True)
