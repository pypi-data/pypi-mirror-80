...
def create_profile(filename, search_criteria):
    ...
    content = textract.process(filename).decode()
    ...
    doc = nlp(content+" sarimax"+ " cnn" + " cnn")
...
def run_dash(path, search_criteria, mode=None):
    ...
    def toggle_navbar_collapse(n):
        if n is None:
            ...
            return [{"name": i, "id": i} for i in df.columns], df.to_dict('records'), str(len(files))+" Resumes found"
