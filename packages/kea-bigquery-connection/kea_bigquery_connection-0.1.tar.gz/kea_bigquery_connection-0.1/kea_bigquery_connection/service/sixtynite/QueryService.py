...
class QueryService(BaseService):
    ...
    def getBigQueryData(self,queryRQ):
        ...
        path = self.getPath()
        try:
            data = QueryModel(url,path).getData(self.getDataset(),self.getTable(),queryRQ.getFields(),queryRQ.getConcats(),queryRQ.getConditions(),queryRQ.getUnnest(),queryRQ.getLimit())
            if data is not None:
