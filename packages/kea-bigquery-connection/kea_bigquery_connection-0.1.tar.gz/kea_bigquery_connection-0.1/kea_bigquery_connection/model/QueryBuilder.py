...
class QueryBuilder(BaseService):

    def __init__(self,dataset, table):
        self.dataset = dataset
        self.table= table

    def render_query(self,select=None, conditions=None,concats=None,
                    groupings=None, having=None, order_by=None, limit=None,unnest=False):
        ...
        query = "%s %s %s %s %s %s %s %s" % (
            self._render_select(select),
            self._render_concat_select(concats),
            self._render_sources(dataset, tables,unnest),
            self._render_conditions(conditions),
            self._render_groupings(groupings),
            self._render_having(having),
            self._render_order(order_by),
            self._render_limit(limit)
        )

        return query


    def _render_select(self,selections):
    ...
    def _format_select(self,formatter, name):
    ...
    def _render_sources(self,dataset, tables,unnest):
        ...
        if isinstance(tables, dict):
            ...
            if unnest is False:
                return "FROM " + ", ".join(
                    ["`%s.%s` ec " % (dataset, table) for table in tables])
            else:
                return "FROM " + ", ".join(
                    ["`%s.%s` ec ,unnest(event_params) as e" % (dataset, table) for table in tables])
    ...
    def _render_conditions(self,conditions):
        ...
        for condition in conditions:
            ...
            if None in (field, field_type, comparators) or not comparators:
                print('Invalid condition passed in: %s' % condition)
                continue

            rendered_conditions.append(
                self._render_condition(field, field_type, comparators))
    ...
    def _render_condition(self,field, field_type, comparators):
        ...
        for comparator in comparators:
            ...
            if condition == "IN":
                ...
                value = self._render_condition_value(value, field_type)
    ...
    def _render_condition_value(self,value, field_type):
    ...
    def _render_groupings(self,fields):
    ...
    def _render_having(self,having_conditions):
    ...
    def _render_concat_select(self,concats):
        ...
        for c in concats:
            ec = "ec"+str(i)
            if len(c)>2:
                concat = concat+",array(Select e.value.{} from `".format(c[0])+dataset+"."+table[0]+"` "+ec+", unnest(event_params) as e where event_name = '{}' and e.key = '{}' and ".format(c[1],c[2])+ec+".event_date=ec.event_date and "+ec+".event_timestamp = ec.event_timestamp)[SAFE_OFFSET(0)]"
            else:
                concat = concat+",array(Select e.value.{} from `".format(c[0])+dataset+"."+table[0]+"` "+ec+", unnest(event_params) as e where e.key = '{}' and ".format(c[1])+ec+".event_date=ec.event_date and "+ec+".event_timestamp = ec.event_timestamp)[SAFE_OFFSET(0)]"
            i=i+1
        return concat
