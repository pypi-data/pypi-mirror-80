...
class QueryModel(AbstractModelResource):
    ...
    def getData(self,dataset,table,fields,concats,conditions,unnest,limit):
        ...
        if limit is not None:
            q = query_builder.render_query(select=selects,concats=concats,conditions=conditions,unnest=unnest,limit=limit)
        else:
            q = query_builder.render_query(select=selects,concats=concats,conditions=conditions,unnest=unnest)
        try:
