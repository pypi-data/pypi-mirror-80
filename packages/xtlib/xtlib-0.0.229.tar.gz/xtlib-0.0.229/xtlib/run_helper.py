...
run_info_props = {"_id": 1, "app_name": 1, "box_name": 1, "compute": 1, "create_time": 1, "description": 1, "exper_name": 1,
    ...
    "xt_build": 1, "xt_version": 1}

run_stats_props = {"status": 1, "start_time": 1, "last_time": 1, "exit_code": 1, "restarts": 1, 
    "end_time": 1, "queue_duration": 1, "run_duration": 1, "end_id": 1, "tags": 1, "metrics_names": 1}
...
def expand_job_entry(store, mongo, workspace, run_list, job_term):
    '''
    Arguments:
        store: an instance of Store (access to storage) 
        mongo: an instance of Mongo (access to related mongoDB)
    ...
    job_list, actual_ws = job_helper.expand_job_list(store, mongo, workspace, [job_term], can_mix=False)

    run_filter = {"job_id": {"$in": job_list}}
    records = mongo.get_info_for_runs(workspace, run_filter, {"_id": 1, "run_num": 1})
    if records:
        # records are not in any order from mongo, so sort by run_num
...
def set_run_tags(store, mongo, name_list, tag_list, workspace, fd, clear):
    ...
    if run_list:
        ...
        if len(run_list)==1 and run_list[0] == "*":
            # update ALL records in ws
            all_ws_runs = mongo.get_info_for_runs(workspace, {"ws_name": workspace}, {"_id": 1})
            run_list = [run["_id"] for run in all_ws_runs]
        else:
            # convert simple run_name into id required by storage version
            run_list = [store_utils.make_id(workspace, run) for run in run_list]

        matched_count = mongo.update_run_stats_from_list(workspace, run_list, fd, clear=clear, upsert=False)
...
def list_run_tags(store, mongo, ws_name, name_list, tag_list):
    run_list, actual_ws = expand_run_list(store, mongo, ws_name, name_list)

    if run_list:
        filter_dict = {"run_name": {"$in": run_list}}
        #fields_dict = {"tags": 1}
        fields_dict = {"run_stats": 1}

        records = mongo.get_info_for_runs(ws_name, filter_dict, fields_dict)
...
def get_run_property_dicts():
    ...
    user_to_actual = {"box": "box_name", "created": "create_time", "child": "is_child", "cluster": "cluster",
        ...
        "service_type": "service_type", "sku": "sku", "status": "status", "target": "compute", 
...
def build_run_filter_dict(ws_name, run_list, user_to_actual, builder, args):
    fd = {}
    ...
    if run_list:
        # filter by specified run names
        # convert simple run_name into id required by storage vx
        run_list = [store_utils.make_id(ws_name, run) for run in run_list]
...
def get_filtered_sorted_limit_runs(store, config, show_gathering, col_dict=None, preserve_order=False, 
    ...
    filter_dict = build_run_filter_dict(workspace, pure_run_list, user_to_actual=user_to_actual, 
    ...
    records, using_default_last, last = builder.get_mongo_records(mongo, filter_dict, workspace, which="runs", 
...
def wrapup_runs_by_node(store, workspace, job_id, node_id, node_cancel_result):
    '''
    ...
    if store.does_job_file_exist(workspace, job_id, constants.FN_MULTI_RUN_CONTEXT):
        ...
        # legacy job (need to download xt_code.zip and extract MRC file)
    ...
    filter_dict = {"job_id": job_id, "node_index": node_index}
    fields_dict = {"status": 1, "run_index": 1, "run_name": 1 }
