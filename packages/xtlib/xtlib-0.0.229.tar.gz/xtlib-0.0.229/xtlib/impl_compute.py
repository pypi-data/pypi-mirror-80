...
class ImplCompute(ImplBase):
    ...
    def monitor_job_node(self, workspace, job_id, sleep, node_index, log_name, escape):
        ...
        if ch == constants.ESCAPE:
            ...
            if ch == "y":
                self.cancel_job(job_id, workspace)
    ...
    def cancel_job(self, job_names, workspace):

        for job_id in job_names:
            console.print("cancelling {}:".format(job_id))

            service_job_info, service_info_by_node, backend = job_helper.get_service_job_info(self.store, self.core, workspace, job_id)
            ...
            for node_id, result in result_by_node.items():
                runs = run_helper.wrapup_runs_by_node(self.store, workspace, job_id, node_id, result)
    ...
    def run(self, args):
        ...
        if monitor == "same":
            ...
            cmd = "xt --echo monitor {} --workspace={}".format(job_id, workspace)
