...
class AzureBatch(BackendBase):
    ...
    def wrap_cmd_for_batch(self, user_cmd, node_id, store_data_dir, data_action, data_writable, 
        ...
        self.add_mount_cmds(cmds, False, storage_name, storage_key, 
            actions, data_action, model_action, store_data_dir, store_model_dir, data_writable, model_writable,
            use_username=False, use_allow_other=False, args=args)
    ...
    def submit_job(self, job_id, job_runs, workspace, compute_def, resume_name, 
        ...
        for n, run_data_list in enumerate(job_runs):    # run_data_list_by_node):
            ...
            node_id = "node" + str(n)
            batch_job_id = self.make_batch_job_id(ws_name, job_id)

            service_info_by_node[node_id] = {"workspace": ws_name, "job_id": job_id, "batch_job_id": batch_job_id, 
                "node_id": node_id, "task_index": n, "merge_batch_logs": merge_batch_logs}
    ...
    def prep_node(self, job_id, node_index, run_data_list, num_nodes, use_low_pri, using_hp,  
        ...
        node_output_files = self.build_output_files(workspace, after_blob_path, output_file_list)
    ...
    def cancel_runs_by_user(self, ws_name, box_name):
        ...
        if job_ids:
            ...
            job_records = mongo.get_info_for_jobs(ws_name, filter_dict, fields_dict)
   ...
   # BACKEND API
    def log_file_iterator(self, service_node_info, log_name, start_offset=0, end_offset=None, 
        ...
        if start_offset == 0:
            ...
            if self.store.does_job_file_exist(ws_name, job_id, job_path):
    ...
    def read_log_file(self, service_node_info, log_name, start_offset=0, end_offset=None, 
        ...
        if not found_file:
            ...
            try:
                stream = self.batch_client.file.get_from_task(batch_job_id, task.id, file_path=log_name, 
                    file_get_from_task_options=gft_opts)
