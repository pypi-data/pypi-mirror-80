...
class Run():
    ...
    def __init__(self, config=None, store=None, xt_logging=True, aml_logging=True, checkpoints_enabled=True,
        ...
        if self.xt_logging and self.direct_run and self.store:
            ...
            if self.store.mongo:
                ...
                if self.context:
                    self.store.mongo.job_run_start(self.ws_name, self.context.job_id)
    ...
    def close(self):
        ...
        self.store.mongo.on_run_close()

        if self.store and self.store.mongo:        
            print("RUN: mongo call stats")
    ...
    def log_metrics(self, data_dict, step_name=None, stage=None):
        ...
        if self.store and self.xt_logging:
            ...
            if metric_names_changed:
                ...
                if self.store.mongo:
                    self.store.mongo.update_run_info(self.ws_name, self.run_name, ddx)
