...
def store_from_context(context):
    ...
    # build mongo_options from context
    mongo_options = {}
    mongo_options["update-job-stats"] = context.update_job_stats
    mongo_options["update-run-stats"] = context.update_run_stats
    mongo_options["add-log-records"] = context.add_log_records
    mongo_options["buffer-metrics"] = context.buffer_metrics
    
    store = Store(context.store_creds, provider_code_path=context.store_code_path, 
        mongo_conn_str=context.mongo_conn_str, mongo_options=mongo_options)
...
# public function
def Store(storage_creds=None, config=None, max_retries=25, run_cache_dir=None, feedback_enabled=True,  mongo_conn_str=None, 
        provider_code_path=None, validate_creds=False, mongo_options=None):
    '''
    ...
    if use_v1:
        ...
        store = StoreV1(storage_creds, config, max_retries, run_cache_dir, feedback_enabled,  mongo_conn_str, 
        ...
        store = StoreV2(storage_creds, config, max_retries, run_cache_dir, feedback_enabled,  mongo_conn_str, 
            provider_code_path, validate_creds, mongo_options=mongo_options)
...
class StoreV2():
    ...
    def __init__(self, storage_creds=None, config=None, max_retries=25, run_cache_dir=None, feedback_enabled=True,  mongo_conn_str=None, 
        provider_code_path=None, validate_creds=False, mongo_options=None):
        '''This is the constructor for the Store class. '''

        # # debug issue of using correct storage account on compute node
        ...
        self.max_log_workers = utils.safe_value(mongo_options, "max-log-workers")
        ...
        if mongo_conn_str:
            ...
            self.mongo = mongo_v2.MongoDB2(mongo_conn_str, run_cache_dir=run_cache_dir, store=self, 
    ...
    def ensure_workspace_exists(self, ws_name, flag_as_error=True):
        ...
        if True:  # created:
            self.mongo.create_workspace_if_needed(ws_name)
    ...
    def create_workspace(self, ws_name, database=None):
        ...
        self.mongo.create_workspace_if_needed(ws_name, database)
    ...
    def delete_workspace(self, ws_name):
        ...
        if result:
            ...
            result2 = self.mongo.delete_workspace_if_needed(ws_name)
            console.print("delete_workspace: result from mongo: {}".format(result2))
    ...
    def get_run_names(self, ws_name):
        ...
        # use mongo for faster results
        #return self.helper.get_run_names(ws_name)

        records = self.mongo.get_info_for_runs(ws_name, {"ws_name": ws_name}, {"run_name": 1})
    ...
    def start_run_core(self, ws_name, run_name, exper_name=None, description=None, username=None,
            ...
            sku=None, search_style=None, is_child=False, run_index=None):
        ...
        # this call will also create run in mongo, as appropriate
    ...
    def end_run(self, ws_name, run_name, status, exit_code, hparams_dict, metrics_rollup_dict, end_time=None, 
         aggregate_dest=None, dest_name=None, is_aml=False):
    ...
    def rollup_and_end_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        if self.mongo:
            self.mongo.update_run_at_end(ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics)
    ...
    def log_run_event(self, ws_name, run_name, event_name, data_dict=None, event_time=None, is_aml=False):
        ...
        if self.mongo:
            ...
            self.mongo.process_run_event(ws_name, run_name, event_name, data_dict, record_dict)
    ...
    def wrapup_run(self, ws_name, run_name, aggregate_dest, dest_name, status, exit_code, primary_metric, maximize_metric, 
        ...
        if self.mongo:
            ...
            if not is_parent:
                ...
                self.mongo.job_run_exit(ws_name, job_id, exit_code)
    ...
    def update_run_at_end(self, ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics):
        ...
        if self.mongo:
            result = self.mongo.update_run_at_end(ws_name, run_name, status, exit_code, end_time, log_records, hparams, metrics)
    ...
    def read_log_records(self, ws_name, run_name):
        ...
        # make this look like a mongo record so consuming code is happy
    ...
    def get_log_records_for_runs(self, ws_name, id_list):
        # use worker processes to make this faster
        ...
        if id_list:
            started = time.time()
            pbar = None
            num_workers = min(len(id_list), self.max_log_workers)

            use = "threads"   # direct, pool, threads, queue
            # if use == "direct": 
            #     num_workers = 0

            print("  reading log_records from storage (use={}, {} runs, {} workers)".format(use, len(id_list), num_workers))

            if len(id_list) > 2:
                pbar = tqdm.tqdm(total=len(id_list), ascii=True, ncols=80, desc="  runs: ")

            # THREADS method
            threads = []
            thread_lock = Lock()   

            def thread_worker(ws_name, ids):
                for id in ids:
                    run_name = id.split("/")[-1]
                    results = self.read_log_records(ws_name, run_name)
                    with thread_lock:
                        nonlocal all_run_records
                        all_run_records += results
                        if pbar:
                            pbar.update(1)

            # split id_list into num_workers sub-lists
            per_list = math.ceil(len(id_list)/num_workers)
            next_index = 0

            for id in id_list:  
                # build sub-list
                ids = id_list[next_index:next_index+per_list]
                next_index += per_list

                thread = Thread(target=thread_worker, args=(ws_name, ids))
                thread.start()
            ...
            # elif use == "pool" and num_workers: 
            #     # POOL method
            #     # build workitems to interate over
            #     work_items = []
            #     config = xt_config.get_merged_config()    

            #     for id in id_list:
            #         run_name = id.split("/")[-1]
            #         work_items.append( [config, ws_name, run_name] )

            #     pool = Pool(processes=num_workers) 

            #     res = [pool.apply_async(worker_read_log_records, args=(item,), callback=lambda _: pbar.update(1)) \
            #         for item in work_items]

            #     runs = [p.get() for p in res]

            #     # # sort them according to id_list (since results were collected as they completed)
            #     # runs.sort(key=lambda r: r["run_name"])

            #     # flatten records into single list
            #     for run in runs:
            #         all_run_records += run["records"]

            # elif use == "direct":
            #     # DIRECT method
            #     # read one at a time in main process
            #     for id in id_list:  
            #         run_name = id.split("/")[-1]
            #         records = self.read_log_records(ws_name, run_name)
            ...
            console.print("  use: {}, elapsed: {:.2f} secs".format(use, elapsed))
    ...
    def create_job(self, ws_name):
        job_num = self.mongo.get_next_job_id(ws_name, "next_job_number")
    ...
    def get_job_secret(self, ws_name, job_id):
        ...
        records = self.mongo.get_info_for_jobs( filter_dict={"_id": key}, fields_dict={"job_secret": 1} )
    ...
    def log_job_info(self, ws_name, job_id, dd):
        ...
        self.mongo.create_mongo_job(dd)
