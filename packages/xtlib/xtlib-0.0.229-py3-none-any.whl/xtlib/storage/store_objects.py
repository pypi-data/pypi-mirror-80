...
class StoreBlobObjs():
    ...
    def validate_storage_and_mongo(self, mongo):
        ...
        if not paired_storage:
            mongo_info = {"paired_storage": storage_name, "storage_format": constants.STORAGE_FORMAT_V2}
            mongo.set_mongo_info(mongo_info)

        if not paired_mongo:
            storage_info = {"paired_mongo": connected_mongo, "storage_format": constants.STORAGE_FORMAT_V2}
            self._set_storage_info(storage_info)

        if mongo_info["storage_format"] != constants.STORAGE_FORMAT_V2:
            errors.store_error("XT mongo format {} expected but found: {}".format(constants.STORAGE_FORMAT_V2, mongo_info["storage_format"]))
    ...
    def job_files(self, ws_name, job_name, use_blobs=False):
        if use_blobs:
            ...
            return store_azure_file.JobFiles(self, ws_name, job_name)
