...
class MongoDB2():
    ...
    def __init__(self, mongo_cs, db_name=None, ws_name=None, run_cache_dir=None, 
        direct_mode=True, store=None, mongo_options=None):
        ...
        if mongo_options:
            self.set_mongo_options(mongo_options)
    ...
    def mongo_with_retries(self, name, mongo_cmd, ignore_error=False, return_records=False, return_single_record=False):
        ...
        entry = {"elapsed": elapsed, "actual_call_time": actual_call_time, "retry_count": i}
    ...
    def get_collection_records(self, collection, ws_name, filter, fields=None):
        ...
        records = self.mongo_with_retries("get_collection_records", lambda: dbx[collection].find(filter, fields), 
            return_records=True)

        # support for mongo.add-log-records == False
        if collection == "log_records":
    ...
    def update_collection(self, collection_name, ws_name, dd, dbx=None, is_flat=True, flat_exceptions=[]):
        ...
        self.mongo_with_retries(my_name, lambda: dbx[collection_name].update_one(filter=filter, update=update, upsert=True) )
    ...
    def on_run_close(self):
        if self.metrics:
            ...
            ws_name = self.metrics["_id"].split("/")[0]
    ...
    def update_run_stats_from_list(self, ws_name, id_list, dd, clear=False, upsert=True):
        ...
        # result = self.mongo_with_retries("update_run_stats_from_list", \
        #     lambda: dbx["run_stats"].update_many( update_filter, update_doc, upsert=upsert) )
        ...
        for id in id_list:
            ...
            result = self.mongo_with_retries("update_run_stats_from_list", \
                lambda: dbx["run_stats"].update_one( update_filter, update_doc, upsert=upsert) )
    ...
    def update_job_stats_from_list(self, ws_name, id_list, dd, clear=False, upsert=True):
        '''
        Update 1 or more job_stats documents from a single set of dd properties.
        Mostly used to update tags on jobs.
        '''
        dbx = self.get_db(ws_name)
        ...
        #    lambda: dbx["job_stats"].update_many( update_filter, update_doc, upsert=upsert) )
        ...
        for id in id_list:
            ...
            result = self.mongo_with_retries("update_job_stats_from_list", \
                lambda: dbx["job_stats"].update_one( update_filter, update_doc, upsert=upsert) )
    ...
    def get_all_experiments_in_ws(self, ws_name):
        ...
        records = self.mongo_with_retries("get_all_experiments_in_ws", lambda: \
            dbx["job_info"].find({"ws_name": ws_name}, {"exper_name": 1}), return_records=True)
...
class MongoRunIndex():
    ...
    '''
    def __init__(self, mongo, ws_name, job_id, parent_run_name, node_id):
        self.ws_name = ws_name
        self.mongo = mongo
        self.job_id = job_id
