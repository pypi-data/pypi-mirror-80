...
class ReportBuilder():
    ...
    def flatten_records(self, records, sort_col, need_alive, args):
        '''
    ...
    def get_mongo_records(self, mongo, filter_dict, workspace, which, actual_to_user,
        ...
        if which == "runs":
            records = mongo.get_filtered_sorted_run_info(workspace, filter_dict, col_dict, sort_col, sort_dir, skip, first, 
                count_runs, buffer_size)
        else:
            records = mongo.get_filtered_sorted_job_info(workspace, filter_dict, col_dict, sort_col, sort_dir, skip, first, 
                count_runs, buffer_size)

        round_trip_count = mongo.round_trip_count
        ...
        if flatten_records:
            ...
            records = self.flatten_records(records, sort_col, need_alive, args)
    ...
    def should_highlight_row(self, highlight_exp, rd):
        ...
        if highlight_exp == "$alive":
            ...
            should = status in ["queued", "running"]
