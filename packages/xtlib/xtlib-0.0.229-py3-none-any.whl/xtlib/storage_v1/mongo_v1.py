...
import xtlib.storage_v1.mongo_run_index as mongo_run_index
...
class MongoDB():
    ...
    def process_run_event(self, ws_name, run_name, log_record):
    ...
    def update_run_info(self, ws_name, run_id, orig_dd, clear=False, upsert=True, update_primary=False):
        ...
        if self.update_run_stats:
            if clear:
                ...
                dd["last_time"] = utils.get_time()
                update_doc = { "$set": dd}

            # update, create prop if needed
            self.mongo_with_retries("update_run_info", \
                lambda: self.mongo_db[ws_name].update_one( {"_id": run_id}, update_doc, upsert=upsert) )


    # def update_run_info(self, ws_name, run_name, dd):
    #     #console.print("update_run_info: ws_name={}, run_name={}, dd={}".format(ws_name, run_name, dd))
    ...
    #         #console.print("update_run_info: ws_name={}, run_name={}, update_doc={}".format(ws_name, run_name, update_doc))

    #         # do a REPLACE DOC update operation
    #         self.mongo_with_retries("update_run_info", lambda: self.mongo_db[ws_name].update_one( {"_id": run_name}, update_doc) )
    ...
    def get_filtered_sorted_run_info(self, ws_name, filter_dict, col_dict, sort_col, sort_dir, skip, first, 
            count_runs, buffer_size):

        self.v2_to_v1_run_cols(filter_dict)
        self.v2_to_v1_run_cols(col_dict)

        records = self.get_filtered_sorted_container(ws_name, filter_dict, col_dict, sort_col, sort_dir, skip, first, 
            count_runs, buffer_size)
    ...
    def update_run_stats_from_list(self, ws_name, id_list, dd, clear=False, upsert=True):
        ...
        if self.update_run_stats:
            ...
            update_filter = {"_id": {"$in": id_list}}

            # update, create prop if needed
            result = self.mongo_with_retries("update_run_stats_from_list", lambda: \
                self.mongo_db[ws_name].update_many( update_filter, update_doc, upsert=upsert) )

            return result

    #---- JOBS ----

    def update_job_stats_from_list(self, ws_name, id_list, dd, clear=False, upsert=True):
        ...
        if self.update_job_stats:
            ...
            result = self.mongo_with_retries("update_job_stats_from_list", \
                lambda: self.mongo_db["__jobs__"].update_many( update_filter, update_doc, upsert=upsert) )
            return result
    ...
    def get_filtered_sorted_container(self, container, filter_dict, col_dict, sort_col, sort_dir,
        ...
        cursor = self.mongo_with_retries("get_mongo_records", fetch)
    ...
    def get_filtered_sorted_job_info(self, workspace, filter_dict, col_dict, sort_col, sort_dir, skip, first, 
            count_runs, buffer_size):

        self.v2_to_v1_job_cols(filter_dict)
        self.v2_to_v1_job_cols(col_dict)

        records = self.get_filtered_sorted_container("__jobs__", filter_dict, col_dict, sort_col, 
            sort_dir, skip, first, count_runs, buffer_size)
    ...
    def run_start(self, ws_name, run_name):
        ...
        if self.update_run_stats:
            ...
            run_restarted = bool(queue_duration) or restarts > 0
            if run_restarted:
    ...
    def build_child_runs_schedule_data(self, total_run_count, node_count):
        active_runs = mongo_run_index.build_active_runs("static", total_run_count, node_count)
        return active_runs, "active_runs"

    def create_child_run_mgr(self, ws_name, job_id, parent_run_name, node_id):
        return mongo_run_index.MongoRunIndex(self, job_id, parent_run_name, node_id)
