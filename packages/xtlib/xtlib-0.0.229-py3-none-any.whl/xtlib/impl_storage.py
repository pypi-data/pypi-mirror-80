...
class ImplStorage(ImplBase):
    ...
    def create_workspace(self, workspace, database):
    ...
    def delete_workspace(self, workspace, response):
        '''
        delete specified workspace from both storage and mongo.  
        '''
        deleted = False

        # delete workspace even when it only exists on 1 of storage or mongo
        
        if not self.store.does_workspace_exist(workspace):

            # cleanup workspace if mongo, if needed
            if self.store.mongo.delete_workspace_if_needed(workspace):
                console.print("workspace (found only in mongo) deleted: " + workspace)
            else:
    ...
    def export_workspace(self, output_file, workspace, tags_all, tags_any, jobs, experiment, response):
        self.impl_storage_api.export_workspace(output_file, workspace, tags_all, tags_any, jobs, experiment, 
            show_output=True, response=response)
    ...
    def view_metrics(self, runs, workspace, steps, merge, metrics, hparams, export, clean):
        ...
        pure_run_list, ws_name = run_helper.expand_run_list(self.store, self.store.mongo, workspace, runs)
        ...
        run_log_records = self.store.mongo.get_info_for_runs(ws_name, fd, fields, include_log_records=True)
    ...
    @option(name="max-workers", type=int, default="$mongo.max-log-workers", help="the maximum number of background workers for reading log records")
    ...
    def plot(self, runs, col_list, x, layout, break_on, title, show_legend, plot_titles, legend_titles, 
        ...
        sort, reverse, alpha, line_size, timebase, x_format, y_format, buffer_size, 
        ...
        args = {"run_list": runs, "workspace": workspace, "all": True, "parent": False, 
        ...
        if len(run_log_records) == default_max_runs:
        ...
        if extract:
            ...
            for run_name in pure_run_list:
                ...
                if nodes == "exper":
                    ...
                    record = self.store.mongo.get_info_for_runs(actual_ws, {"_id": id}, {"job_id": 1})
                    job_id = utils.safe_cursor_value(record, "job_id")
    ...
    def explore(self, aggregate_name, workspace, cache_dir, steps_name, log_interval_name, 
        ...
        if job_helper.is_job_id(dest_name):
            ...
            records = self.store.mongo.get_info_for_jobs(workspace, filter_dict, fields_dict)
    ...
    def view_mongo(self, name, workspace, log_records):
        ...
        if run_helper.is_run_name(name):
            ...
            fields_dict = {"run_info": 1, "run_stats": 1, "hparams": 1, "metrics": 1, "log_records": log_records}
            mongo_docs = self.store.mongo.get_info_for_runs(ws, filter_dict, fields_dict)
            ...
            fields_dict = {"job_info": 1, "job_stats": 1, "connect_info": 1, "service_info_by_node": 1, 
                "hparams": 1}

            mongo_docs = mongo.get_info_for_jobs(workspace, filter_dict, fields_dict)
    ...
    def list_tags(self, name_list, tag_list, workspace):
        ...
        if job_helper.is_job_id(first_name):
            # jobs
            job_helper.list_job_tags(self.store, mongo, workspace, name_list, tag_list)

        elif run_helper.is_run_name(first_name):
            # runs
            run_helper.list_run_tags(self.store, mongo, workspace, name_list, tag_list)
