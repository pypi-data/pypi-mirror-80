...
def _get_error_details(exception: BaseException, logger: Union[logging.Logger, logging.LoggerAdapter]) -> \
        Tuple[str, str, str]:
    """
    ...
    if isinstance(exception, AzureMLException):
        try:
            ...
            error_code = str(error.get("inner_error")) or ErrorTypes.Unclassified
            ...
            logger.warning("Failed to parse error details while logging traceback from exception of type {}".format(
                exception))
            # revert to defaults
