...
from jupyhai.handlers.download import DownloadHandler
...
def prepare(nb_server_app: NotebookApp):
...
def install_handlers(nb_server_app: NotebookApp):
    ...
    web_app.add_handlers(
        ...
        [
            (
                url_path_join(base_url, '/jupyhai/download/(.*)'),
                DownloadHandler,
                {'root_dir': root_dir},
            )
        ],
