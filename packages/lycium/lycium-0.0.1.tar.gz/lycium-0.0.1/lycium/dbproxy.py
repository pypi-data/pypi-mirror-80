...
class DbProxy(object):
    ...
    def setup_rdbms_connection(self, category: str, dbconf: dict) -> bool:
        ...
        conndsn = self.formatConnectionString(dbconf)
    ...
    def setup_mongodb_elenment(self, category: str, dbconf: dict) -> bool:
        mongo_cli = motor.motor_tornado.MotorClient(self.formatConnectionString(dbconf))
    ...
    def formatConnectionString(self, dbconf: dict) -> str:
        ...
        if engine == 'oracle':
            ...
            connection_str = "oracle://%s:%s@%s" % (user, pwd, dsnStr)
            # connection_str = "oracle://%s:%s@%s:%s/%s" % (user, pwd, host, port, dbname)
        else:
            connection_str = '%s%s://%s:%s@%s:%d/%s' % (engine, driverpart, user, pwd, host, port, dbname)
