...
class _WorkerDelegateType(_Constant):
    """
    """
    CONSUMER = 1
    PUBLISHER = 2
...
class _RabbitMQDefaults(_Constant):
    """
    """
    SOCKET_TIMEOUT = 15
...
class WorkerDelegate(object):
    ...
    def executor(self, callback, *args, **kwargs):
...
class PublishesMessage(object):
    ...
    def __init__(self, exchange, routing_key, body, properties=None):
...
class RabbitMQWorker(object):
    ...
    def setup_queue(self, worker_cfg):
    ...
    def on_bindok(self, unused_frame, worker_cfg):
    ...
    def on_message(self, ch, basic_deliver, properties, body):
        ...
        if worker_cfg and worker_cfg.callback:
            ...
            self._connection.ioloop.add_callback(functools.partial(worker_cfg.executor, worker_cfg.callback, ch, basic_deliver, properties, body))
    ...
    def start_worker(self, worker_cfg):
        ...
        if worker_cfg.worker_type == WORKER_TYPE.CONSUMER:
            LOGGER.info("Issuing consumer related RPC commands for queue:'%s'", worker_cfg.queue)
    ...
    def schedule_next_message(self):
        ...
        if self._connection and self._publishes:
            ...
            self._connection.ioloop.add_callback(functools.partial(self.publish_message))
        else:
            self._publish_working = False
...
class RabbitMQFactory(object):
    ...
    def consume(self, virtual_host, exchange, exchange_type, binding_key, queue, durable, callback, auto_ack = True, exchange_durable = None):
        """Consumes a message
    ...
    def initialize_publisher(self, virtual_host, exchange, exchange_type, binding_key, queue, durable, exchange_durable = None):
        """Initialize a publishing queue and exchange
    ...
    def publish(self, virtual_host, exchange, routing_key, message, properties = None):
        """publishing a message
        ...
        """
        self._instances[virtual_host].publish(exchange, routing_key, message, properties)
...
def test_amqplib():
    ...
    example.publish(virtual_host, example_exchange, example_queue, 'test')
