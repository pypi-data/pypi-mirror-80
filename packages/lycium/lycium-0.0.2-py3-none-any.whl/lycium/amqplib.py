                ...
                if resp and properties.reply_to and properties.correlation_id:
                    ...
                    if isinstance(resp, (str, bytes)):
                        if properties.correlation_id in worker._processing_replyies:
                            ...
                            reply_properties.timestamp = int(time.time() * 1000)
...
class RabbitMQWorker(object):
    ...
    def __init__(self,
        ...
        self.statistic_queue = ''
        self.statistic_message_content = False
    ...
    def on_message(self, ch: pika.channel.Channel, basic_deliver: pika.spec.Basic.Deliver, properties: pika.BasicProperties, body: bytes):
        ...
        if worker_cfg and worker_cfg.callback:
            ...
            if self.statistic_queue:
                self._connection.ioloop.add_callback(functools.partial(self._publish_statistic), body, properties)
        else:
    ...
    def _do_publish_message(self):
        ...
        for element in self._publishes:
            ...
            if callable(element.callback):
                ...
                if not element.properties.correlation_id:
                    ...
                    if element.properties.reply_to:
                        if not element.properties.headers:
                            ...
                            element.properties.headers['X-Forward-For'] = element.properties.reply_to
                element.properties.timestamp = int(time.time()*1000)
                ...
                if False == self._rpc_started:
                    self._channel.queue_declare(self._rpc_queue_name, 
                                                callback=lambda method: self._channel.basic_consume(self._rpc_queue_name, self._rpc_response)
                                               )
            ...
            self._channel.basic_publish(element.exchange, element.routing_key,
                                        ...
                                        pub_properties)
            if self.statistic_queue:
                self._connection.ioloop.add_callback(functools.partial(self._publish_statistic), element.message, element.properties)
        self._publishes = []
    ...
    @tornado.gen.coroutine
    def _publish_statistic(self, message: bytes, properties: pika.BasicProperties):
        """If the class is not stopping, publish a message to RabbitMQ,
        appending a list of deliveries with the message number that was sent.
        This list will be used to check for delivery confirmations in the
        on_delivery_confirmations method.
        ...
        """
        if not self.statistic_queue or self._channel is None or not self._channel.is_open:
            return

        self._channel.basic_publish('', self.statistic_queue,
                                    message if self.statistic_message_content else bytes('', 'utf-8'),
                                    properties)
