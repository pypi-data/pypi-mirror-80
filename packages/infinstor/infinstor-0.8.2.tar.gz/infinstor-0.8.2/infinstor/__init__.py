...
def get_label_info(label):
    ...
    url = 'https://api.' + service + '.com/listlabels'
    try:
    ...
    for one_label in labels:
        ...
        if (labelname == label):
            return one_label['timespec'], one_label['bucketname'], one_label['prefix'], service
        else:
...
def get_xform_info(xformname):
    ...
    url = 'https://api.' + service + '.com/listxforms'
    try:
        response = requests.post(url, data=payload, headers=headers)
        response.raise_for_status()
    except HTTPError as http_err:
        ...
        print(f'Other error occurred: {err}')
        raise
    xforms = response.json()['xforms']
    for xform in xforms:
        xfn = xform['xformname']
        if (xfn == xformname):
            if ('dockerfile' in xform):
                return xform['xformcode'], xform['dockerfile']
            else:
                return xform['xformcode'], None
        else:
            return None, None
...
# returns a pandas DataFrame with index 'YY-MM-dd HH:MM:SS bucketname/filename'
# and one column named RawBytes that contains the raw bytes from the object
def download_objects(client, bucketname, prefix, array_of_files, is_csv):
    with start_run() as run:
        return download_objects_inner(client, bucketname, prefix, array_of_files, is_csv,\
                None, None)
...
def test_infin_transform(service_name, input_data_spec, **kwargs):
    ...
    if (input_data_spec['type'] == 'infinsnap' or input_data_spec['type'] == 'infinslice'):
        ...
        prefix_in = '' # ignored for labels
        bucketname = '' # ignored for labels
        session = boto3.session.Session(profile_name='infinstor', Config=config)
        client = session.client('s3', endpoint_url=endpoint)
    elif (input_data_spec['type'] == 'mlflow-run-artifacts'):
        bucketname, prefix_in = get_mlflow_run_artifacts_info(input_data_spec['run_id'])
        session = boto3.session.Session()
