...
class DataBaseWorker:
    ...
    def insert_vertica(self, table_name, data, schema=None):
        """
        ...
        vertica_insert(
            cursor=cursor,
            target=(schema or self.schema) + "." + table_name,
    ...
    def insert_merge_vertica(
        ...
        self.log(sql_create_staging)
        self.conn.execute(sql_create_staging)
        try:
            self.insert_vertica(table_name, data, schema=staging_schema)
            vertica_merge(
                conn=self.conn,
