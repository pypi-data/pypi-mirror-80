...
def GetCodeObjectAtLine(module, line):
  ...
  for code_object in _GetModuleCodeObjects(module):
    lines = sorted(list(_GetLineNumbers(code_object)))

    for co_line_number in lines:
...
def _FindCodeObjectsReferents(module, start_objects, visit_recorder):
  ...
  def CheckIgnoreCodeObject(code_object):
    ...
    module_file, module_file_ext = os.path.splitext(module.__file__)

    # The simple case.
    if code_object_file == module_file:
      return False

    # If file has been pre-compiled, we only compare base names instead of full paths
    if ".pyc" == module_file_ext and \
            os.path.basename(code_object_file) == os.path.basename(module_file):
      return False
