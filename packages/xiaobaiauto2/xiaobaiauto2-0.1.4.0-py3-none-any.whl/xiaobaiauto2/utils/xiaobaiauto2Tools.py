...
def raw_convert(raw: str = None, is_xiaobaiauto2: int = 0, **kwargs) -> str:
    ...
    _code_xiaobaiauto2_import = 'try:\n\timport pytest\n' \
                          '\tfrom xiaobaiauto2 import api_action, PUBLIC_VARS\n' \
                          'except ImportError as e:\n' \
                          '\timport os\n' \
                          '\tos.popen("pip install pytest")\n\n'
    ...
    if raw:
        ...
        for i in range(len(method_list) - 1):
            raw_sub_list = raw_list[method_list[i]:method_list[i+1]]
            ...
            if is_xiaobaiauto2 == 1:
                _code += f'''\
                \r@pytest.mark.run(order={i})\
                \rdef test_xiaobai_api_{i}():\
                \r\theaders = {_headers}\
                \r\turl = '{_url}' \
                \r\tdata = '{_data}'\
                ...
                \r\t\t_re='left_template(.+?)right_template', _re_var='保存被提取值的变量名', **kwargs)\n\n\
                '''
            else:
                _code += f'''\
                \rheaders = {_headers}\
...
def api_raw():
    ...
    if params.file and os.path.isfile(params.file):
        with open(params.file, 'r', encoding='utf-8') as fr:
            code = raw_convert(raw=fr.read(), is_xiaobaiauto2=params.xiaobai)
            if params.save:
