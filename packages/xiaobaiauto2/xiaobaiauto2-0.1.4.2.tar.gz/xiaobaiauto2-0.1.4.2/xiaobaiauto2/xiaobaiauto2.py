...
def api_action(method='POST', url='', headers=None, params=None, data=None,
               json_path=None, json_assert=None, contains_assert=None,
               _re='left_template(.+?)right_template', _re_var='change_key',
               file=None, auth=None, proxies=None, verify=True, cert=None, *args, **kwargs):
    '''
    接口操作方法
    :param method: 接口请求方式
    :param url: 接口地址
    ...
    if method.lower() == 'get':
        ...
        res = request(method=method, url=url, headers=headers, data=data, verify=verify, **kwargs)
    global PUBLIC_VARS
