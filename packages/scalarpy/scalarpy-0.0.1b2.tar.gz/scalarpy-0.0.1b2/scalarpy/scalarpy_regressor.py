...
class regressor:
    ...
    def auto_regression(self,verbose=1):
        ...
        if(self.score_grid.shape[0]>1):
            self.score_grid=self.score_grid.style.apply(highlight_min,subset=pd.IndexSlice[:, ["MSE","MAE","MLE","RMSE","RMSLE"]])
            #display(type(self.score_grid))
            self.score_grid=self.score_grid.apply(highlight_max,subset=pd.IndexSlice[:, ["R2_Score"]])
        if(self.tune_grid.shape[0]>1):
            self.tune_grid=self.tune_grid.style.apply(highlight_min,subset=pd.IndexSlice[:, ["MSE","MAE","MLE","RMSE","RMSLE"]])
            #display(type(self.score_grid))
            self.tune_grid=self.tune_grid.apply(highlight_max,subset=pd.IndexSlice[:, ["R2_Score"]])
    ...
    def get_results(self):
        ...
        '''
        print("=============================Test Results===========================================")
        display(self.score_grid)
        print()
        print("======================Hyperparameter Tunning Results=================================")
        display(self.tune_grid)
