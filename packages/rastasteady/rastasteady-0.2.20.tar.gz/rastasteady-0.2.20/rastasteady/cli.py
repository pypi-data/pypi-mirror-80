...
def cli(video, debug, dual, no_rastaview, no_reusar, perfil):
    ...
    print('RastaSteady 0.2.20. Estabiliza y amplia el FOV del video de tu sistema DJI FPV.')
    print('\nProcesanto fichero %s:' % inputpathlib.name)
    ...
    print(' * Generando stabilized.mp4 usando el perfil de estabilizado \'%s\'.' % perfil)
    myVideo.stabilize(perfil)
    if not no_rastaview:
        print('\n * Generando rastaview.mp4.')
        myVideo.rastaview()
    if dual:
        print('\n * Generando dual.mp4.')
        myVideo.dual()
