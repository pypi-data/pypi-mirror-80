...
class RastaSteady:
   ...
   def stabilize(self, profile='default'):
      ...
      if not self.tmpfileexists('transforms.trf'):
          command1 = 'ffmpeg ' + self.debugopts() + ' -y -i ' + self.inputfile + ' -vf vidstabdetect=shakiness=10:accuracy=15:result=' + self.tmpfile('transforms.trf') + ' -f null -'
          os.system(command1)
      if not self.tmpfileexists('stabilized.mp4'):
          command2 = 'ffmpeg ' + self.debugopts() + ' -y -i ' + self.inputfile + ' -vf vidstabtransform=input="' + self.tmpfile('transforms.trf') + options[profile] + '" -acodec copy -vcodec libx264 -level 50 ' + self.tmpfile('stabilized.mp4')
          os.system(command2)
