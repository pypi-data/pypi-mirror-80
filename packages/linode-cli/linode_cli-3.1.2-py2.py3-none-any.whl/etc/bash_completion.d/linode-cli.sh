...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            COMPREPLY=( $(compgen -W "account events maintenance users domains default images linodes kernels stackscripts lke longview managed networking nodebalancers object-storage profile sshkeys regions tickets tags volumes --help" -- ${cur}) )
            return 0
            ;;
        account)
        ...
        images)
            ...
            ;;
        linodes)
            COMPREPLY=( $(compgen -W "list create view update delete backups-list snapshot backups-cancel backups-enable backup-view backup-restore boot clone configs-list config-create config-view config-update config-delete disks-list disk-create disk-view disk-update disk-delete disk-clone disk-reset-password disk-resize ips-list ip-add ip-view ip-update ip-delete migrate upgrade linode-reset-password reboot rebuild rescue resize shutdown transfer-view volumes types type-view --help" -- ${cur}) )
            return 0
            ;;
        kernels)
        ...
        stackscripts)
            ...
            ;;
        lke)
            COMPREPLY=( $(compgen -W "clusters-list cluster-create cluster-view cluster-update cluster-delete pools-list pool-create pool-view pool-update pool-delete pool-recycle api-endpoints-list kubeconfig-view versions-list version-view --help" -- ${cur}) )
            return 0
            ;;
        longview)
