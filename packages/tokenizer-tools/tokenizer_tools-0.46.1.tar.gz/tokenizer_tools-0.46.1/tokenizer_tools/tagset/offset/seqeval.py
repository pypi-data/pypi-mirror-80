...
def get_entities(seq):
    ...
    Returns:
        list: list of (index, chunk_type, chunk_start, chunk_end).
    ...
    chunks = []
    for idx, doc in enumerate(seq):
        for span in doc.span_set:
            chunks.append((span.entity, span.start, span.end, idx))
...
def accuracy_score(y_true: Corpus, y_pred: Corpus):
    ...
    # TODO: assert y_true and y_pred has same member
    #       and each member pair has same length

    y_true = [tag for doc in y_true for tag in doc.entity_biluo_tags()]
    y_pred = [tag for doc in y_pred for tag in doc.entity_biluo_tags()]
