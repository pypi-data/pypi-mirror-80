...
class TqApi(object):
    ...
    def __init__(self, account: Union[TqAccount, TqSim, None] = None, auth: Union[TqAuth, str, None] = None, url: Optional[str] = None,
                 backtest: Union[TqBacktest, TqReplay, None] = None, web_gui: [bool, str] = False, debug: Union[bool, str, None] = False,
                 loop: Optional[asyncio.AbstractEventLoop] = None, _stock: bool = True, _ins_url=None, _md_url=None, _td_url=None) -> None:
        """
        创建天勤接口实例
        ...
        Example2::
            ...
            api = TqApi(TqKq(), auth="信易账户,账户密码")  # 根据填写的信易账户参数连接指定的快期模拟账户

        Example3::

            # 使用模拟帐号直连行情服务器
            from tqsdk import TqApi, TqSim
            api = TqApi(TqSim(), auth="信易账户,账户密码")  # 不填写参数则默认为 TqSim() 模拟账号
        ...
        Example4::
            ...
            api = TqApi(backtest=TqBacktest(start_dt=date(2018, 5, 1), end_dt=date(2018, 10, 1)), auth="信易账户,账户密码")
        ...
        Example5::
            ...
            api = TqApi(backtest=TqReplay(replay_dt=date(2019, 12, 16)), auth="信易账户,账户密码")

        Example6::

            # 开启 web_gui 功能，使用默认参数True
            from tqsdk import TqApi
            api = TqApi(web_gui=True, auth="信易账户,账户密码")

        Example7::

            # 开启 web_gui 功能，使用本机IP端口固定网址生成
            from tqsdk import TqApi
            api = TqApi(web_gui=":9876", auth="信易账户,账户密码")  # 等价于 api = TqApi(web_gui="0.0.0.0:9876", auth="信易账户,账户密码")
    ...
    def close(self) -> None:
        ...
        Example::
            ...
            with closing(TqApi(auth="信易账户,账户密码")) as api:
    ...
    def insert_order(self, symbol: str, direction: str, offset: str, volume: int, limit_price: Union[str, float, None] = None,
        ...
        Example2::
            ...
            # 限价开多3手 DCE.m1901
            from tqsdk import TqApi
            with TqApi(auth="信易账户,账户密码") as api:
    ...
    # ----------------------------------------------------------------------
    def get_risk_management_rule(self, exchange_id: Optional[str] = None) -> Union[RiskManagementRule, Entity]:
        """
        获取账户风控统计规则
        ...
        if exchange_id:
            ...
            return _get_obj(self._data, ["trade", self._account._account_id, "risk_management_rule", exchange_id], RiskManagementRule(self))
        else:
            return _get_obj(self._data, ["trade", self._account._account_id, "risk_management_rule"])
    ...
    def set_risk_management_rule(self, exchange_id: str, enable: bool, count_limit: int = None, insert_order_count_limit: Optional[int] = None,
        ...
        Example::
            ...
            api.close()
        """
        if type(self._account) is not TqAccount:
            self._logger.warning("模拟账户不支持设置风控规则")
            return None
        if exchange_id not in ["SSE", "SZSE"]:
            raise Exception(f"{exchange_id} 不支持设置风控规则，只有 SSE，SZSE 支持")
        account_id = self._account._account_id
        rule_pack = {
        ...
        del rule_pack["aid"]
        rule = _get_obj(self._data, ["trade", account_id, "risk_management_rule", exchange_id])
    ...
    # ----------------------------------------------------------------------
    def get_risk_management_data(self, symbol: Optional[str] = None) -> Union[RiskManagementData, Entity]:
        """
        获取账户风控统计数据
        ...
        """
        if symbol:
            return _get_obj(self._data, ["trade", self._account._account_id, "risk_management_data", symbol], RiskManagementData(self))
        else:
            return _get_obj(self._data, ["trade", self._account._account_id, "risk_management_data"])
    ...
    def query_options(self, underlying_symbol: str, option_class=None, delivery_year=None, delivery_month=None, strike_price=None, exchange_id:str=None, expired:bool=None, has_A=None):
        """
        ...
        Args:
            ...
            delivery_year (str): [可选] 到期年份

            delivery_month (str): [可选] 到期月份
        ...
        Example::
            ...
            ls = api.query_options("SSE.510300", exchange_id="SSE", delivery_year=2021, delivery_month=10)
            print(ls)  # 上交所沪深300etf期权, 限制条件 2021 年 10 月份到期
                            ...
                            derivative (class: $derivative_class) { 
                                edges {
                                    node {
                                        ...
                                        ... on option{ expired expire_datetime last_exercise_day strike_price call_or_put}
                                    }
                                }
                            } 
                        } 
            ...
            if quote.get("derivative"):
                for edge in quote["derivative"]["edges"]:
                    ...
                    if (option_class and option["call_or_put"] != option_class) \
                            or (delivery_year and datetime.fromtimestamp(option["expire_datetime"] / 1e9).year != delivery_year) \
                            or (delivery_month and datetime.fromtimestamp(option["expire_datetime"] / 1e9).month != delivery_month) \
    ...
    def _setup_connection(self):
        ...
        if self._stock is False:  # self._stock == False 需要旧版的合约服务文件
            ...
            quotes = self._fetch_symbol_info(os.getenv("TQ_INS_URL", "https://openmd.shinnytech.com/t/md/symbols/2020-08-03.json"))
            ws_md_recv_chan.send_nowait({
                "aid": "rtn_data",
                "data": [{
        ...
        if isinstance(self._account, TqSim):
            ...
            # todo: 将检查权限的步骤，放在 account 模块的 _run 函数中
            if isinstance(self._account, TqKq):
                self._account._account_id = self._auth._auth_id
                self._account._password = self._auth._auth_id
            elif not self._auth._has_account(self._account._account_id):
                self._auth._add_account(self._account._account_id)
            ws_td_send_chan, ws_td_recv_chan = TqChan(self), TqChan(self)
            if self._td_url is None:
                self._td_url = self._auth._get_td_url(self._account._broker_id, self._account._account_id)  # _td_url 如果还是默认地址，即用户没有特别指定，换成期货公司交易地址
                ...
                for d in pack["data"]:
                    ...
                    for q_id, query_result in d.get("symbols").items():
                        ...
                        if q_id.startswith("PYSDK_quote"):
                            ...
                            # todo 应该删掉无用的请求数据，节约内存，尤其是第一个请求全部合约的查询，这个查询会在 backtest 里存一份
                            #  未来应该发送给服务器，临时方案采用删除收到的 result
                            del query_result["result"]
