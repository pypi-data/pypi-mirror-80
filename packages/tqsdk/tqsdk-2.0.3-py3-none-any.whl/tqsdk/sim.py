...
class TqSim(object):
    ...
    def __init__(self, init_balance: float = 10000000.0, account_id: str = "TQSIM") -> None:
        ...
        Example::
            ...
            api = TqApi(TqSim(init_balance=100000), auth="信易账户,账户密码")
    ...
    async def _quote_handler(self, symbol, quote_chan, order_chan):
        try:
            ...
            if quote["ins_class"].endswith("INDEX") and quote["exchange_id"] == "KQ":
                # 指数可以交易，需要补充 margin commission volume_multiple
                if "margin" not in quote:
                    quote["volume_multiple"] = quote["index_multiple"]
                    quote_m = await self._ensure_quote_info(symbol.replace("KQ.i", "KQ.m"), quote_chan)
                    quote["margin"] = self._data["quotes"][quote_m["underlying_symbol"]]["margin"]
                    quote["commission"] = self._data["quotes"][quote_m["underlying_symbol"]]["commission"]
