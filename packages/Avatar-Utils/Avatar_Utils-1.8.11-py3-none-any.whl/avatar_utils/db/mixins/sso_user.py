...
class SSOUserMixin:
    ...
    latest_session_started_at = Column(DateTime(timezone=True), nullable=True, default=None)
    latest_session_request_at = Column(DateTime(timezone=True), nullable=True, default=None)

    _after_creation_callback = None

    @classmethod
    def auth_callback(cls: Type['SSOUserMixin'], token: str):
        ...
        sso_id = payload["sub"]
        user = cls.read(sso_id=sso_id)  # noqa
        if not user:
            user = cls.create(sso_id=sso_id)  # noqa
            if cls._after_creation_callback:
                cls._after_creation_callback(payload)
    ...
    def update_latest_session_time(self) -> datetime:
        ...
        if not self.is_online:
            ...
            self.update(latest_session_started_at=timestamp)  # noqa
        self.update(latest_session_request_at=timestamp)  # noqa
        return timestamp
