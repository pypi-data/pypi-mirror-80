...
def data_normalization(scaler, raw_train=None, raw_test=None):
    ...
    '''if scaler==False:
        ...
        scaler = pickle.load(open('scaler_8_30_triple.pkl', 'rb'))
...
def _data_normalization(scaler, raw_train=None, raw_test=None):
    # 若传入scaler则使用保存的归一化参数来归一化
    '''if scaler==False:
        shape_train, shape_test = raw_train.shape, raw_test.shape
        ...
        shape_test = raw_test.shape
        scaler = pickle.load(open('scaler_9_11_1,3_4.pkl', 'rb'))
...
def _load_data(root, x, y, section):
    """
...
def _k_folds(data_path, folds, sub):
