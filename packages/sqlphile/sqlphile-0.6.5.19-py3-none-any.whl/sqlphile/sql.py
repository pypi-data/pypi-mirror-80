...
class SQL:
    ...
    def is_model (self, model, set_model = True):
    ...
    def insert (self, table, alias = None):
        self._template = "INSERT INTO " + render_table (self.is_model (table), alias) + " ({this.columns}) VALUES ({this.values})"
        return self

    def update (self, table, alias = None):
        self._template = "UPDATE " + render_table (self.is_model (table), alias) + " SET {this.pairs}"
        self._check_filter = True
        return self

    def select (self, table, alias = None):
        self._template = "SELECT {select} FROM " + render_table (self.is_model (table), alias)
        self.feed (select = ["*"])
        return self

    def delete (self, table, alias = None):
        self._template = "DELETE FROM " + render_table (self.is_model (table), alias)
        self._check_filter = True
        return self
    ...
    def validate_data (self, data):
        if data:
            return self._model.validate (data, not self._check_filter)
        return data
