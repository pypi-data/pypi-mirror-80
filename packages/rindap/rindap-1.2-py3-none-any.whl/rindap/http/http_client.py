...
from rindap.http import HttpClient
from rindap.http.response import Response
from rindap.http.request import Request as RindapRequest
import logging
from rindap.compat import urlencode

_logger = logging.getLogger('rindap.http_client')


class RindapHttpClient(HttpClient):
    """
    General purpose HTTP Client for interacting with the Rindap API
    """
    def __init__(self, pool_connections=True, request_hooks=None, timeout=None, logger=_logger, proxy=None, max_retries=None):
        """
        Constructor for the RindapHttpClient
    ...
    def request(self, method, url, params=None, data=None, headers=None, auth=None, timeout=None,
        ...
        :rtype: A :class:`Response <rindap.rest.http.response.Response>` object
        """
        if timeout is not None and timeout <= 0:
            raise ValueError(timeout)
        ...
        headers['Authorization'] = 'Bearer ' + '.'.join(auth)
        ...
        self.last_request = RindapRequest(**kwargs)
