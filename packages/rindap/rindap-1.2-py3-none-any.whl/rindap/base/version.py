...
class Version(object):
    ...
    def relative_uri(self, uri):
        ...
        """
        return '{}/rindap-rest-gw/{}'.format(self.version.strip('/'), uri.strip('/'))
    ...
    def exception(cls, method, uri, response, message):
        """
        Wraps an exceptional response in a `RindapRestException`.
        """
        ...
        try:
            ...
            return RindapRestException(response.status_code, uri, message, code, method, details)
        except Exception:
            return RindapRestException(response.status_code, uri, message, response.status_code, method)
