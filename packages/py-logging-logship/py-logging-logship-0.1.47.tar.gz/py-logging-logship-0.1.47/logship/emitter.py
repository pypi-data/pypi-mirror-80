...
class LokiEmitter(abc.ABC):
    ...
    def __init__(self, url: str, tags: Optional[dict] = None, auth: BasicAuth = None, tenant : str = None):
        """
        ...
        self.auth = auth
        # Tenant will be submitted along with the push in X-Scope-Orgid header
        self.tenant = tenant
    ...
    def session(self) -> requests.Session:
        ...
        if self._session is None:
            ...
            if self.tenant:
                self._session.headers.update({'X-Scope-Orgid': str(self.tenant)})
