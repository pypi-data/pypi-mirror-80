...
def build_FCFP_fingerprint_fun(fp_length=512, fp_radius=4):
    def fingerprints_from_smiles(weights, smiles):
        # Morgan fingerprints don't use weights.
        return fingerprints_from_smiles_tuple(tuple(smiles))
    ...
    @memoize  # This wrapper function exists because tuples can be hashed, but arrays can't.
    def fingerprints_from_smiles_tuple(smiles_tuple):
        return smiles_to_fps1(smiles_tuple, fp_length, fp_radius)

    return fingerprints_from_smiles


def build_FCFP_deep_net(fp_length, fp_depth, net_params):
    empty_parser = WeightsParser()
    morgan_fp_func = build_FCFP_fingerprint_fun(fp_length, fp_depth)
    return build_fingerprint_deep_net(net_params, morgan_fp_func, empty_parser, 0)


def build_maccs_fingerprint_fun(fp_length=512, fp_radius=4):
    def fingerprints_from_smiles(weights, smiles):
        # Morgan fingerprints don't use weights.
        ms = [Chem.MolFromSmiles(s) for s in smiles]
        fps1 = np.array([MACCSkeys.GenMACCSKeys(x) for x in ms])
        fps2 = []
        for each in fps1:
            eachfp = []
            for i in range(166):
                eachfp.append(each[i])
            fps2.append(eachfp)
        fps = np.array(fps2)
        return fps
...
def build_maccs_deep_net(fp_length, fp_depth, net_params):
    ...
    return build_fingerprint_deep_net(net_params, maccs_fp_func, empty_parser, 0)


def build_chemdec_fun(fp_length=512, fp_radius=4):
    def chemdec_from_smiles(weights, smiles):
        # Morgan fingerprints don't use weights.
        fps1 = []
        for each in smiles:
            t = []
            mol = PyChem2d()
            mol.ReadMolFromSmile(each)
            temp = mol.GetGeary()
            for eachcase in temp:
                t.append(temp[eachcase])
            fps1.append(t)
        fps = np.array(fps1)
        return fps

    return chemdec_from_smiles


def build_chemdec_deep_net(fp_length, fp_depth, net_params):
    empty_parser = WeightsParser()
    chemdec_func = build_chemdec_fun(fp_length, fp_depth)
    return build_fingerprint_deep_net(net_params, chemdec_func, empty_parser, 0)
