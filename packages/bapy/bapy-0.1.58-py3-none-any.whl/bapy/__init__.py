...
ic = icecream.IceCreamDebugger(prefix=str(), includeContext=True)
...
async def aiocmd(command: UnionTextList, onlyout: Bool = True) -> Union[TupleTextTextInt, Text]:
    """
    Asyncio run cmd.
...
def cmd(command: Iterable, onlyout: Bool = False, shell: Bool = True, sysexec: Bool = False,
...
def distribution(package: Text = str()) -> importlib_metadata.Distribution:
    """
    ...
    """
    try:
        return importlib_metadata.distribution(package if package else path.repo)
...
class Distro:
    ...
    _version_parts_build_number: Int = int(_distro_version_parts.build_number)
...
class Option:
    """APP/CLI Option."""
    Function: GenericAlias = Literal['version',]  # type: ignore
...
class Path(Pathlib, pathlib.PurePosixPath, Asdict, DataProxy):
    ...
    def scripts(self) -> List:
        p = self if self.name == 'scripts' else self / 'scripts'
        if p.is_dir():
            if p.scripts_suffix:
                scripts = [item for item in p.iterdir() if item.suffix.endswith(Path.Suffix.BASH.value)
                           or item.suffix.endswith(Path.Suffix.SH.value)]
            else:
                scripts = [item for item in p.iterdir()]
            if p.scripts_relative:
                return [item.relative_to(p.project).text for item in scripts]
            return scripts
        return list()
    ...
    def description(self) -> Text:
        ...
        readme = self.readme()
        try:
            return readme.splitlines()[0].split('#')[1]
...
class Url(furl.furl):
    ...
    bind: Int = None
    container: Int = None
    organization: Text = os.environ.get('GITHUB_ORGANIZATION', str())
    id: Text = os.environ.get('GITHUB_ORGANIZATION_ID', str())
    scheme_default: Text = 'https'
    group: Text = os.environ.get('NFERX_DEFAULT_GROUP', str())
    token: Text = os.environ.get('NFERX_GITHUB_PASSWORD', str())
    ...
    @staticmethod
    @app.command()
    def email(username: Text, stdout: Bool = False) -> Text:
        """
        User Email.
    ...
    @staticmethod
    @app.command()
    def lumenbiomics(http: Bool = False, username: Text = None, password: Text = token,
                     repo: Text = 'Repository', suffix: Bool = False, stdout: Bool = False) -> Union[Url, Text]:
        """
    ...
    def nference(stdout: Bool = False) -> Url:
        ...
        Returns:
            Url:
        """
        rv = Url(os.environ.get('NFERENCE', str()))
        if stdout:
            console.print(rv.url)
        return rv
    ...
    def nferx(stdout: Bool = False) -> Url:
        ...
        Returns:
            Url:
        """
        rv = Url(os.environ.get('NFERX_SERVERS', str()))
        if stdout:
            console.print(rv.url)
        return rv
