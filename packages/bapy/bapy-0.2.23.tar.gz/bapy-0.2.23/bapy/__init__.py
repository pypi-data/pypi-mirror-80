...
async def aiocmd(command: UnionTextList, onlyout: Bool = True,
                 onlyrc: Bool = False) -> Union[TupleTextTextInt, Text, Int]:
    """
    Asyncio run cmd.
...
def cmd(command: Iterable, onlyout: Bool = False, onlyrc: Bool = False, shell: Bool = True, sysexec: Bool = False,
        lines: Bool = True) -> TupleTextTextInt:
    """
    Runs a cmd.
...
def metadata(package: Text = str(), stdout: Bool = False) -> Dict:
    """
...
@sync_to_async
@app.command()
def my_ip(iponly: Bool = True, stdout: Bool = False) -> Union[Text, Ip]:
    """
    My Public IP.

    Args:
        iponly: IP only or Ip instance.
        stdout: stdout.

    Returns:
        Union[Text, Ip]:
    """
    rv: Ip = async_to_sync(Ip().init)
    if iponly:
        rv = rv.ip
    else:
        rv = async_to_sync(rv.asdict)
    if stdout:
        console.print(rv)
    return rv
...
Executable = dataclasses.make_dataclass('Executable',
                                        [(item.upper(), Bool, dataclasses.field(default=Distro.exec(item)))
                                         for item in ('apt', 'brew', 'curl', 'docker', 'go', 'haproxy', 'make', 'npm',
                                                      'pip', 'r', 'sudo', 'tar', 'yum',)])

Executor = Enum.new('Executor', 'NONE THREAD PROCESS')


async def executor(func: Any, *args: Any, pool: Executor = Executor.NONE.name, **kwargs: Any) -> Awaitable:
    """
...
class GitHub:
    path: Any
    api: git.Repo = dataclasses.field(init=False)
    cli: Dict[Text, List] = dataclasses.field(init=False)  # type: ignore
    cmd: git.Git = dataclasses.field(init=False)
    ...
    def __post_init__(self):
        try:
            self.api = git.Repo(self.path)
            self.cmd = self.api.git
    ...
    def add(self, force: Bool = False, write: Bool = True):
        ...
        Args:
            ...
            write: write
        """
        for file in self.api.untracked_files:
            added = self.api.index.add(file, force=force, write=write)
            ic(f'{added[0][3]=}')
...
class Ip:
    ...
    Examples:
        >>> assert asyncio.run(Ip('10:12:14').init)[0].ip is None
        >>> valid = asyncio.run(Ip('8.8.8.8').init)[0]
        >>> assert valid.reverse_pointer == '8.8.8.8.in-addr.arpa'
        >>> assert valid.ipv4 == '8.8.8.8'
        >>> assert valid.ipv6 is False
        >>> assert valid.dns_name == '8.8.8.8.in-addr.arpa.'
        >>> assert valid.ptr == 'dns.google.'
        >>> assert valid.country_name == 'United States'
        >>> assert valid.city is None
        >>> assert valid.ping is True
        >>> assert 'ping' in await valid.attrs()
        >>> assert await valid.asdict['is_global'] is True
        >>> local = asyncio.run(Ip('192.168.1.134').init)[0]
        >>> assert local.ping is False
        >>> myip = asyncio.run(Ip().init)[0]
        >>> assert myip.is_global is True
    """
    city: OptionalText = dataclasses.field(default=None, init=False)
    country_code: OptionalText = dataclasses.field(default=None, init=False)
    country_name: OptionalText = dataclasses.field(default=None, init=False)
    dns_name: OptionalText = dataclasses.field(default=None, init=False)
    ip: OptionalText = urllib.request.urlopen('https://ident.me').read().decode('utf8')
    ipv4: OptionalBoolText = dataclasses.field(default=None, init=False)
    ipv6: OptionalBoolText = dataclasses.field(default=None, init=False)
    is_global: OptionalBool = dataclasses.field(default=None, init=False)
    latitude: OptionalText = dataclasses.field(default=None, init=False)
    longitude: OptionalText = dataclasses.field(default=None, init=False)
    postal: OptionalText = dataclasses.field(default=None, init=False)
    ping: OptionalBool = dataclasses.field(default=None, init=False)
    ptr: OptionalText = dataclasses.field(default=None, init=False)
    reverse_pointer: OptionalText = dataclasses.field(default=None, init=False)
    state: OptionalText = dataclasses.field(default=None, init=False)
    version: OptionalInt = dataclasses.field(default=None, init=False)

    @property
    async def asdict(self) -> Dict:
        """
        Instance Dict.
    ...
    async def init(self) -> Ip:
        ...
        if await self.IP:
            _, _, ping_rc = await asyncio.gather(
                self.DNS,
                self.location,
                aiocmd(f'sudo ping -c {pings} {self.ip}', False, True)
            )
            if ping_rc == 0:
                self.ping = True
            elif ping_rc == 2:
    ...
    @property
    async def IP(self) -> OptionalIPaddress:
        """
        ...
        """
        try:
            rv = ipaddress.ip_address(self.ip)
            self.ip = rv.compressed
            self.ipv4 = rv.compressed if rv.version == 4 else False
            self.ipv6 = rv.compressed if rv.version == 6 else False
            self.is_global = rv.is_global
            self.reverse_pointer = rv.reverse_pointer
            self.version = rv.version
        except ValueError:
            rv = None
        return rv

    @property
    async def DNS(self) -> Optional[dns.name.Name]:
        """
        DNS Name CLass.
        ...
        if self.ip:
            try:
                rv = dns.reversename.from_address(self.ip)
                self.dns_name = rv.to_text()
                _ = self.PTR
            except (dns.resolver.NXDOMAIN, dns.exception.SyntaxError):
                self.dns_name = rv = None
            return rv

    @property
    async def PTR(self) -> Optional[dns.resolver.Resolver]:
        """
        Validates, stores ip, ipv4 and ipv6 as str and returns IPaddress.

        Returns:
            Optional[dns.resolver.Resolver]: PTR
        """
        if self.ip:
            try:
                rv = dns.resolver.resolve(self.DNS, "PTR")[0]
                self.ptr = rv.to_text()
            except (dns.resolver.NXDOMAIN, dns.exception.Timeout, dns.resolver.NoNameservers, dns.resolver.NoAnswer):
                self.dns_name = rv = None
            return rv
    ...
    async def location(self) -> OptionalDict:
        ...
        if self.ip:
            with urllib.request.urlopen(
                    f'https://geolocation-db.com/json/697de680-a737-11ea-9820-af05f4014d91/{self.ip}') as geolocation:
                location = json.loads(geolocation.read().decode())
            self.city = location['city'] if location['city'] != 'Not found' else None
            self.country_code = location['country_code'] if location['country_code'] != 'Not found' else None
            self.country_name = location['country_name'] if location['country_name'] != 'Not found' else None
            self.latitude = location['latitude'] if location['latitude'] != 'Not found' else None
            self.longitude = location['longitude'] if location['longitude'] != 'Not found' else None
            self.postal = location['postal'] if location['postal'] != 'Not found' else None
            self.state = location['state'] if location['state'] != 'Not found' else None
            return location
...
class Log(verboselogs.VerboseLogger):
    ...
    """
    Level = Literal['DEBUG', 'VERBOSE', 'INFO', 'WARNING', 'SUCCESS', 'ERROR', 'CRITICAL']
...
class Logr(Logger):
    Level: GenericAlias = Literal['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']  # type: ignore
    Levels: Type[Enum] = Enum.new('Levels', Level.__args__, start=logging)
    ...
    def get_log(cls, name: Text = None, p: Union[PathLike, Pathlib, Path] = None, file: UnionTextInt = file,
                ...
                markup: Bool = False) -> Union[Logr, Logger]:
        """
        Sets Logger.
...
class Path(Pathlib, pathlib.PurePosixPath, Asdict, DataProxy):
    ...
    def requirements(self) -> Dict:
        ...
        # noinspection PydanticTypeChecker,,PyTypeChecker,PydanticTypeChecker
        return {name: [str(req.requirement) for req in parse_requirements(file.text, session='workaround')]
                for name, file in self.scan(suffix=Path.Suffix.TXT).items()}
    ...
    def scripts(self) -> List:
        ...
        if self.is_dir():
            if self.scripts_suffix:
                return [item for item in self.iterdir() if item.suffix.endswith(Path.Suffix.BASH.value)
...
def paths(file: Text) -> Path:
    p = Path(file).resolved.parent
    installed = Frame().installed(p)
    work = User.home.mkdir_add(f'.{p.name}')
    prefix = f'{p.name.upper()}_'
    try:
        p.attrs_set(
            package=p, repo=p.name, project=p.parent, installed=installed, prefix=prefix, work=work,
            log=Logr.get_log(name=p.name, p=work.mkdir_add('log') / f'{p.name}.log',
                             stream=Env().file.log_level(upper_prefix('LOG_LEVEL', prefix=prefix),
                                                         Logr.Levels.ERROR.value), installed=installed),
            git=GitHub(p.parent)
        )
    except git.exc.InvalidGitRepositoryError as exc:
        p.log.debug(f'Invalid Git Repo: {dbg(exc)}')
...
class Progress:
    ...
    def console(self, msg, error: Bool = False, no: Bool = False):
        if self.stdout:
            if error:
                console.print(f"[bold blue]{self.percentage} "
                ...
                return
            if no:
                console.print(f"[bold blue]{self.percentage}[white] "
                              f"\[ok: [bold green]{self.ok}[white], "
                              f"error: [bold red]{self.error}[white], "
                              f"no: [bold magenta]{self.no}[white], "
                              f"total: [bold blue]{self.total}[white]] "
    ...
    def out_err(self, msg: Text = str()):
        self.error += 1
        self.count += 1
        self.percentage = self.get_percentage()
        self.console(msg, error=True)

    def out_ok(self, msg: Text = str()):
        self.ok += 1
        self.count += 1
        self.percentage = self.get_percentage()
        self.console(msg)

    def out_no(self, msg: Text = str()):
        self.no += 1
        self.count += 1
        self.percentage = self.get_percentage()
...
class Up:
    """APP/CLI Option."""
    Bump: GenericAlias = Literal['patch', 'minor', 'major']
...
def up(bump: Text = Up.option()):
    ...
    """
    if Distro.MACOS:
        os.system(f'{path.repo} --version && project-upload.sh {bump} {shlex.quote("$BAPY")} && '
                  f'{path.repo} --version')
    elif Distro.KALI:
        os.system(f'{path.repo} --version && project-upgrade.sh {path.repo} && '
                  f'{path.repo} --version')
...
class Url(furl.furl):
    ...
    """
    bind: Int = None
    company: Text = 'nference.net'
    container: Int = None
    group: Text = 'DevOps'
    id: Text = 'org-4379404'
    ops: Text = 'nferxops.net'
    organization: Text = 'lumenbiomics'
    scheme_default: Text = 'https'
    server: Text = 'nferx.com'
...
class UserProcess:
    ...
    def __post_init__(self):
        if self.sudo_user and self.ROOT and self.sudo_user != UserActual.name:
            self._exception = True
            raise Exception(f'{self.sudo_user=}, {self.ROOT=}, {UserActual()=}, {self=}')
