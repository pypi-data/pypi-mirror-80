...
class Path(Pathlib, pathlib.PurePosixPath, Asdict, DataProxy):
    ...
    def scripts(self) -> List:
        ...
        if p.is_dir():
            if p.scripts_suffix:
                rv = [item for item in p.iterdir() if item.suffix.endswith(Path.Suffix.BASH.value)
                      or item.suffix.endswith(Path.Suffix.SH.value)]
            else:
                rv = [item for item in p.iterdir()]
            if p.scripts_relative:
                return [item.relative_to(p.project).text for item in rv]
            return rv
        return list()
