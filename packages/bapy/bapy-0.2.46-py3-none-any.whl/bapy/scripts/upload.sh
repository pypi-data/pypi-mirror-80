...
[[ "${path-}" ]] || { path="${BAPY}"; name="$( basename "${path}" )"; }
bump="${bump:-patch}"
twine="${twine:-${NFERX_GITHUB_USERNAME}}"
test -n "${twine}" || { error.sh "twine repository" "empty"; exit 1; }
export USERNAME USER GITHUB_USERNAME NFERX_GITHUB_USERNAME BAPY PEN bump twine path name site
debug.sh USERNAME USER GITHUB_USERNAME NFERX_GITHUB_USERNAME BAPY PEN bump twine path name site
...
if isuserdarwin.sh && [[ "${USERNAME}" == "${USER}" ]]; then
  ...
  if error="$( "${virtual}python3" "${path}/setup.py" sdist 2>&1 )"; then
    info.sh sdist "${name}"
  else
    error.sh sdist "${name}" "${error}"; exit 1
  fi
  if error="$( "${virtual}python3" "${path}/setup.py" bdist_wheel 2>&1 )"; then
    info.sh wheel "${name}"
  else
...
unset starting bump twine virtual file error name path PYTHONPATH
