...
class Ip:
    ...
    Examples:
        >>> assert asyncio.run(Ip('10:12:14').init).ip is None
    ...
    async def DNS(self) -> Optional[dns.name.Name]:
        ...
        if self.ip:
            try:
                rv = await sync_to_async(dns.reversename.from_address)(self.ip)
    ...
    async def PTR(self) -> Optional[dns.resolver.Resolver]:
        ...
        """
        if self.ip and self.dns_name:
            try:
                rv = await sync_to_async(dns.resolver.resolve)(self.dns_name, "PTR")
                rv = rv[0]
                self.ptr = rv.to_text()
