...
async def aiocmd(command: UnionTextList, onlyout: Bool = True,
                 onlyrc: Bool = False, decode: Bool = True) -> Union[TupleTextTextInt, Text, Int]:
    """
    Asyncio run cmd.
...
class GitHub:
    path: Any
    api: git.Repo = dataclasses.field(default=None, init=False)
    cli: Dict[Text, List] = dataclasses.field(init=False)  # type: ignore
    cmd: git.Git = dataclasses.field(default=None, init=False)
...
class Ip:
    ...
    async def init(self) -> Ip:
        ...
        """
        pings = 3
        self.DNS = self.Info = self.Location = dict()
