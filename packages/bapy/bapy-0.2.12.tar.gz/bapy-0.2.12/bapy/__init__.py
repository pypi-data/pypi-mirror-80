...
def up(bump: Text = Option.option(Option.Function.__args__[0], 'Part of version to increase')):
    ...
    """
    if Distro.MACOS:
        os.system(f'{path.repo} --version && project-upload.sh {bump} {shlex.quote(os.environ["PEN"])} git && '
                  f'{path.repo} --version')
    elif Distro.KALI:
        os.system(f'{path.repo} --version && project-upgrade.sh {shlex.quote(os.environ["PEN_BASENAME"])} && '
                  f'{path.repo} --version')
...
def version_callback(value: bool):
    if value:
        typer.echo(f"{path.repo}: {__version__}")
        raise typer.Exit()
# </editor-fold>
...
class Path(Pathlib, pathlib.PurePosixPath, Asdict, DataProxy):
    ...
    def scripts_relative(self) -> List:
        ...
        return [item.relative_to(self.project) for item in scripts] if scripts else list()
