...
class Ip:
    ...
    Examples:
        ...
        >>> assert valid.DNS['Reverse Pointer'] == '8.8.8.8.in-addr.arpa'
        >>> assert valid.Info['IPv4'] == '8.8.8.8'
        >>> assert valid.Info['IPv6'] is False
        >>> assert valid.DNS['DNS Name'] == '8.8.8.8.in-addr.arpa.'
        >>> assert valid.DNS['PTR'] == 'dns.google.'
        >>> assert valid.Location['Country Name'] == 'United States'
    ...
    """
    DNS: Dict = dataclasses.field(default=None, init=False)
    ...
    async def IPClass(self) -> OptionalIPaddress:
        ...
        try:
            ...
            self.DNS['Reverse Pointer'] = rv.reverse_pointer
    ...
    @property
    async def DNSClass(self) -> Optional[dns.name.Name]:
        """
        DNS Name CLass.
        ...
        if self.IP:
            try:
                ...
                self.DNS['DNS Name'] = rv.to_text()
                _ = await self.PTRClass
            except (dns.resolver.NXDOMAIN, dns.exception.SyntaxError):
                self.DNS['DNS Name'] = rv = None
            return rv

    @property
    async def PTRClass(self) -> Optional[dns.resolver.Resolver]:
        """
        Validates, stores ip, ipv4 and ipv6 as str and returns IPaddress.
        ...
        """
        if self.IP and self.DNS['DNS Name']:
            try:
                rv = await sync_to_async(dns.resolver.resolve)(self.DNS['DNS Name'], "PTR")
                rv = rv[0]
                self.DNS['PTR'] = rv.to_text()
            except (dns.resolver.NXDOMAIN, dns.exception.Timeout, dns.resolver.NoNameservers, dns.resolver.NoAnswer):
                self.DNS['PTR'] = rv = None
            return rv
    ...
    async def location(self) -> OptionalDict:
        ...
        if self.IP:
            ...
            self.Location['Country Code'] = location['country_code'] \
                if location['country_code'] != 'Not found' else None
            self.Location['Country Name'] = location['country_name'] \
...
def setup():
    return User.gecos, Url.email(), path.repo, path.project.description(), path.project.requirements['requirements'], \
           path.scripts_relative, path.project.requirements['requirements_setup'], \
           path.project.requirements['requirements_test'], Url.lumenbiomics(http=True, repo=path.repo).url
