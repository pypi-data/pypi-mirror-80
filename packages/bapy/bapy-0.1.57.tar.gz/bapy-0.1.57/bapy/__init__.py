    ...
    def scripts(self) -> List:
        ...
        if self.is_dir():
            if self.scripts_suffix:
                scripts = [item for item in self.iterdir() if item.suffix.endswith(Path.Suffix.BASH.value)
                           or item.suffix.endswith(Path.Suffix.SH.value)]
            else:
                scripts = [item for item in self.iterdir()]
            if self.scripts_relative:
                return [item.relative_to(self.project) for item in scripts]
            return scripts
        return list()
