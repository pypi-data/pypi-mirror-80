...
def _install_nvidia_pypi_index():
    ...
    for key, files in file_dict.items():
        for file in files:
            print("\nProcessing pip conf file: %s ..." % file)
            try:
                if not os.path.exists(file):
                    print("Creating pip conf file: %s ..." % file)
                    create_pip_config_file(file)
                else:
                    print("Editing pip conf file: %s ..." % file)
...
if "sdist" not in sys.argv:
    # force re-name the package to `nvidia-pyindex` if needed.
