...
class Connection:
    ...
    def get_datasets(self, verbose=False):
        ...
        response = self.do_get_request(self.datasets_url, allowable_response_status_codes=[200], params=params)
        parsed_response = utils.parse_json_response(response)
    ...
    def get_config_history(self, entity_id, since=None, limit=None, reverse=None):
        ...
        response = self.do_get_request(url, allowable_response_status_codes=[200], params=request_params)
        utils.validate_equal_case_insensitive(response.headers["Content-Type"], "application/json")
        config = utils.parse_json_response(response)
    ...
    def get_api_info(self):
        response = self.do_get_request(self.sesamapi_base_url, allowable_response_status_codes=[200])
        info = response.json()
        return info
