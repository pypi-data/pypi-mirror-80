...
def chi_square(X,y,alpha:float):
    result = pd.DataFrame(columns=['Independent_Variable','Alpha','Degree_of_Freedom', 'Chi_Square','P_value'])
    for col in X.columns:
        ...
        if p_value <= alpha:
            print(f"Test result rejects the null hypothesis. There's is a relationship between the {col} and the response variable")
        else:
            print(f"Test result fails to reject the null hypothesis. There's no evidence to prove there's a relationship between {col} and the response variable")
        result = result.append(pd.DataFrame([[col,alpha, dof,chi_square_statistic, p_value]],columns=result.columns))
    return result
