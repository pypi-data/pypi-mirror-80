...
def icp_refinement_with_ppf_match(source_cloud, target_cloud, n_points=3000, tolerance=0.05):
    ...
    icp_fnc = cv2.ppf_match_3d_ICP(1000, tolerence=tolerance, rejectionScale=2.5, numLevels=2) 
    retval, residual, pose = icp_fnc.registerModelToScene(source_np_cloud, target_np_cloud)

    # open3d.visualization.draw_geometries([source_cloud.paint_uniform_color([0, 1, 0]), target_cloud.paint_uniform_color([1, 0, 0])])
    # open3d.visualization.draw_geometries([source_cloud.transform(pose).paint_uniform_color([0, 1, 0]), target_cloud.paint_uniform_color([1, 0, 0])])
