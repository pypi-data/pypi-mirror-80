...
class LiveStyledResourceClient(LiveStyledAPIClient):
    ...
    def get_device_realities(
        ...
        if device:
            if isinstance(device, Device):
                filters['device'] = '/{}/{}'.format(DeviceSchema.Meta.url, device.id)
            else:
                filters['device'] = device
        if reality:
            if isinstance(reality, Reality):
                filters['reality'] = '/{}/{}'.format(RealitySchema.Meta.url, reality.id)
            else:
    ...
    def delete_device_reality(
        ...
        self._delete_resource(DeviceReality, device_reality)
