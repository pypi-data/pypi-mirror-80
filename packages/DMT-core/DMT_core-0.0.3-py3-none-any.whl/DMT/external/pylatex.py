...
class Tex(Section):
    ...
    def __init__(self, numbering=None, label=False, **kwargs):
        super().__init__('dummy', numbering=None, label=label) #dont need a title for a few tex commands
...
class Listing(Container):
    ...
    def __init__(self, content=None, file_path=None, language='TRADICA', caption=None, label=None, **kwargs):
...
class SubFile(Section):
    ...
    def dumps(self):
        ...
        string = "\\documentclass["+self.master+"]{subfiles}\n" + "\\begin{document}\n" + string + "\n \\end{document} \n"
        return string
...
class CommandInput(CommandBase):
    ...
    _latex_name = 'input'
    _default_escape = False
    packages = [Package('siunitx'), Package('tikz'), Package('fontspec'), Package('pgfplots'), Package('inputenc', options='utf8')]
