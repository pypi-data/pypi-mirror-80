...
from DMT.core import DutType, print_progress_bar, specifiers, sub_specifiers, specifiers_ss_para, DATA_CONFIG, DutView
...
class __Filter(object):
    ...
    def filter(self, dev, testStructure):
        ...
        try:
            ...
            raise IOError('DMT -> DutLib -> Filter: the property ' + self.devProp + ' is not existent.')
...
class LenNameFilter():
    ...
    def filter(self, dev, testStructure):
        return self.nameFilter.filter(dev, testStructure) & self.lenFilter.filter(dev, testStructure)
...
class DutLib(object):
    ...
    def __init__(self, deem_types=None, AC_filter_names=None, DC_filter_names=None, is_deembedded_DC=False, is_deembedded_AC=False, save_dir=None, force=False):
        ...
        self.deem_open         = DutType.flag_open  # deem_open_bjt # look only for the flag not for the device!
    ...
    def save_dir(self, path):
        ...
        if (path / 'dut_lib.p').is_file():
            raise FileExistsError("The path you chose is already used by an other library! Either delete the already existing library or load it.")
    ...
    def import_directory(self, import_dir, dut_filter, dut_level=1, temperature_converter=None, force=True, **kwargs):
        ...
        if not callable(dut_filter):
            raise IOError('DMT -> DataManager -> import_directory(): You did not specify a dut_filter function.\n How shall DMT know what kind of Dut is where?')
        ...
        if len(duts) == 0:
            raise IOError('DMT -> collect_data: Did not find a single candidate for importing in directory ' + str(import_dir) + ' .')
        ...
        for dut, path in zip(duts, dut_paths):
            ...
            for root, _dors, files in os.walk(path):
                for name in files:
                    #get etension name and cast to lower case
                    extension = name.split(".")[-1]
                    extension = extension.lower()
                    if (extension ==  'mdm') or (extension ==  'elpa') or(extension == 'csv'):
                        path_root = Path(root)
                        #update the progress bar
                        print_progress_bar(count_duts, len(duts), prefix = 'Importing:', suffix = 'Complete', length = 50)
                        ...
                        key = dut.join_key_temperature(*key_list, name.split('.')[0], temperature_converter=temperature_converter )
                        # key       =  dut.join_key(*key_list, name.split('.')[0])
        ...
        print('DutLib imported ' + str(count_duts) + ' DUTs.')
    ...
    def save(self):
        ...
        for dut in self.duts:
            if dut.database_dir != os.path.join(self.save_dir,'duts'):
                # dut.data
                dut.database_dir = os.path.join(self.save_dir,'duts')
        ...
        if self._dut_ref is not None:
            ...
            if filecmp.cmp(path_abs, self.dut_ref.dut_dir):
                ...
                raise IOError("DMT->DutLib->Save: I can not convert the relative dut path of dut_ref into an absolute path!")
        ...
        if self._dut_internal is not None:
            ...
            if filecmp.cmp(path_abs, self.dut_internal.dut_dir):
                ...
                raise IOError("DMT->DutLib->Save: I can not convert the relative dut path of dut_internal into an absolute path!")
        ...
        if self._dut_intrinsic is not None:
            ...
            if filecmp.cmp(path_abs, self.dut_intrinsic.dut_dir):
                ...
                raise IOError("DMT->DutLib->Save: I can not convert the relative dut path of dut_intrinsic into an absolute path!")
    ...
    def load(lib_directory):
        ...
        with (lib_directory/'dut_lib.p').open(mode='rb') as handle:
            ...
            dut_lib._save_dir = Path(dut_lib.save_dir)

        # need to cast paths? This is needed when the machine is changed -> new absolute paths...
        if lib_directory != dut_lib.save_dir: # dut_lib.save_dir is the old absolute path of the lib
        ...
        for dir_name, dir_list, file_list in os.walk(dut_lib.save_dir/'duts'): #find the duts
            for file_dut in file_list:
                if file_dut.endswith(".p"):
                    dut_lib.duts.append(
                        DutView.load_dut(os.path.join(dir_name, file_dut))
                    )
        ...
        if dut_lib.dut_ref_dut_dir is not None:
            ...
            if not dut_lib.dut_ref_dut_dir.exists():
                dut_lib.dut_ref_dut_dir = Path(str(dut_lib.dut_ref_dut_dir).replace(save_dir_old, str(lib_directory)))
        if dut_lib.dut_internal_dut_dir is not None:
            ...
            if not dut_lib.dut_internal_dut_dir.exists():
                dut_lib.dut_internal_dut_dir = Path(str(dut_lib.dut_internal_dut_dir).replace(save_dir_old, str(lib_directory)))
        if dut_lib.dut_intrinsic_dut_dir is not None:
            ...
            if not dut_lib.dut_intrinsic_dut_dir.exists():
                dut_lib.dut_intrinsic_dut_dir = Path(str(dut_lib.dut_intrinsic_dut_dir).replace(save_dir_old, str(lib_directory)))
    ...
    def deembed_AC(self, width_filter, length_filter, name_filter, user_fun=None):
        ...
        if len(open_list) == 1 and len(short_list) == 1:
            ...
            for i_dev, dev in enumerate(dev_list):
                ...
                print_progress_bar(i_dev, len(dev_list), prefix = 'Deembedding AC:', suffix = dev.name, length = 50)
        ...
        if not width_filter and not length_filter and not name_filter and user_fun is None:
            raise IOError('DMT -> DutLib: You did not select any filter flags that would allow deembedding! Alternative: pass user_fun')
        ...
        for i_dev, dev in enumerate(dev_list):
            ...
            if len(suitable_opens)>1 or len(suitable_shorts)>1:
                ...
                raise NoOpenDeembeddingDut('For ' + dev.name + ' no open was found. Deemb_name is ' + dev.deemb_name + '.')
            ...
            print_progress_bar(len(dev_list), len(dev_list), prefix = 'Deembedding AC:', suffix = dev.name, length = 50)
    ...
    def deembed_DC(self, width_filter, length_filter, name_filter, function_dut=None, function_df=None, t_ref=300):
        """Assign O&S structures to the correct devices for later deembedding process and adds the thus assigned devices to a new list,
        which only includes teststructures for characterization and no O&S structures separately.
        ...
        method      : function
            a function that takes dut_short and returns the metallization resistances as a dict {'R_EM':float64, 'R_BM':float64, 'R_CM':float64}. Use this if the default DMT DC deembeding is not suitable.
        ...
        if len(short_list) == 1:
            ...
            for i_dev, dev in enumerate(dev_list):
                if dev == self.dut_ref:
                    mres = self.deembed_dut_DC(dev, short_list[0], function_dut=function_dut, function_df=function_df, t_ref=t_ref)
                else:
                    self.deembed_dut_DC(dev, short_list[0], function_dut=function_dut, function_df=function_df, t_ref=t_ref)
                print_progress_bar(i_dev, len(dev_list), prefix = 'Deembedding DC:', suffix = dev.name, length = 50)

            print_progress_bar(len(dev_list), len(dev_list), prefix = 'Deembedding DC:', suffix = 'finish', length = 50)
            self.is_deembedded_DC = True
            return mres

        #more than one deemb device, then we need filters.
        if not width_filter and not length_filter and not name_filter:
            raise IOError('DMT -> DutLib: You did not select any filter flags that would allow deembedding!')
        ...
        for i_dev, dev in enumerate(dev_list):
            ...
            if len(suitable_shorts)>1:
                ...
                print_progress_bar(i_dev, len(dev_list), prefix = 'Deembedding DC:', suffix = dev.name, length = 50)
                mres = self.deembed_dut_DC(dev, suitable_shorts[0], function_dut=function_dut, function_df=function_df, t_ref=t_ref)

        print_progress_bar(len(dev_list), len(dev_list), prefix = 'Deembedding DC:', suffix = dev.name, length = 50)
        ...
        for key in dut.data.keys():
            ...
            for (meas_filter, deem_filter) in self.AC_filter_names:
                ...
                if re.search(meas_filter, key,re.IGNORECASE):
                    ...
                    if len(short_keys) == 0 and len(open_keys) == 0:
                    ...
                    if len(short_keys) == 1 and len(open_keys) == 1:
                        ...
                        dut.data[key]            = dut.data[key].deembed(df_open, df_short, ports=dut.nodes, ndevices=dut.ndevices, ndevices_open=dut_open.ndevices, ndevices_short=dut_short.ndevices)
                    ...
                    #bad news...try to find same temperatures. #TODO: Does not work if no keys are found
                        ...
                        try:
                            ...
                            raise IOError('The dataframes with keys ' + open_key + ' and ' + short_key + ' from open device ' + dut_open.name + ' and short device ' + dut_short.name + ' are not matching the data in key ' + key + ' from dut ' + dut.name + '.')
            ...
            if not deembedded and requires_deemb:
                raise IOError('During AC Deembedding: Dataframe ' + key + ' seems to require deembedding, but not a single suitable key was found.')
    ...
    def deembed_dut_AC_internal(self, dut, mc, method):
        ...
        for meas_filter in self.AC_filter_names:
            for key in dut.data.keys():
                #check if df needs to be AC deembedded & find O&S structure
                if re.search(meas_filter, key,re.IGNORECASE):
                    print_progress_bar(n, n_keys, prefix = 'Deembedding internal:', suffix = dut.name, length = 50)
    ...
    def load_database(self, database_dir, only_meas=True):
        ...
        for dir_dut in os.listdir(database_dir):
            ...
            duts.append(
                DutView.load_dut(
                    os.path.join(database_dir, dir_dut, 'dut.p')
                )
            )
    ...
    def deembed_dut_DC(self, dut, dut_short, function_dut=None, function_df=None, t_ref=300):
        ...
        if function_dut is not None and function_df is not None:
            raise IOError('DMT->DutLib->deembed_DC: Either function_dut or function_df can be given, but not both!')
        ...
        if function_dut is not None:
            ...
            for key in dut.data.keys():
                print(key)
                dut.data[key] = dut.data[key].deembed_DC(mres=mres_tref)
        else:
            mres_tref = None
                ...
                for key in dut.data.keys():
                    ...
                    if re.search(meas_filter, key,re.IGNORECASE):
                        ...
                        if len(short_keys) == 1:
                            ...
                            key_temperature = dut.get_key_temperature(key)
                            for short_key in short_keys:
                                if np.isclose(key_temperature, dut_short.get_key_temperature(short_key)):
                                    break
                            df_short      = dut_short.data[short_key]
                            ...
                            if function_df is None:
                                try:
                                    ...
                                    raise IOError("Column missing in df of dut " + dut_short.name + " of df with key " + short_key + ". Available keys: " + str(df_short.data.keys()) + '.')
                            else:
                        ...
                        dut.data[key] = dut.data[key].deembed_DC(mres=mres)
    ...
    def toTex(self):
        ...
        with doc.create(Section('Measured Devices')):
            with doc.create(Subsection('Geometry Overview')):
                ...
                for dut_type in dut_types:
                    ...
                    for config in configs:
                        if config is None:
                            doc.append(NoEscape(r'Measurements for devices of type ' + str(dut_type) + r' are available with the following geometries:'))
                        else:
                            doc.append(NoEscape(r'Measurements for devices with contact configuration ' + config.replace('_', r'\_') + r' and device type ' + str(dut_type) + r' are available with the following geometries:'))

                        doc.append('\r')
                        duts   = list(set([dut            for dut in self if dut.dut_type == dut_type and dut.contact_config == config]))
                        lE0s   = list(set([dut.length     for dut in duts]))
                        bE0s   = list(set([dut.width      for dut in duts]))
                        lE0s.sort()
                        bE0s.sort()
                        header = '|' + ' c | '* (len(lE0s)+1) #one col for each length and one for bE0 indices
                        with doc.create(Center()) as _centered:
                            with doc.create(Tabular(header)) as table:
                                table.add_hline()
                                first_row  = [NoEscape(r'\backslashbox{$b_{\mathrm{E,drawn}}/\si{\micro\meter}$}{$l_{\mathrm{E,drawn}}/\si{\micro\meter}$}')]
                                try:
                                    first_row += ['{:04.2f}'.format(lE0*1e6) for lE0 in lE0s]
                                except TypeError:
                                    first_row += [','.join(['{:04.2f}'.format(lE0_a*1e6) for lE0_a in lE0]) for lE0 in lE0s]
                                ...
                                for bE0 in bE0s:
                                    try:
                                        ...
                                        row = [','.join(['{:04.2f}'.format(bE0_a*1e6) for bE0_a in bE0])]
                                    for lE0 in lE0s:
                                        try:
                                            #check if dut with these dimensions exists
                                            dut = next(dut for dut in duts if dut.length == lE0 and dut.width == bE0)
                                            row.append('x')
            ...
            with doc.create(Subsection('Measurement Data over Temperature and Deembedding Structures')):
                ...
                for dut_type in dut_types:
                    ...
                    for config in configs:
                        if config is None:
                            doc.append(NoEscape(r'The following table gives an overview of all measurements for devices of type ' + str(dut_type) + r'.'))
                        else:
                            doc.append(NoEscape(r'The following table gives an overview of all devices with contact configuration ' + config.replace('_', r'\_') + r' and device type ' + str(dut_type) + r'.'))
                        ...
                        with doc.create(Center()) as _centered:
                            with doc.create(SmallText()) as _small:
                                with doc.create(Tabular(header)) as table:
                                    table.add_hline()
                                    first_row  = [r'name ', NoEscape(r'measured @'), NoEscape(r'$l_{\mathrm{E0,drawn}}$'), NoEscape(r'$b_{\mathrm{E0,drawn}}$'), r'Open Deem. Structure', r'Short Deem. Structure']
                                    table.add_row(first_row)
                                    second_row  = [r'', NoEscape(r'$T/\si{\kelvin}$'), NoEscape(r'$/\si{\micro\meter}$'), NoEscape(r'$/\si{\micro\meter}$'), r'', r'']
                                    ...
                                    for dut in duts:
                                        ...
                                        if len(temps)<3:
                                            temps_str = ', '.join(['{}']*len(temps)).format(*temps)
                                            row = [
                                                '{:s}'.format(dut.name),
                                                temps_str,
                                            ...
                                            temps_str = ', '.join(['{}']*len(temps[:3])).format(*temps[:3])
                                            row = [
                                                '{:s}'.format(dut.name),
                                                temps_str,
                                                '{:04.2f}'.format(lE0_drawn),
            ...
            doc.append(NoEscape(r'The reference device ' + self.dut_ref.name.replace('_',r'\_') + r' is of type ' + str(self.dut_ref.dut_type) + ' in ' + self.dut_ref.contact_config.replace('_', r'\_') + r' configuration and has $l_{\mathrm{E,drawn}}$ of $\SI{' + lE0 + r'}{\micro\meter}$ and $b_{\mathrm{E,drawn}}$ of $\SI{' + bE0 + r'}{\micro\meter}$.'))
            doc.append(NoEscape(r'\enspace All extraction steps that do not deal with special test structures (like tetrodes) or with multiple device geometries, show measured data of the reference device.'))
    ...
    def plot_all(self, mode=None, devices=None, output_settings=None, plot_specs=None, show=True):
        ...
        plot_defaults = {
            DutType.npn:{
                ...
                'y21_jc_vbc':{
                    'x_log'          : True,
                    'y_log'          : True,
                    'at'             : [specifiers.FREQUENCY, specifiers.VOLTAGE+'B'+'C'+sub_specifiers.FORCED],
                    'quantity_x'     : specifiers.CURRENT_DENSITY+'C',
        ...
        for plot_spec in plot_specs:
            try:
                ...
                raise IOError('type not specified for plot specification.')

            valid_plots = plot_defaults[plot_spec['dut_type']].keys()
            if not plot_type in valid_plots:
                raise IOError('Plot type '+ plot_type + ' not valid. Valid: ' + ' '.join(str(plot_defaults.keys())) + ' .')
        ...
        for plot_spec in plot_specs:
            ...
            for dut in duts:
                ...
                for temp in temps:
                    ...
                    x_scale = None #autoscale
                    y_scale = None
                    x_label = None #autolabel
                    y_label = None
                    if quantity_y.specifier in specifiers.SS_PARA_Y: #special cases that I do not want in DMT
                    ...
                    for key in dut.data.keys():
                        ...
                        if match:
                            ...
                            if specifiers.FREQUENCY in plot_spec.keys():
                                try:
                                    df = df[ df[specifiers.FREQUENCY]==plot_spec[specifiers.FREQUENCY] ]
                                except KeyError:
                                    pass

                            for quantity in quantities_to_ensure:
                                try:
                                    df.ensure_specifier_column(quantity, area=AE0_drawn,ports=dut.ac_ports)
                                except KeyError:
                                    if quantity == specifiers.TEMPERATURE:
                                        #calculate rough temperature
                                        pdiss = df[specifiers.CURRENT+'C'].to_numpy() * df[specifiers.VOLTAGE+'C'].to_numpy()
                            ...
                            for point in f:
                                df_filter = True
                                at_str = r'$'
                                for i,(speci,u,scale_) in enumerate(zip(at_specifier,units,at_scale)):
                                    df_filter = np.logical_and(df_filter,np.isclose(df[speci],point[i], rtol=1e-3))
                                    if at_str != r'$':
                                        at_str += r',\,'
                                    if plot_spec['no_at']:
                                        at_str   += r'{0:1.2f}'.format(point[i]*scale_) + u
                                    else:
                                        at_str   += speci.to_tex() + r' = {0:1.2f}'.format(point[i]*scale_) + u
                                ...
                                if 'mark_ft' in plot_type:
                                    ...
                                    try:
                                        vbe_new       = np.linspace(x[index_peak_ft-10],x[index_peak_ft+10],201) #may error
                                    except IndexError:
                                        vbe_new       = np.linspace(x[index_peak_ft-10],x[-1],201) #may error
                    ...
                    print('...finished plot of type ' + plot_type + ' for dut ' + dut.name + ' , found ' + str(n) + ' lines.')
        ...
        for plt in plts:
            ...
            plot_path = os.path.join(base_path, 'figs', plt.dut_name, 'T' + str(plt.temp) + 'K', plt.plot_type)
            if plt.plot_type == 'tj_jc_at_vbc':
                plot_path = os.path.join(base_path, 'figs', plt.dut_name, 'T' + str(plt.temp) + 'K', plt.plot_type + 'rth_' +  '{0:1.1f}'.format(plt.dut.rth*1e-3) + 'kWperK')
        ...
        if output_settings['create_doc']:
            ...
            try:
                ...
                with open(os.path.join(destination, 'content', 'deckblatt.tex'), 'r', encoding='utf-8') as deckblatt:
                ...
                string_deckblatt = string_deckblatt.replace('_wafer'        , str(self.wafer).replace('_', r'\_'))
                string_deckblatt = string_deckblatt.replace('_date_TO'      , str(self.date_tapeout).replace('_', r'\_'))
                string_deckblatt = string_deckblatt.replace('_date_received', str(self.date_received).replace('_', r'\_'))

                with open(os.path.join(destination, 'content', 'deckblatt.tex'), 'w', encoding='utf-8') as deckblatt:
                                ...
                                if plt.temp == temp:
                                    ...
                                    if os.path.isfile(plt.path):
                                        ...
                                        with doc.create(Figure(position='ht!')) as _plot:
                                            ...
                                            _plot.add_caption(NoEscape(plot_defaults[dut.dut_type][plt.plot_type]['tex']))
