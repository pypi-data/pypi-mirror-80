...
class DutType(Flag):
    ...
    deem_bjt    = DutTypeInt(deemb_struct | flag_bjt_deemb, nodes = ['B', 'C', 'E', 'S'], string='bjt_deemb') # because of node names :(
    deem_mos    = DutTypeInt(deemb_struct | flag_mos_deemb, nodes = ['G', 'D', 'S', 'B'], string='mos_deemb') # because of node names :(
    ...
    tlm         = DutTypeInt(meas_struct | flag_tlm, nodes=['L', 'M', 'R'], string='tlm') # left, middle, right
    ...
    vdp         = DutTypeInt(meas_struct | flag_vdp, nodes=['A', 'B', 'C', 'D'], string='vdp') # four arbitrary contacts
    ...
    tetrode     = DutTypeInt(meas_struct | flag_tetrode , nodes=['B1', 'B2', 'E', 'C', 'S'], string='tetrode')

    cap_ac     = DutTypeInt( cap | meas_struct, nodes=['L', 'R', 'G', 'S'], string='capacitance-ac' ) # capacitance in GSG pads, each pad is one Capacitance, so S(1,1) and S(2,2) are wanted...
