...
def df_to_sweep(df_to_convert, temperature=300, name=None, from_forced=True, SweepDefClass=None):
    ...
    """
    vars_other = {specifiers.TEMPERATURE: temperature} # temperature is usually in the dut.data key and not in the df. Additionally it is always constant ?!?
    def_output = []
    potentials_forced = [] # list of forced voltages
    for col in df_to_convert.columns:
        try:
            if col.specifier not in [specifiers.VOLTAGE, specifiers.FREQUENCY]: # all columns except voltages and frequency are outputs
                def_output.append(col)
            if (col.specifier == specifiers.VOLTAGE) and len(col.nodes) == 1 and (not from_forced or (sub_specifiers.FORCED in col)): # forced voltages are needed to create sweepdef
                potentials_forced.append(col)
        except AttributeError: # can only analyze specifiers
            def_output.append(col)

    if not potentials_forced:
        raise IOError("DMT->df_to_sweep: No forced potentials in the columns. Make sure all forced potentials in the columns are SpecifierStr!")

    # find one potential with only one value and set it as reference potential!
    potential_common = ''
    for potential in sorted(potentials_forced, key=lambda x: x.nodes[0]): # sort to make sure everytime the same is found.
    ...
    if not potential_common: # no potential found
        ...
        raise IOError('DMT -> df_to_sweep: The user supplied voltages do not define unambiguous sweep conditions for the transistor. No common reference potential was found.')

    def_sweep = [{'var_name':potential_common, 'sweep_order':0, 'sweep_type':'CON', 'value_def':[0]},] # 0 because the other Sweepdefs are added as voltages.
    sweep_order = 1

    # now we need all sweep potentials with regard to one reference potential or other synced sweeps.
    potentials_sweep = [potential for potential in set(potentials_forced) if potential != potential_common]
    ...
    for i_voltage, (voltage, _count) in enumerate(sorted(count_ops.items(), key=lambda kv: kv[1])[:-1]): # last of the sorted list is the one we don't need
        df_to_convert.ensure_specifier_column(voltage)
        values = np.unique(np.round(df_to_convert[voltage].to_numpy(), 6))
        ...
        if voltage.nodes[1] == potential_common.nodes[0]: # convert to potential if possible
            ...
            def_sweep.append(
                {'var_name':potential, 'sweep_order':sweep_order, 'sweep_type':'LIST', 'value_def':values}
            )
        else: # sweep a voltage is more difficult
            # nodes in all other voltages:
            other_nodes = ''
            for voltage_other, _count in sorted(count_ops.items(), key=lambda kv: kv[1])[i_voltage+1:-1]:
            ...
            def_sweep.append(
                {'var_name':voltage_offset, 'sweep_order':sweep_order, 'sweep_type':'LIST', 'value_def':values}
            )

            # find sweep_order of potential_master
            sweep_order_master = 0
            for i_voltage_master, (voltage_master, _count) in enumerate(sorted(count_ops.items(), key=lambda kv: kv[1])[:-1]):
                if voltage_master.nodes[1] == potential_common.nodes[0]: # convert to potential if possible
                    if voltage_master.nodes[0] == potential_master.nodes[0]:
                        sweep_order_master = i_voltage_master+1 # plus 1 bc 1st swd is root
                        break

            def_sweep.append(
                {'var_name':potential, 'sweep_order':sweep_order_master, 'sweep_type':'SYNC', 'master': potential_master, 'offset':voltage_offset}
            )
    ...
    if specifiers.FREQUENCY in df_to_convert.columns:
        ...
        def_sweep.append(
            {'var_name':specifiers.FREQUENCY, 'sweep_order':sweep_order, 'sweep_type':'LIST', 'value_def':freq},
        )
        is_ac = True
    else:
        is_ac = False
    ...
    return Sweep(name, sweepdef=def_sweep, othervar=vars_other, outputdef=def_output, SweepDefClass=SweepDefClass)
