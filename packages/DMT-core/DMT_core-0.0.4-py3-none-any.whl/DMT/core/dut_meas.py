...
class DutMeas(DutView):
    ...
    nodes_fallback : dict{meas_nodes: dut_type_node}
        Used for reading measurements.
    ...
    def __init__(self, database_dir, name, dut_type, nodes_fallback=None,
        force=False, wafer=None, die=None, ndevices=1, deemb_name=None,
        **kwargs):
    ...
    def deembed(self, filter_func, short_key='dummies/short', open_key='dummies/open'):
        ...
        if len(keys)>0 and no_dummies:
            raise IOError('DMT -> DutMeas -> deembed: Found keys that require deembedding, however no short or open dummies are present in this Dut.')
        ...
        if self.ndevices > 1: #if we have more than one device in parallel, normalize the currents to the area of one device
            for key in self.data.keys():
                df             =  self.data[key]
    ...
    def clean_data(self, fallback=None, **kwargs):
        ...
        if fallback is None:
            fallback = {}
        fb = self.nodes_fallback
        fb.update(fallback)

        for key, df in self.data.items():
            df             =  df.clean_data(self.nodes, self.reference_node, fallback=fb, ac_ports=self.ac_ports, **kwargs)
            self.data[key] = df


    def get_output_data(self, sweep):
        raise NotImplementedError('DMT -> DutMeas -> get_output_data(): DutMeas does not implement this method on purpose!')
    ...
    def join_key_temperature(self, *key_parts, temperature_converter=None):
        ...
        for i_key_part, key_part in enumerate(key_parts):
            ...
            if temp == -1: # this value is returned it the temperature is not in the key.
                continue
            if temp < 0:
                raise ValueError("Got a negative temperature for the key. This is nonphysical since the temperature must be in Kelvin!")
            elif temp < 50:
                warning = "DutMeas -> join_key_temperature: The temperature of a measurement is below 50K. The given measurement is " + self.join_key(*key_parts) + " of the DuT " + self.name + "."
