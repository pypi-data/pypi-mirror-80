...
import copy
import logging
from DMT.core import DatabaseManager, read_data, DataFrame
...
class DutView(object):
    ...
    sim_folder :  str
    inp_name   :  str
        The name of the input file to be generated. (TODO:Mario?)
    save_dir   : string
    ...
    def __init__(self, database_dir, name, dut_type,
    copy_va_files=False, force=False, separate_databases=False, list_copy=None, t_max=None, simulate_on_server=None,
    simulator_command=None, simulator_arguments=None, technology=None, width=None, length=None, nfinger=None, contact_config=None,
    ac_ports=None, reference_node=None, nodes=None, ports=None):
        ...
        if os.path.isabs(name):
            raise IOError("The DuT name must not be an valid absolute directory path!")
        self.name           =  name
        ...
        if (width is not None) and (length is not None) and (not isinstance(length, tuple) and (not isinstance(width, tuple))):
    ...
    def prepare_simulation(self, sweep):
        ...
        sweep : Sweep
        """
        logging.info("Preparing to simulate the Dut %s the sweep with the hash %s", self.get_hash(), sweep.get_hash())
        ...
        print("Prepared a simulation in the folder: \n" + str(sim_folder))
        logging.info("Prepared a simulation in the folder: \n%s", sim_folder)
    ...
    def import_output_data(self, sweep, delete_sim_results=False):
        ...
        """
        raise NotImplementedError('DutView does not implement the concrete import_output_data method!')
    ...
    def get_sim_folder(self, sweep):
        ...
        """
        return self.sim_dir / (self.name + str( self.get_hash() )) / (sweep.name + '_' + sweep.get_hash())
    ...
    def delete_sim_results(self, sweep, ignore_errors=False):
        ...
        logging.info("The simulation data of the sweep %s with the hash %s was deleted", sweep.name, sweep.get_hash())
    ...
    def add_data(self, data, key=None, force=True, **kwargs):
        ...
        if key in self.data.keys():
            if force:
                logging.info('DMT -> DutView -> add_data(): Removed a dataframe with key %s since it was already existent in dut.data.', key)
                self.remove_data(key)
            else:
                logging.info('DMT -> DutView -> add_data(): Skipped a dataframe with key %s since it was already existent in dut.data.', key)
                return
    ...
    def del_db(self):
        ...
        """
        for _root, _dirs, files in os.walk(self.save_dir):
    ...
    def clean_data(self):
        """ Iterate through all keys in the DutView's data and try to clean the column names according to the internal DMT format.
        """
        for key, df in self.data.items():
            self.data[key] = df.clean_data(self.nodes, self.reference_node)

        logging.info('DMT -> DutView -> clean_all_data(): Cleaned dataframes.')
        ...
        if self._separate_databases:
            ...
            for sweep_key in sweep_keys:
                ...
                for key, data in self._data.items():
                    if not key.startswith('_') and self.join_key(*self.split_key(key)[0:-1]) == sweep_key:
    ...
    def get_key_temperature(self, key):
        ...
        * Range of temperatures: "T[xxx.xx-sss.ss-yyy.yy]K", s is the step
        ...
        raise NameError("DMT -> DutMeas -> get_key_temperature: Was not able to extract the temperature from the key: " + key + ".")