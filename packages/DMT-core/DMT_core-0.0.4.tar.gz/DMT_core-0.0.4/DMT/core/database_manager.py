...
class DatabaseManager(object, metaclass=Singleton):
    ...
    def load_db(self, db_dir):
        ...
        if config.USE_HDF5STORE:
            try:
                db = pd.io.pytables.HDFStore(str(db_dir.expanduser()), mode = 'r', complevel = 9, complib='zlib')
    ...
    def save_db(self, db_dir, data):
        ...
        if config.USE_HDF5STORE:
            db = pd.io.pytables.HDFStore(str(db_dir.expanduser()), mode = 'w', complevel = 9, complib='zlib') #file is created if it does not exist
            for key, df in data.items():
