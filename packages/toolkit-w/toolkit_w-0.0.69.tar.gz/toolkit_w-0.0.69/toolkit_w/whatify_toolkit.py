...
class MyWhatify:
    ...
    def run_prediction(cls, data_name, ensemble_id, data_file=None, wait=True, file_path=None, header=None,
        ...
        with allure.step('running prediction on data name: ' + data_name + ' with ensemble id: ' + str(ensemble_id) + ' and wait is: ' + str(wait)):
            ...
            if skip_if_exist:
                ret_list = cls.get_prediction_list(prediction_name=data_name)
                if len(ret_list) > 0:
                    return int(ret_list[0]['id']), True, None
            try:
    ...
    # @classmethod
    # def func_base(cls, func, list_ids):
    #     for id in list_ids:
    #         func(id)
    #
    # @classmethod
    # def delete_data_source_by_name(cls, data_name):
    #     logging.info('deleting data Source with data_name: ' + str(data_name))
    #     data_id_list = my_toolkit.Datasource.list(search_term=data_name)
    #     data_id_list = [d['id'] for d in data_id_list['hits']]
    #     cls.func_base(func=my_toolkit.Datasource.delete, file_id=data_id_list)
    ...
    @classmethod
    def get_prediction_list(cls, prediction_name):
        with allure.step('getting the prediction list for the prediction_name: ' + str(prediction_name)):
            return my_toolkit.Prediction.list(search_term=prediction_name)['hits']
