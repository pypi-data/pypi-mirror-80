...
import threading
import time
from typing import Any, cast, Dict, IO, Iterator, List, Optional
import unicodedata
    ...
    if not resp.ok:
        if resp.status_code == 500:
            raise click.ClickException(
                ...
                "Please report this to the Anyscale team at https://airtable.com/shrz7hmCPWOr8mAdQ "
...
def get_active_sessions(project_id: str, session_name: str) -> Any:
...
def wait_for_session_start(project_id: str, session_name: str) -> str:
...
def get_rsync_command(
    ...
    command_list = ["rsync", "--rsh", '"' + " ".join(ssh_command) + '"', "-avz"]
    if not sync_git:
        command_list += [
            "--exclude='*/.git'",
            "--exclude='*/.git/*'",
        ...
        if os.path.exists(".gitignore"):
            command_list += ["--exclude-from=.gitignore"]
    if modified_files is not None:
        # TODO(pcm): This seems to not be supported on some versions of macOS.
        # We might need to ship an up-to-date version of rsync to make it
        # possible to have files deleted on the server.
        # "--delete-missing-args",
        command_list.append("--files-from={}".format(modified_files.name))
    command_list += [
        source,
